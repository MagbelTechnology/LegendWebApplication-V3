...
public class LoginServlet extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		response.sendRedirect(contextPath + "/DocumentHelp.jsp?np=login");		
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if ((nickname != null && nickname.length() > 3 && nickname.indexOf(" ") == -1) && (sex != null && (sex.equals("m") || sex.equals("f") ) ))
		{
			try
			{
				...
				if (chatterexists)
				{
					response.sendRedirect(contextPath + "/DocumentHelp.jsp?np=login&d=t&n="+nickname);
				}
				else
				{
					...
					response.sendRedirect(contextPath + "/DocumentHelp.jsp?np=listrooms");
			...
			catch(Exception exception)
			{
				...
				response.sendRedirect(contextPath + "/DocumentHelp.jsp?np=error");  
			}
		}
		else
		{
			response.sendRedirect(contextPath + "/DocumentHelp.jsp?np=login&ic=t");
		}
	}
}