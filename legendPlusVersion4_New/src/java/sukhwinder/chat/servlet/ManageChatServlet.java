...
public class ManageChatServlet extends HttpServlet
{
	...
	*/
	...
	{
		System.out.println("-----------------in ManageChatServlet----------------");
		try
		{
			String path = "";
	...
	*/
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (roomName == null || (roomName = roomName.trim()).length() == 0 || roomDescr == null || (roomDescr = roomDescr.trim()).length() == 0)
		{
			...
			getServletContext().getRequestDispatcher("/DocumentHelp.jsp?np=addRoom").forward(request, response);
			return;
		...
		try
		{
			...
			{
				addNewRoom(rooms, roomName, roomDescr);
				response.sendRedirect(request.getContextPath() + "/DocumentHelp.jsp?np=listrooms");
