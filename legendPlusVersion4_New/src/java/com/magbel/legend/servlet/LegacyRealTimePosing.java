...
public class LegacyRealTimePosing extends HttpServlet
{
...
// 	   if(!BatchApiUrl.equals("")){
	   ...
	   JSONObject json = new JSONObject(dateStatus);
	   String postingDate = json.getJSONObject("data").getString("postingDate");
...
// 	  }
		...
		debit.setTrnRefNo(transactionId);
		debit.setTxNarrative(costDrNarration);
		debit.setFcyAmt(0);
		debit.setValueDt(valueDate);
		debit.setDrCrInd("D");
		debit.setExchRate(0);
		debit.setRelatedCustomer(MenuPage);
		debit.setInstrumentCode("");
		debit.setLcyAmt(costDrAcct);
		...
		credit.setLcyAmt(costCrAcct);
		credit.setRelatedCustomer("");
		credit.setTrnCode("MSC");
		credit.setTrnRefNo(transactionId);
		...
		try {
			...
			JSONObject debitObject = new JSONObject(debitDetails);
			JSONObject creditObject = new JSONObject(creditDetails);
			...
			//accountingDetails.setUserId(Integer.parseInt(lgcyUserId));
			accountingDetails.setUserId(lgcyUserId);
			accountingDetails.setSource("LEGEND");
			//accountingDetails.setAccEntryDetails(ccy);
//		accountingDetails.setAccEntryDetails(arr.toString().replace("\"", ""));
		...
		System.out.println("API Response >>>> " + apiResponse);
		   ...
		   String accountId = responseJSON.getJSONObject("data").getString("accountId");
		...
		String legacyTransId="LGD000235";
