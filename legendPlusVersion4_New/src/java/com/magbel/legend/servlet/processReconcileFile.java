...
import magma.net.manager.ReconcileExcelUploadManager;

@WebServlet(
   name = "processReconcileFile"
)
@MultipartConfig(fileSizeThreshold = 1024 * 1024,
maxFileSize = 1024 * 1024 * 5, 
maxRequestSize = 1024 * 1024 * 5 * 5)
public class processReconcileFile extends HttpServlet {
   private static final long serialVersionUID = 1L;

   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      System.out.println("Calling Process Asset Reconcile Upload File");
    	String payFile = "";
    	String dname = "";
    	int quantity = 1;
    	...
    	Part part = request.getPart("file");
             ...
             File savedFile = new File(getServletContext().getRealPath("/"),part.getSubmittedFileName());
              // File savedFile = new File(home + "\\Downloads",part.getSubmittedFileName());
               ...
               ReconcileExcelUploadManager excelManager = new ReconcileExcelUploadManager(uid);
    		//it.write(savedFile);
          part.write(savedFile.toString());
    		...
    		if(!list.isEmpty())
    		{
    			...
    			out.print("<script>location.replace('DocumentHelp.jsp?np=uploadAssetReconcileDetail');</script>");
