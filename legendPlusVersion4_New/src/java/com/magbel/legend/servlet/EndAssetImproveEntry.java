...
public class EndAssetImproveEntry extends HttpServlet
{
...
//        System.out.println("destination ================== "+destination);
 ...
 //       System.out.println("strCalcCost"+strCalcCost+"  id: "+id+"   tranId: "+tranId);
        ...
        */
        String transIdCost = "26";
        String transIdVendor = "27";
        String transIdWitholding = "28";
  ...
  //      String isoValueReverse=records.getCodeName("select count(*) from am_Raisentry_Transaction where Trans_id='"+tranIdReverse+"' and page1 = 'ASSET IMPROVEMENT RAISE ENTRY' and iso != '000' ");         
        ...
        String  reversalStatus = records.getCodeName("SELECT REVERSED FROM am_asset_improvement WHERE revalue_Id = '"+tranId+"'"); 
        ...
        try
        {
            ...
            {
           	assetMan.processImprovement(id,CalcCost,CalcNBV,Calcvatable,CalcVatAmt,CalcWhtAmt,usefullife,newCost,newVatableCost,nbvresidual,tranIdInt);
           	String mailaddress=records.getCodeName("select email from am_ad_branch where BRANCH_CODE='"+branchCode+"'");
           	String to = mailaddress;
           	String subject = "Asset Improvement";
...
//            	System.out.println("<<<<<<<<<<<<<<<<Reversal isoValueReverse ================== "+isoValueReverse);
           	...
           	assetMan.processImprovementReversal(id,oldCost,oldNbv,oldVatableCost,oldVatAmt,oldWhtAmt,usefullife,oldimprovCost,oldimprovvatableCost,nbvresidual,tranIdInt,oldimprovNBV,oldimprovaccum);
           	String mailaddress=records.getCodeName("select email from am_ad_branch where BRANCH_CODE='"+branchCode+"'");
           	String to = mailaddress;
           	String subject = "Asset Improvement";
