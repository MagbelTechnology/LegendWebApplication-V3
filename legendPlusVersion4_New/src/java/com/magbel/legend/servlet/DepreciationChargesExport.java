...
public class DepreciationChargesExport extends HttpServlet
{
	/**
	 * 
	 */
...
//    if(exists==false){
            ...
            String branchCodeSelected = records.getCodeName("select BRANCH_CODE from am_ad_branch where BRANCH_ID = ? ",branch_Id);
            System.out.println("<<<<<<chargeYear: "+chargeYear+"    reportDate: "+reportDate+"  branch_Id: "+branch_Id);
            String ColQuery = "";
            /*
            ...
            */
            if(branch_Id.equals("***")  && categoryId.equals("***")){
                ColQuery ="SELECT *FROM MonthlyDeprCharcges_View WHERE CHARGEYEAR = ? AND DEP_DATE = ?";
            }
            if(!branch_Id.equals("***")  && categoryId.equals("***")){
                ColQuery ="SELECT *FROM MonthlyDeprCharcges_View WHERE CHARGEYEAR = ? AND DEP_DATE = ? AND BRANCH_CODE = ?";
            }
            if(branch_Id.equals("***")  && !categoryId.equals("***")){
                ColQuery ="SELECT *FROM MonthlyDeprCharcges_View WHERE CHARGEYEAR = ? AND DEP_DATE = ? AND CATEGORY_CODE = ?";
            }
            if(!branch_Id.equals("***")  && !categoryId.equals("***")){
                ColQuery ="SELECT *FROM MonthlyDeprCharcges_View WHERE CHARGEYEAR = ? AND DEP_DATE = ? AND BRANCH_CODE = ? AND CATEGORY_CODE = ?";
            }
            ColQuery = ColQuery+" AND ASSET_STATUS = 'ACTIVE' ";
            System.out.println("======>>>>>>>ColQuery: "+ColQuery);
         //   java.util.ArrayList list =rep.getDepreciationChargesExportRecords(ColQuery);
            java.util.ArrayList list =rep.getDepreciationChargesExportRecords(ColQuery,chargeYear,reportDate,branchCodeSelected,categoryId);
            if(list.size()!=0){
