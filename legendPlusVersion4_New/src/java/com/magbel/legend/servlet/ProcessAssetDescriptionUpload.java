...
@WebServlet(
   name = "ProcessAssetDescriptionUpload"
)
@MultipartConfig(fileSizeThreshold = 1024 * 1024,
maxFileSize = 1024 * 1024 * 5, 
maxRequestSize = 1024 * 1024 * 5 * 5)
public class ProcessAssetDescriptionUpload extends HttpServlet {
   private static final long serialVersionUID = 1L;

   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      System.out.println("Calling Process Asset Description Upload File");
   ...
   // File savedFile = new File(home + "\\Downloads",part.getSubmittedFileName());
	...
	ExcelAssetDescriptionUploadManager excelManager = new ExcelAssetDescriptionUploadManager(userid);
	//it.write(savedFile);
   ...
   ArrayList list = excelManager.getFileFromServer(savedFile);
	...
	out.println("<script>window.location='DocumentHelp.jsp?np=uploadAssetDescriptionDetail';</script>");
//	response.sendRedirect("DocumentHelp.jsp?np=uploadStaffDetail");
		...
		session.setAttribute("PATH",payFile);
		   ...
		   out.println("location='DocumentHelp.jsp?np=excelAssetDescriptionUpload';");
		   out.println("</script>");
		}
