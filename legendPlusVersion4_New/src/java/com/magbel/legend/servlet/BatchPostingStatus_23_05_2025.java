...
public class BatchPostingStatus_23_05_2025 extends HttpServlet
{
...
//   	   		records.updateAssetWithBatchId(tableName, columnName,  whereFld,  groupid, status);
	        ...
	        String qap = "update a SET a.process_status = 'A',a.asset_status = 'ACTIVE' from am_asset_approval a, am_group_asset g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.group_Id) and POSTING_BATCH = '"+groupid+"'";
...
//   	   	records.updatePostWithBatchId(tableName, posttable, groupid,status1,status2);
  		 ...
  		 status2 = "ACTIVE";
//  		records.updatePostWithBatchId(tableName, posttable, groupid, status1, status2);
        String qa = "update a SET a.process_status = 'A',a.asset_status = 'ACTIVE' from am_asset_approval a, am_asset_improvement g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.asset_id) and POSTING_BATCH = '"+groupid+"'";
        ad.updateAssetStatusChange(qa);	   	   	
   		tableName = "AM_ASSET";
...
//      		records.updatePostWithBatchId(tableName, posttable, groupid, status1, status2);
	        String qd = "update a SET a.Asset_Status = 'DISPOSED' from AM_ASSET a, am_assetdisposal g where CONVERT(VARCHAR, a.Asset_id) = CONVERT(VARCHAR, g.asset_id) and g.POSTING_BATCH = '"+groupid+"'";
...
//      		records.updatePostWithBatchId(tableName, posttable, groupid, status1, status2);
           ...
           if(tableName.equals("am_assetTransfer")){
        	   ...
        	   System.out.println("am_assetTransfer 1");
          		...
          		System.out.println("am_assetTransfer 3");
           		String query_trt ="update g SET g.approval_status = 'POSTED' from am_asset_approval a, am_assetTransfer g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.asset_id) and POSTING_BATCH = '"+groupid+"' ";     		
           		ad.updateAssetStatusChange(query_trt); 
           		System.out.println("am_assetTransfer 4");
           		posttable = "am_asset_approval";
         		 status1 = "A";
         		 status2 = "ACTIVE";     
           		String query_tapp ="update a SET a.process_status = 'A',a.asset_status = 'ACTIVE' from am_asset_approval a, am_assetTransfer g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.asset_id) and POSTING_BATCH = '"+groupid+"' ";     		
           ...
           if(tableName.equals("am_gb_bulkTransfer")){
        	   ...
        	   System.out.println("am_gb_bulkTransfer 2");
          		...
          		System.out.println("am_gb_bulkTransfer 4");
           		String query_tapp ="update a SET a.process_status = 'A',a.asset_status = 'ACTIVE' from am_asset_approval a, am_gb_bulkTransfer g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.Batch_id) and POSTING_BATCH = '"+groupid+"' ";     		
           		ad.updateAssetStatusChange(query_tapp);   
           ...
           if(tableName.equals("am_assetReclassification")){
        	   ...
        	   records.updatePostingRecordScript("UPDATE a SET a.Asset_id  = b.new_asset_id,a.OLD_ASSET_ID = b.Asset_id, a.Category_ID = b.new_category_id,a.CATEGORY_CODE = c.category_code, "
        	   		...
        	   		+ "from  am_asset a, am_assetReclassification b, am_ad_category c where a.Asset_id = b.Asset_id and a.Category_ID = c.category_ID and b.status is null and a.NBV != 10.00 and b.POSTING_BATCH = ? ",groupid);
        	   records.updatePostingRecordScript("UPDATE a SET a.Asset_id  = b.new_asset_id,a.OLD_ASSET_ID = b.Asset_id, a.Category_ID = b.new_category_id,a.CATEGORY_CODE = c.category_code, "
           	   		+ "a.Monthly_Dep = b.new_monthly_dep,a.Dep_Rate = b.new_depr_rate,a.Dep_End_Date = b.new_dep_end_date "
           	   		+ "from  am_asset a, am_assetReclassification b, am_ad_category c where a.Asset_id = b.Asset_id and a.Category_ID = c.category_ID and b.status is null and a.NBV = 10.00 and b.POSTING_BATCH = ? ",groupid);
      		 ...
      		 status1 = "A";
      		 status2 = "ACTIVE";     
        		String query_tapp ="update a SET a.process_status = 'A',a.asset_status = 'ACTIVE' from am_asset_approval a, am_assetReclassification g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.asset_id) and POSTING_BATCH = '"+groupid+"' ";     		
        		ad.updateAssetStatusChange(query_tapp);   
...
//           		records.updateAssetReclassWithBatchId(tableName, posttable, groupid);
           ...
           if(tableName.equals("am_assetUpdate")){
        	   ...
        	   records.updatePostingRecordScript("UPDATE a SET a.Asset_Status  = b.Asset_Status "
        	   		+ "from  am_asset a, am_assetUpdate b, am_ad_category c where a.Asset_id = b.Asset_id and a.Category_ID = c.category_ID and b.Asset_Status is null and b.POSTING_BATCH = ? ",groupid);
...
//           		records.updateTransferWithBatchId(tableName, posttable, groupid);
        		String query_tapp ="update a SET a.process_status = 'A',a.asset_status = 'ACTIVE' from am_asset_approval a, am_AcceleratedDepreciation g where CONVERT(VARCHAR, a.asset_id) = CONVERT(VARCHAR, g.asset_id) and POSTING_BATCH = '"+groupid+"' ";     		
        		ad.updateAssetStatusChange(query_tapp); 
