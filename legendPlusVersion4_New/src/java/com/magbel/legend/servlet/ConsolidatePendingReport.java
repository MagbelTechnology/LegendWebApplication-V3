...
//    String branch_Code = request.getParameter("initiatorSOLID");
    String branch_Id = request.getParameter("branch").equals("") ? "0" : request.getParameter("branch");
    if(!branch_Id.equals("0")) {
    branch_Id = records.getCodeName("select BRANCH_CODE from am_ad_branch where BRANCH_ID = ? ",branch_Id);
    }
...
//   System.out.println("<<<<<<bran ch_Id: "+branch_Id+"    categoryCode: "+categoryCode+"  branchCode: "+branchCode);
  System.out.println("<<<<<<branch_Id: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate);
     String ColQuery = "select * from Asset_PendingTranaction_View WHERE 1=1";
     ...
     if(list.size()!=0){
   	 if(report.equalsIgnoreCase("rptMenuPendingConsolid")){
         ...
         rowhead.createCell( 4).setCellValue("Asset Status");
         rowhead.createCell( 5).setCellValue("Tran Type");
         rowhead.createCell( 6).setCellValue("Transaction Date");
	 ...
	 for(int k=0;k<list.size();k++)
     {
    	 com.magbel.legend.vao.newAssetTransaction  newassettrans = (com.magbel.legend.vao.newAssetTransaction)list.get(k);    	 
			...
			String transactionDate = newassettrans.getPostingDate() != null ? getDate(newassettrans.getPostingDate()) : "";
			...
			row.createCell((short) 4).setCellValue(assetStatus);
			row.createCell((short) 5).setCellValue(tranType);
			row.createCell((short) 6).setCellValue(transactionDate);
