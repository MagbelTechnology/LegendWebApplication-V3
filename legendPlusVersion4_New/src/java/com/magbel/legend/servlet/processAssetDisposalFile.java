...
public class processAssetDisposalFile extends HttpServlet {
...
//             System.out.println("Trimed file to be saved to the disk is "+savedFile);
             DisposalExcelUploadManager excelManager = new DisposalExcelUploadManager(uid,groupAssetByAsset,groupid);
  		//it.write(savedFile);
        part.write(savedFile.toString());
  		...
  		if(!list.isEmpty())
  		{
  			...
  			out.print("<script>location.replace('DocumentHelp.jsp?np=uploadAssetDisposalDetail&gid="+groupid+"');</script>");
