...
import magma.net.manager.ExcelMobileRegistrationUploadManager;
...
public class processMobileRegistrationFile extends HttpServlet {
   private static final long serialVersionUID = 1L;

   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      System.out.println("Calling Process Mobile Registration Upload File");
...
//  File savedFile = new File(home + "\\Downloads",part.getSubmittedFileName());
	...
	ExcelMobileRegistrationUploadManager excelManager = new ExcelMobileRegistrationUploadManager(userid);
	//it.write(savedFile);
	...
	if(!list.isEmpty())
	{
       ...
       out.print("<script>window.location='DocumentHelp.jsp?np=excelMobileRegistrationUpload';</script>");
	...
	out.println("<script>window.location='DocumentHelp.jsp?np=uploadMobileRegistrationDetail';</script>");
 //out.print("<script>window.location='DocumentHelp.jsp?np=uploadStaffCreationDetail';</script>");
		...
		session.setAttribute("PATH",payFile);
		   ...
		   out.println("location='DocumentHelp.jsp?np=excelMobileRegistrationUpload';");
		   out.println("</script>");
	}
