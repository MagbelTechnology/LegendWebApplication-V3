 ...
 */
public class BulkUncapUploadProcessServlet extends HttpServlet {
    ...
    public BulkUncapUploadProcessServlet()
    {
        super();
        try
        {
...
//		System.out.println("<<<<<<<<=====vendoramount: "+vendoramount);
		...
		String status = "Uploaded Uncapitalized Assets have been made Available in the Register Ensure Bulk Posting is done on Legacy System"; 
...
//		System.out.println("<<<<<<<<raiseBtn: "+raiseBtn+"     reject: "+reject);
		...
		String userId =(String)session.getAttribute("CurrentUser");
		  ...
		  try
			  ...
			  java.util.ArrayList list =assetMan.getUploadUncapitalizedAssetRecords(groupid);
			  System.out.println("<<<<<<<<=====list.size() from getUploadUncapitalizedAssetRecords: "+list.size());
			     for(int k=0;k<list.size();k++)
			     {
					...
					ad.updateAssetStatusChange("update AM_ASSET_UNCAPITALIZED set ASSET_STATUS = 'ACTIVE' where GROUP_ID='"+groupid+"' AND ASSET_ID = '"+assetid+"'");
					ad.updateAssetStatusChange("update AM_GROUP_ASSET_UNCAPITALIZED set ASSET_STATUS = 'ACTIVE', RAISE_ENTRY = 'Y' where GROUP_ID='"+groupid+"' AND ASSET_ID = '"+assetid+"'");
	                ...
	                String msgText1 = "Your transaction for Bulk Uncapitalized Asset Upload with Batch ID " + groupid + " has been Posted. The new Batch ID is " + groupid;
	                mail.sendMailTransactionInitiator(Integer.parseInt(groupid), "Bulk Uncapitalized Asset Upload", msgText1);
			  }
...
//	                System.out.println(">>>>>>>>>>> About to send Mail on the Batch "+groupid);
	                ...
	                out.print("<script>window.location='DocumentHelp.jsp?np=raiseEntryBranch&batchId="+groupid+"'</script>");
		        }
		        }
		        catch(Exception e)
		        {
	...
	public void DeletebulkUncapUploadRejections(String batchId){
		String query1 ="delete from AM_GROUP_ASSET_UNCAPITALIZED where GROUP_ID = '"+batchId+"'";
		String query2 ="delete from AM_ASSET_UNCAPITALIZED where GROUP_ID = '"+batchId+"'";
		...
		try {
			con = mgDbCon.getConnection("legendPlus");
				...
				int i =ps.executeUpdate();
		            ...
		            System.out.println("BulkUncapUploadRejectionServlet: DeletebulkUploadRejections() for AM_ASSET>>>>>" + ex);
		        } finally {
		            closeConnection(con, ps);
		        }		
		try {
			con = mgDbCon.getConnection("legendPlus");
				...
				int i =ps.executeUpdate();
		            ...
		            System.out.println("BulkUncapUploadRejectionServlet: DeletebulkUploadRejections() for am_group_asset_main>>>>>" + ex);
		        } finally {
		            closeConnection(con, ps);
		        }		
		try {
			con = mgDbCon.getConnection("legendPlus");
				...
				int i =ps.executeUpdate();
		            ...
		            System.out.println("BulkUncapUploadRejectionServlet: DeletebulkUploadRejections() for am_asset_approval>>>>>" + ex);
		        } finally {
		            closeConnection(con, ps);
		        }			
		}	
