...
public class ProcessPartPayCost extends HttpServlet
{
 ...
 //       String costcreditAcctNo = costcreditAccount.substring(8,16);
        System.out.println("<<<<<<<costcreditAccount: "+costcreditAccount+"    =======costdebitAccount: "+costdebitAccount);
     //   String costdebitAcctNo = costdebitAccount.substring(8,16);
        if(costdebitAccount.substring(0,2)=="NGN"){costdebitAcctNo = costdebitAccount.substring(8,16);}
        if(costdebitAccount.substring(2,5)=="NGN"){costdebitAcctNo = costdebitAccount.substring(6,14);}
  //      String costcreditAcctNo = costcreditAccount.substring(8,16);
        if(costcreditAccount.substring(0,2)=="NGN"){costcreditAcctNo = costcreditAccount.substring(8,16);}
        if(costcreditAccount.substring(2,5)=="NGN"){costcreditAcctNo = costcreditAccount.substring(6,14);}
        String costamount = request.getParameter("cost_amount");
        costamount = costamount.replaceAll(",","");
        String supervisor = request.getParameter("supervisor");
        String pid = request.getParameter("pid");
        String transactionId = request.getParameter("transactionIdPartPayCost");
     ...
     if(ad.isoChecking(   id, page1, transactionId,tranId))
       ...
       // iso=bus.interaccounttransferFund(costdebitNarration,costdebitAccount,costcreditAccount,Double.parseDouble(costamount),finacleTransId,SbuCode,SbuCode);
         ...
         //  ad.insertRaiseEntryTransaction(userId,costdebitNarration,costdebitAccount,costcreditAccount,Double.parseDouble(costamount),iso,id,page1,transactionId);
            ad.insertRaiseEntryTransaction(userId,raiseEntryNarration,costdebitAccount,costcreditAccount,Double.parseDouble(costamount),iso,id,page1,transactionId,systemIp,tranId,assetCode,finacleTransId,costdebitAcctName,costcreditAcctName,userId,recType);
              ...
              ad.insertVendorTransaction(userId,costcreditAccount,costdebitAccount,costcreditAccount,Double.parseDouble(costamount),location,transactionId,page1,projectCode,vendorId);
        }
