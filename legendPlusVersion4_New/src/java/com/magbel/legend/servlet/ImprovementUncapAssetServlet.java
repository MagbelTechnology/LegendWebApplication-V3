...
public class ImprovementUncapAssetServlet extends HttpServlet
{
     ...
     
   {
	...
	String description = aprecords.getCodeName("SELECT DESCRIPTION from AM_ASSET_UNCAPITALIZED where asset_id = '" + id+"'") ;
	  ...
	  int numOfTransactionLevel =  ad.getNumOfTransactionLevel("71");
	  ...
	  String[] pa = new String[12];
		 ...
		 pa[6]= effDate; 
		 pa[8]="ACTIVE"; 
		 pa[9]="Uncapitalized Improvement"; 
		 pa[10]="P"; 
...
//	    System.out.println("invnumb >>>>>>>>>>>>>>>> "+invnumb);
	 htmlUtil.insToAm_Invoice_No(id,lpo,invnumb,"Uncapitalized Improvement"); 
	 ...
	 */ 
	 ...
	 branchCode =aprecords.getCodeName("SELECT branch_code from AM_ASSET_UNCAPITALIZED where asset_id = '" + id+"'") ;
	categoryCode =aprecords.getCodeName("SELECT category_code from AM_ASSET_UNCAPITALIZED where asset_id = '" + id+"'");
	  
	oldAccumDep = Double.parseDouble(aprecords.getCodeName("SELECT Accum_Dep FROM AM_ASSET_UNCAPITALIZED WHERE ASSET_ID = '"+id+"'"));
		oldNbv = Double.parseDouble(aprecords.getCodeName("SELECT NBV FROM AM_ASSET_UNCAPITALIZED WHERE ASSET_ID = '"+id+"'"));
...
//		System.out.println("KKKKKKKKKKKKKKKKKKKKKKKKk oldAccumDep "+oldAccumDep);
	...
	//System.out.println("################################## about to raise entry 1");
		...
		String assetRaiseEntry =aprecords.getCodeName(" SELECT raise_entry from am_gb_company ");
		 String[] raiseEntryInfo = aprecords.raiseEntryInfo(id);
			...
			String url = "DocumentHelp.jsp?np=uncapitalizedImprovementRaiseEntry&id=" +id + "&operation=1&exitPage=manageMaitenance&pageDirect=Y";
	...
	out.print("<script> alert('Asset improvement successful')</script>");
	out.print("<script> window.location='DocumentHelp.jsp?np=manageUncapiImprovement'</script>");
	 }
	 else{  
	...
	mtid = mtid + user_Id;
	 ...
	 String revalue_query = "update am_Uncapitalized_improvement set approval_Status='PENDING' where revalue_id = '"+mtid+"'";
	...
	String subjectr ="Uncapitalized Asset Improvement Approval";
	String msgText11 ="Asset with ID: "+ id +" for Uncapitalized Improvement is waiting for your approval.";
	...
	String otherparam = "assetImprovementApproval&operation=1&id="+id+"&tranId="+approvaltransId+"&transaction_level=1&approval_level_count=0&assetCode="+assetCode;				
	 ...
	 out.print("<script> alert('Uncapitalized Asset Improvement Transaction submitted to "+supervisorName+" for approval')</script>");
	out.print("<script> window.location='DocumentHelp.jsp?np=manageUncapImprovement'</script>");
	 }
...
//				System.out.println("SupervisorId#$$$$$$$$$$$ "+supervisorId);
		  		 ad.setPendingTransMultiApp(pa,"71",assetCode,supervisorId,mtid);
...
//			 ad.setPendingMultiApprTransArchive(ad.setApprovalData(id),"1",Integer.parseInt(lastMTID),assetCode,supervisorId);
	...
	String subjectr ="Uncapitalized Asset Improvement Approval";
	String msgText11 ="Uncapitalized Asset with ID: "+ id +" for Improvement is waiting for your approval.";
	...
	out.print("<script> alert('Uncapitalized Asset Improvement Transaction submitted for approval')</script>");
	out.print("<script> window.location='DocumentHelp.jsp?np=manageUncapImprovement'</script>");
...
//	                 System.out.println("The new file path is >=: " + newPath);
	  ...
	  else if(opt.equals("1"))
	  {
	   ...
	   {
	    assetMan.deleteAssetmMaintenance(id);
	    i = assetMan.insertAssetMaintanance(cost,nbv,id,0.0,reason,revDate,userId,vatableCost,vatAmt,whtAmt,vendorAcct,raiseEntry,accumDep,oldCost,oldVatableCost,oldVatAmt,oldWhtAmt,oldNbv,oldAccumDep,effDate,mtid,assetCode,location,projectCode,sbuCode);
	   ...
	   if(i > 0 )
	   {
	    ...
	    response.sendRedirect("DocumentHelp.jsp?np=uncapitalizedImprovement&id="+id+"&operation=1&exitPage=revaluedAssetList");
	   }
	  }
	  else
	  {}
