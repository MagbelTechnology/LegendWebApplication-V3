...
import magma.net.manager.FuelExcelUploadManager;

@WebServlet(
   name = "processFuelFile"
)
@MultipartConfig(fileSizeThreshold = 1024 * 1024,
maxFileSize = 1024 * 1024 * 5, 
maxRequestSize = 1024 * 1024 * 5 * 5)
public class processFuelFile extends HttpServlet {
   private static final long serialVersionUID = 1L;

   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      System.out.println("Calling process Fuel file");
  	String payFile = "";
  	String dname = "";
  	int quantity = 1;
...
//             System.out.println("Trimed file to be saved to the disk is "+savedFile);
         FuelExcelUploadManager excelManager = new FuelExcelUploadManager(uid,transType);
  		//it.write(savedFile);
		...
		if(transType.equals("F")){recType = "Fuelling";}
		if(transType.equals("I") || transType.equals("V")){out.print("<script>location.replace('DocumentHelp.jsp?np=Fleet_FuelUpdate&recType="+recType+"');</script>");}
		else{out.print("<script>location.replace('DocumentHelp.jsp?np=FleetFuelUploadDetail');</script>");}
