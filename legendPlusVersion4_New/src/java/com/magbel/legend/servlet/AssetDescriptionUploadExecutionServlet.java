...
import legend.admin.objects.AssetDescription;
...
public class AssetDescriptionUploadExecutionServlet extends HttpServlet {
    /**
	 * 
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String ColQuery ="select * from AM_ASSET_DESCRIPTION_UPLOAD where DESCRIPTION not in (select DESCRIPTION from AM_ASSET_DESCRIPTION)";
		  java.util.ArrayList list =getUploadDescriptionSqlRecords(ColQuery);
//			try {		
...
//				  System.out.println("list.size() ============================ "+list.size());
			...
			legend.admin.objects.AssetDescription  assetDescription = (legend.admin.objects.AssetDescription)list.get(k);    	 					
...
//	       String RecordId = usr.getUserName();
			String subCatgeoryCode = assetDescription.getSubCategoryCode();
			String description = assetDescription.getDescription();
			String decsriptionStatus = assetDescription.getDescriptionStatus();
			String createdBy = assetDescription.getUserId();
			String createdDate = assetDescription.getCreateDate();
			
			assetDescription.setSubCategoryCode(subCatgeoryCode);
			assetDescription.setDescription(description);
			assetDescription.setDescriptionStatus(decsriptionStatus);
         ...
         //  System.out.println("staffId ================= "+staffId+"  fullname: "+fullname+"  createdBy: "+user_Id);
           ...
           boolean value = StaffUpload(assetDescription);

           if(value == true) {
        	   out.print("<script>alert('Upload Done Successfully!!!')</script>");
				out.print("<script>window.location = 'DocumentHelp.jsp?np=manageAssetDescriptions&status=ACTIVE'</script>");
           }
		     }
    ...
    public java.util.ArrayList getUploadDescriptionSqlRecords(String query)
    {
...
//         System.out.println("====query in getUploadUserSqlRecords-----> "+query);
    	...
    	ResultSet rs = null;
    	Statement s = null; 
    	 legend.admin.objects.AssetDescription assetDescription = null;
    	try {
    		    c = getConnection();
    			s = c.createStatement();
    			...
    			while (rs.next())
    			   {				
                    ...
                    String descriptionStatus = rs.getString("STATUS");
                    String createdBy = rs.getString("USER_ID");
                    String createdDate = rs.getString("CREATE_DATE");
                    
                    
                    assetDescription = new legend.admin.objects.AssetDescription();
                    assetDescription.setSubCategoryCode(subCategoryCode);
                    assetDescription.setDescription(description);
                    assetDescription.setDescriptionStatus(descriptionStatus);
                    assetDescription.setUserId(createdBy);
                    assetDescription.setCreateDate(createdDate);
                  
                    _list.add(assetDescription);				
    			   }
    	 }    
    				 catch (Exception e)
    					{
    ...
    public boolean StaffUpload(AssetDescription description) {

		String UPDATE_QUERY = "insert into AM_ASSET_DESCRIPTION (sub_category_code, description, status, user_id, create_date) values (?, ?, ?, ?, ?)";
		...
		try {
			...
			ps.setString(1, description.getSubCategoryCode());
			ps.setString(2, description.getDescription());
			ps.setString(3, description.getDescriptionStatus());
			ps.setString(4, description.getUserId());
			ps.setString(5, description.getCreateDate());
			done = ps.execute();
