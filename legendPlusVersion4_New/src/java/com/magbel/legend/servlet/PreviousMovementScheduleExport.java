...
public class PreviousMovementScheduleExport extends HttpServlet
{
...
//    String userName = request.getParameter("userName");
    String fileName = branch_Code+"By"+userName+"PreviousMovementScheduleExportReport.xlsx";    	
...
//    if(exists==false){   	
   ...
   System.out.println("<<<<<<branch_Id: "+branch_Id+"    categoryCode: "+categoryCode+"  branchCode: "+branchCode);
     ...
     if(FromDate.equals("")  && ToDate.equals("")){
	     ...
	     ColQuery ="select '1' AS SERIAL,'BALANCES AS AT BEGINING OF YEAR' AS NARRATION, * from (select class_name, cost_open_bal from fixed_asset_schedule) fixed_asset_schedule "
	     		...
	     		+ "    ) ) as pivottable "
	     		+ "UNION "
	     		+ "select '6' AS SERIAL,'BALANCES AS AT BEGINING OF YEAR', * from (select class_name, dep_open_bal from fixed_asset_schedule) fixed_asset_schedule "
	     		...
	     		+ "    ) ) as pivottable "
	     		+ "UNION "
	     		+ "select '7' AS SERIAL,'CHARGE FOR THE YEAR' AS NARRATION, * from (select class_name,dep_charge from fixed_asset_schedule) fixed_asset_schedule"
	     		...
	     		+ "    ) ) as pivottable "
	     		+ "UNION "
	     		+ "select '8' AS SERIAL,'TRANSFER' AS NARRATION, * from (select class_name,dep_disposal from fixed_asset_schedule) fixed_asset_schedule"
	     		...
	     		+ "    ) ) as pivottable "
	     		+ "UNION "
	     		+ "select '9' AS SERIAL,'DISPOSAL' AS NARRATION,* from (select class_name,dep_disposal from fixed_asset_schedule) fixed_asset_schedule"
	     		+ "    pivot(sum(dep_disposal) for class_name"
	     		+ "    in ([FURNITURE FITTINGS & EQUIPMENT],[COMPUTER EQUIPMENT],[MACHINE & EQUIPMENT],[INTANGIBLE ASSETS],"
	     		+ "    [MOTOR VEHICLES],"
	     		+ "    [LAND & LEASEHOLD] "
	     		+ "    ) ) as pivottable "
	     		+ "UNION "
	     		+ "select '9.1' AS SERIAL,'NBV AS YEAR END' AS NARRATION,* from (select class_name,nbv_closing_bal from fixed_asset_schedule) fixed_asset_schedule"
	     		...
	     		+ "    ) ) as pivottable "
	     		+ "UNION "
	     		+ "select '9.2' AS SERIAL,'NBV OPENING BALANCE' AS NARRATION,* from (select class_name,nbv_open_bal from fixed_asset_schedule) fixed_asset_schedule"
