...
import magma.net.manager.AssetBidExcelUploadManager;

@WebServlet(
   name = "processAssetBidFile"
)
@MultipartConfig(fileSizeThreshold = 1024 * 1024,
maxFileSize = 1024 * 1024 * 5, 
maxRequestSize = 1024 * 1024 * 5 * 5)
public class processAssetBidFile extends HttpServlet {
   private static final long serialVersionUID = 1L;

   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      System.out.println("Calling Process Asset Bid Upload File");
  	String payFile = "";
  	String dname = "";
  	int quantity = 1;
	...
	try {
  	...
  	Part part = request.getPart("file");
          ...
          //   File savedFile = new File(home + "\\Downloads",part.getSubmittedFileName());
             ...
             AssetBidExcelUploadManager excelManager = new AssetBidExcelUploadManager(uid);
  		//it.write(savedFile);
        part.write(savedFile.toString());
  		...
  		if(!list.isEmpty())
  		{
  			...
  			out.print("<script>location.replace('DocumentHelp.jsp?np=uploadAssetBidDetail');</script>");
  			...
  			session.setAttribute("PATH",payFile);
  				...
  				out.println("<script type=\"text/javascript\">");
				   ...
				   out.println("location='DocumentHelp.jsp?np=bidTransferUpload';");
				   out.println("</script>");
  			}
  	/*
  	out.print("<script>");
