    ...
    if(reportType.equals("BankWide")){
     if(FromDate.equals("")  && ToDate.equals("")){
	     ...
	     ColQuery ="select '1' AS SERIAL,'COST/VALUATION BALANCES AS AT BEGINING OF YEAR' AS NARRATION, * from (select class_name, cost_open_bal from fixed_asset_schedule) fixed_asset_schedule "
	     		...
	     		+ ") ) as pivottable "	   
	     		+ "UNION   "
	     		+ "select '5.2' AS SERIAL,'CLOSE ASSET' AS NARRATION,* from (select class_name,cost_CloseAsset from fixed_asset_schedule) fixed_asset_schedule "
	     		+ "pivot(sum(cost_CloseAsset) for class_name "
	     		+ "in ([BUILDING],[LAND],[LEASEHOLD IMPROVEMENT],[FURNITURE FITTINGS & EQUIPMENT],[COMPUTER EQUIPMENT],[MOTOR VEHICLES],[AIRCRAFT],"
	     		+ "[COMPUTER SOFTWARE] "
	     		+ ") ) as pivottable "		     		
	     		+ "UNION   "
	     		...
	     		+ ") ) as pivottable "
	     		+ "UNION "
	     		+ "select '8.1' AS SERIAL,'CLOSE ASSET CHARGE ' AS NARRATION, * from (select class_name,dep_CloseAsset from fixed_asset_schedule) fixed_asset_schedule "
	     		+ "pivot(sum(dep_CloseAsset) for class_name "
	     		+ "in ([BUILDING],[LAND],[LEASEHOLD IMPROVEMENT],[FURNITURE FITTINGS & EQUIPMENT],[COMPUTER EQUIPMENT],[MOTOR VEHICLES],[AIRCRAFT],"
	     		+ "[COMPUTER SOFTWARE] "
	     		+ ") ) as pivottable "	     		
	     		+ "UNION "
    ...
    if(reportType.equals("ByBranch")){
        if(FromDate.equals("")  && ToDate.equals("")){
   	     ...
   	     ColQuery ="select '1' AS SERIAL,'COST/VALUATION BALANCES AS AT BEGINING OF YEAR' AS NARRATION, * from (select class_name, cost_open_bal from fixed_asset_schedule_ByBranch) fixed_asset_schedule_ByBranch "
   	     		...
   	     		+ ") ) as pivottable "	   
	     		+ "UNION   "
	     		+ "select '5.2' AS SERIAL,'CLOSE ASSET' AS NARRATION,* from (select class_name,cost_CloseAsset from fixed_asset_schedule_ByBranch) fixed_asset_schedule "
	     		+ "pivot(sum(cost_CloseAsset) for class_name "
	     		+ "in ([BUILDING],[LAND],[LEASEHOLD IMPROVEMENT],[FURNITURE FITTINGS & EQUIPMENT],[COMPUTER EQUIPMENT],[MOTOR VEHICLES],[AIRCRAFT],"
	     		+ "[COMPUTER SOFTWARE] "
	     		+ ") ) as pivottable "		
   	     		...
   	     		+ ") ) as pivottable "
	     		+ "UNION "
	     		+ "select '8.1' AS SERIAL,'CLOSE ASSET CHARGE ' AS NARRATION, * from (select class_name,dep_CloseAsset from fixed_asset_schedule_ByBranch) fixed_asset_schedule "
	     		+ "pivot(sum(dep_CloseAsset) for class_name "
	     		+ "in ([BUILDING],[LAND],[LEASEHOLD IMPROVEMENT],[FURNITURE FITTINGS & EQUIPMENT],[COMPUTER EQUIPMENT],[MOTOR VEHICLES],[AIRCRAFT],"
	     		+ "[COMPUTER SOFTWARE] "
	     		+ ") ) as pivottable "	
...
if(BankingApp.equalsIgnoreCase("FINACLE")) {     
    if(FromDate.equals("")  && ToDate.equals("")){
	     ColQuery =""
	     		...
	     		+ ") ) as pivottable "
	     		+ "UNION   "
	     		+ "select '5.2' AS SERIAL,'TRANSFER TO' AS NARRATION,* from (select class_name,cost_CloseAsset from fixed_asset_schedule) fixed_asset_schedule "
	     		+ "pivot(sum(cost_CloseAsset) for class_name "
	     		+ "in ([LAND & LEASEHOLD],[WORK IN PROGRESS],[MOTOR VEHCILE],[MACHINE & EQUIPMENT],[COMPUTER],[FURNITURE & FITTINGS],[INTANGIBLE ASSETS]"
	     		+ ") ) as pivottable 	     		"
	     		+ "UNION   "
