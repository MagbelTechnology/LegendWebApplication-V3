 ...
 */
public class FacWorkOrderImageRepostServlet extends HttpServlet {
	 ...
	 */
	public FacWorkOrderImageRepostServlet() {
		super();
...
//http://localhost:8080/legend3.net/DocumentHelp.jsp?np=uploadRequisitionImage&pgNo=1&image=N&ReqnID=REQN/5&CompCode=
	...
	private void processRequisitionImage(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException
	{
		...
		if(!isRequisitionImageExisting(workOrderCode, pName, compCode))
        {
			 ...
			 if ((pgNum==2)&&(output==true))
	            {
                              urlLink = "window.location='DocumentHelp.jsp?np=facilityMgtWorkCompletionFormUpdateRepost&workOrderCode="+workOrderCode
                                                +"&pgNo=2&image=Y&tranId="+tranId+"&assetId="+assetId+"&CompCode="+compCode+"'";
	            	//urlLink = "window.location='DocumentHelp.jsp?np=facilityRequisitionFormRepostUpdate&image=y&" +
	            			//"ReqnID="+reqnId+"&CompCode="+compCode+"'";
	            	//System.out.println("[processRequisitionImage ] urlLink ::::::: " + urlLink);
	            }
			 else if ((pgNum==2)&&(output==false))
	            {
                              exitLink = "window.location='DocumentHelp.jsp?np=facilityUploadWorkOrderRepostImage&workOrderCode="+workOrderCode
                                                +"&pgNo=2&image=N&tranId="+tranId+"&assetId="+assetId+"&CompCode="+compCode+"'";
				 	//exitLink = "window.location='DocumentHelp.jsp?np=facilityUploadRequisitionRepostImage&pgNo=2&image=N&" +
     					//	"ReqnID="+reqnId+"&CompCode="+compCode+"'";
		...
		try {
			for (Part part : request.getParts()) {
			   ...
			   File fullfile = new File(part.getName());
				...
				System.out.println("Error occured in FacRequisitionImageRepostServlet: captureData() method"	+ e);
			}
