...
//   	 System.out.println("======>>>>>>>Branch Selected: ");
	     ColQuery ="SELECT "
	     		+ "AM_GB_BATCH_POSTING.BATCH_ID AS GROUP_ID,"
	     		+ "AM_GB_BATCH_POSTING.BATCH_NO AS BATCH_NO,"
	     		+ "AM_GB_BATCH_POSTING.ASSET_ID AS ASSET_ID,"
	     		+ "AM_GB_BATCH_POSTING.SBU_CODE AS SBU_CODE,"
	     		+ "AM_GB_BATCH_POSTING.DRACCOUNT_NO AS DRACCOUNT_NO,"
	     		+ "AM_GB_BATCH_POSTING.CRACCOUNT_NO AS CRACCOUNT_NO,"
	     		+ "AM_GB_BATCH_POSTING.DR_DESCRIPTION AS DR_DESCRIPTION,"
	     		+ "AM_GB_BATCH_POSTING.CR_DESCRIPTION AS CR_DESCRIPTION,"
	     		+ "AM_GB_BATCH_POSTING.AMOUNT AS COST_PRICE,"
	     		+ "AM_GB_BATCH_POSTING.User_ID AS User_ID,"
	     		+ "AM_GB_BATCH_POSTING.DRACCOUNT_NO AS ACCOUNT_NO,"
	     		+ "AM_GB_BATCH_POSTING.TRANSTYPE AS TRANSTYPE,"
	     		+ "AM_GB_BATCH_POSTING.POSTING_DATE AS POSTING_DATE,"
	     		+ "LEGACY_TRANSACTION.AMOUNT AS AMOUNT,"
	     		+ "AM_GB_BATCH_POSTING.AMOUNT - coalesce(LEGACY_TRANSACTION.AMOUNT,0) AS COST_DIFFERENCE "
	     		+ "FROM "
	     		+ "dbo.LEGACY_TRANSACTION LEGACY_TRANSACTION RIGHT OUTER JOIN dbo.AM_GB_BATCH_POSTING AM_GB_BATCH_POSTING ON LEGACY_TRANSACTION.BATCH_ID = AM_GB_BATCH_POSTING.BATCH_ID "
	     		+ "AND LEGACY_TRANSACTION.POSTING_DATE = AM_GB_BATCH_POSTING.POSTING_DATE "
	     		//+ "WHERE "
	     		+ " AND LEGACY_TRANSACTION.POSTING_DATE BETWEEN '"+startDate+"' AND '"+endDate+"' "
	     		+ "ORDER BY "
	     		+ "AM_GB_BATCH_POSTING.BATCH_ID ASC,"
	     		+ "AM_GB_BATCH_POSTING.SBU_CODE ASC ";
	}      
	 if(branch_Id.equals("***") && !start_Date.equals("") && !end_Date.equals("")){	   
//	   System.out.println("======>>>>>>>Category Selected: ");
	     ColQuery ="SELECT "
...
//     System.out.println("<<<<<<list.size(): "+list.size());
	 for(int k=0;k<list.size();k++)
     {
      ...
      if(BankingApp.equalsIgnoreCase("FINACLE")) {          
          ...
          rowhead.createCell((short) 2).setCellValue("DR ACCOUNT NO");
          rowhead.createCell((short) 3).setCellValue("CR ACCOUNT NO");
          rowhead.createCell((short) 4).setCellValue("DR NARRATION");
          rowhead.createCell((short) 5).setCellValue("CR NARRATION");
          rowhead.createCell((short) 6).setCellValue("TRANSACTION TYPE");
          rowhead.createCell((short) 7).setCellValue("AMOUNT");
          rowhead.createCell((short) 8).setCellValue("COST DIFFERENCE (LEGEND COST - LEGACY COST)");
          rowhead.createCell((short) 9).setCellValue("USER ID");
 	 ...
 	 for(int k=0;k<list.size();k++)
      {
     	 com.magbel.legend.vao.newAssetTransaction  newTransaction = (com.magbel.legend.vao.newAssetTransaction)list.get(k);    	 
 			...
 			String groupId = newTransaction.getOldassetId();
 			String drDescription = newTransaction.getDebitAccountName();
 			String draccountNo = newTransaction.getDebitAccount();
 			String crDescription = newTransaction.getCreditAccountName();
 			String craccountNo = newTransaction.getCreditAccount();
 			String batchNo = newTransaction.getAssetUser();
 			String accountNo = newTransaction.getVendorAC();
 			double amount = newTransaction.getCostPrice();
 			double cost_Difference = newTransaction.getAccumDifference();
 			String tranType = newTransaction.getTranType();
 			branchCode = newTransaction.getBranchCode();
 			String postingDate = newTransaction.getPostingDate();
 			String blankField = "";
 			Row row = sheet.createRow((int) i);
             ...
             row.createCell((short) 2).setCellValue(draccountNo);
             row.createCell((short) 3).setCellValue(craccountNo);
             row.createCell((short) 4).setCellValue(drDescription);
             row.createCell((short) 5).setCellValue(crDescription);
             row.createCell((short) 6).setCellValue(tranType);
             row.createCell((short) 7).setCellValue(amount);
             row.createCell((short) 8).setCellValue(cost_Difference);
