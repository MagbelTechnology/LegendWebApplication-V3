...
public class AssetProofExcelExport extends HttpServlet
{
...
//     else{ColQuery ="SELECT *FROM am_Asset_Proof WHERE PROCESS_STATUS = 'WFA' AND BATCH_ID = '"+prooftranId+"'";}
     String prooftranId = records.getCodeName("SELECT TOP 1 BATCH_ID FROM am_Asset_Proof WHERE PROCESS_STATUS IS NULL AND GROUP_ID = '"+groupId+"' ORDER BY ID DESC");
     String tableName = "Temp"+prooftranId;
     System.out.println("======>>>>>>tableName in Excel Download: "+tableName);
     ...
     System.out.println("======>>>>>>list.size()in Excel Download: "+list.size());
     if(list.size()!=0){
...
//           s.addCell(new Label(18, i, totalnbv);
 ...
 //    String q1 = "IF EXISTS(select * from "+tableName+" where ASSET_ID is not null) drop table "+tableName+" ";
     ...
     System.out.println("<<<<<<<<<<<<<Check in Downlad>>>>>>>>>>>>");
     w.write();
     response.getOutputStream().flush();
     response.getOutputStream().close();
     w.close();
...
//     PrintWriter out = response.getWriter();
//     out.print("<script>window.location='DocumentHelp.jsp?np=AssetProofByBranchExist'</script>");
/*else{
