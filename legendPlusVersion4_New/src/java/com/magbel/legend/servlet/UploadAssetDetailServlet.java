...
public class UploadAssetDetailServlet extends HttpServlet
{
     ...
     
   {
	   try {
		   ...
		   if(!type.equals("VIEW")) {groupid = groupid+usid;}
		   String catRate = approvalRec.getCodeName("select dep_rate from AM_GROUP_ASSET where CONVERT(VARCHAR,Group_id) = '"+groupid+"'");
		   ...
		   String total = approvalRec.getCodeName("SELECT SUM(COST_PRICE) FROM AM_GROUP_ASSET WHERE CONVERT(VARCHAR,GROUP_ID)='"+groupid+"' AND USER_ID="+userid+" AND PROCESS_FLAG='N'");
