...
import magma.net.manager.UncapitalizedExcelUploadManager;

@WebServlet(
   name = "processUncapitalizedAssetFile"
)
@MultipartConfig(
...
public class processUncapitalizedAssetFile extends HttpServlet {
   private static final long serialVersionUID = 1L;

   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      System.out.println("Calling Process Uncapitalized Asset Upload File");
  	String payFile = "";
  	String dname = "";
  	int quantity = 1;
	...
	try {
  	...
  	Part part = request.getPart("file");
         ...
         //    File savedFile = new File(home + "\\Downloads",part.getSubmittedFileName());
             System.out.println("Trimed file to be saved to the disk is "+savedFile+"   ======>groupid: "+groupid);
             UncapitalizedExcelUploadManager excelManager = new UncapitalizedExcelUploadManager(uid,groupAssetByAsset,groupid);
  		//it.write(savedFile);
        part.write(savedFile.toString());
  		...
  		if(!list.isEmpty())
  		{
  			...
  			out.print("<script>location.replace('DocumentHelp.jsp?np=uploadUncapitalizedAsset&gid="+groupid+"');</script>");
  			...
  			session.setAttribute("PATH",payFile);
  				...
  				out.println("<script type=\"text/javascript\">");
				   ...
				   out.println("location='DocumentHelp.jsp?np=groupExcelUncapUpload';");
				   out.println("</script>");
  			}
  	/*
  	out.print("<script>");
