...
public class FacRequisitionRepostServlet extends HttpServlet {
    ...
    public FacRequisitionRepostServlet() {
        super();
    }
    ...
    private void processRequisition(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      //-- int workId= (Integer)request.getSession().getAttribute("facilityMgtWorkId");
       ...
       //-- System.out.println("the workId for this request is >>>>>>>> "+workId);
        ...
        //--String reqnID = "REQN/" + applHelper.getGeneratedId("FM_Requisition");
        ...
        String am_ReqnDelQry="Delete   from  FM_Requisition where ReqnID='"+reqnID+"'";

	String am_asset_ApprovDelQry="Delete   from  am_asset_APPROVAL where asset_id='"+reqnID+"'";

        String am_asset_MaintanceDueDelQry="Delete   from  FM_MAITENANCE_DUE where ReqnID='"+reqnID+"'";
	//--String image_qry="select ID from am_ad_Requisition_Image where ReqnID='"+reqnID+"'";


                genList.updateTable(am_ReqnDelQry);
		genList.updateTable(am_asset_ApprovDelQry);
                genList.updateTable(am_asset_MaintanceDueDelQry);
        ...
        try {
           ...
           // pstmt.setString(17, itemQty);
            ...
            if ((!result) && (status.equalsIgnoreCase("P"))) {
                ...
                out.print("<script>alert('Facility Management Requisition Repost successfully sent to " + supervisorName + "  for Approval.')</script>");
                out.print("<script>window.location='DocumentHelp.jsp?np=facilityRequisitionFormRepostUpdate&" +
                ...
                out.print("<script>alert('Facility Management Requisition Repost successfully sent to Facility Management Dept  for Approval.')</script>");
                out.print("<script>window.location='DocumentHelp.jsp?np=facilityRequisitionFormRepostUpdate&" +
                        "ReqnID=" + reqnID + "&CompCode=" + compCode + "&image=N'</script>");
            } else {
