...
public class FinacleUncapAssetUploadExport extends HttpServlet
{
...
//        System.out.println("<<<<<<branchId: "+branchId+"    Group Id: "+groupid+"    userId: "+userId);
        String currency = records.getCodeName("SELECT iso_code FROM AM_GB_CURRENCY_CODE WHERE local_currency = 'Y'"); 
        if(branchId==null){branchId = "0";}
        if(branchId.equals("0")){branchId = records.getCodeName("select BRANCH_ID from GROUP_UNCAPITALIZED_UPLOAD where GROUP_ID = '"+groupid+"' ");}
        String branchCode = records.getCodeName("select BRANCH_CODE from am_ad_branch where BRANCH_ID = "+branchId+" ");
//        String subjectTovat = records.getCodeName("select distinct Subject_TO_Vat from AM_ASSET_UNCAPITALIZED where GROUP_ID = '"+groupid+"'");
//        String subjectTowhTax = records.getCodeName("select distinct Wh_Tax from AM_ASSET_UNCAPITALIZED where GROUP_ID = '"+groupid+"'");
        ...
        String supervisorNameQry = records.getCodeName("select distinct Subject_TO_Vat+':'+substring(Wh_Tax,1,1)+':'+coalesce(TIN,'N')+':'+coalesce(RCNo,'N') from AM_ASSET_UNCAPITALIZED a, am_ad_vendor v where v.account_number like '%"+accountNumber+"%' and GROUP_ID = '"+groupid+"' and v.Vendor_Status = 'ACTIVE'");
...
//        System.out.println("<<<<<<branch Id: "+branchId+"   branch_Code: "+branchCode+"  subjectTovat: "+subjectTovat+"  subjectTowhTax: "+subjectTowhTax+"  TIN: "+TIN+"    RCNo: "+RCNo);
        ...
        String fileName = branchCode+"By"+userName+"FinacleUncapitalizedAssetUploadExport.xls";
...
//            System.out.println("======>>>>>>>subjectTovat: "+subjectTovat+"   subjectTowhTax: "+subjectTowhTax);
            if(subjectTovat.equals("Y")  && subjectTowhTax.equals("S")){
            	...
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGUNCAUPLOAD' AND type = 'VATYWHTS'");
            }
            if(!subjectTovat.equals("Y")  && subjectTowhTax.equals("S")){
            	System.out.println("===!subjectTovat.equals('Y')  && subjectTowhTax.equals('S')");
//            	System.out.println("======>>>>>>>subjectTovat: "+subjectTovat+"   subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGUNCAUPLOAD' AND type = 'WHTS'");
            }
            if(subjectTovat.equals("Y")  && !subjectTowhTax.equals("S")){
            	System.out.println("===subjectTovat.equals('Y')  && !subjectTowhTax.equals('S')");
//            	 System.out.println("======>>>>>>>subjectTovat: "+subjectTovat+"   subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGUNCAUPLOAD' AND type = 'VATY'");
            }
            if(subjectTovat.equals("Y")  && subjectTowhTax.equals("F")){
            	System.out.println("===subjectTovat.equals('Y')  && subjectTowhTax.equals('F')");
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGUNCAUPLOAD' AND type = 'VATYWHTF'");
            }
/*            if(subjectTovat.equals("Y")  && !subjectTowhTax.equals("S")){
    //        	 System.out.println("======>>>>>>>subjectTovat: "+subjectTovat+"   subjectTowhTax: "+subjectTowhTax);
                ColQuery ="SELECT DISTINCT (select c.iso_code +(select accronym from am_ad_ledger_type where series = substring(a.Asset_Ledger,1,1))+"+
                		...
                		"where a.currency_id = c.currency_id and a.category_code = AM_ASSET_UNCAPITALIZED.CATEGORY_CODE "+
                		"and d.branch_code = AM_ASSET_UNCAPITALIZED.BRANCH_CODE) AS DR_ACCT,	'D' AS DR_CR, AM_ASSET_UNCAPITALIZED.Cost_Price AS COST_PRICE, "+
                		"(coalesce(AM_ASSET_UNCAPITALIZED.INTEGRIFY,'')+ '*'+AM_ASSET_UNCAPITALIZED.Description) AS Description, '' AS E, '' AS F, "+
                		"AM_ASSET_UNCAPITALIZED.Asset_id AS ASSET_ID, '' AS H, '' AS I, AM_ASSET_UNCAPITALIZED.SBU_CODE AS SBU_CODE "+
                		"FROM "+
                		"am_ad_branch am_ad_branch INNER JOIN AM_ASSET_UNCAPITALIZED AM_ASSET_UNCAPITALIZED ON am_ad_branch.BRANCH_CODE = AM_ASSET_UNCAPITALIZED.BRANCH_CODE "+
                		"INNER JOIN am_ad_category am_ad_category ON AM_ASSET_UNCAPITALIZED.CATEGORY_CODE = am_ad_category.category_code "+
                		"INNER JOIN am_ad_department am_ad_department ON AM_ASSET_UNCAPITALIZED.Dept_code = am_ad_department.Dept_code "+
                		"INNER JOIN am_asset_approval am_asset_approval ON CAST(AM_ASSET_UNCAPITALIZED.GROUP_ID AS VARCHAR(50)) = am_asset_approval.batch_id "+
                		"WHERE AM_ASSET_UNCAPITALIZED.Asset_Status = 'ACTIVE' AND AM_ASSET_UNCAPITALIZED.GROUP_ID = ? "+
                		"UNION "+
                		"SELECT Vendor_AC AS DR_ACCT,'C' AS DR_CR,SUM(COST_PRICE) AS COST_PRICE, (coalesce(INTEGRIFY,'')+'*'+Description) AS Description,'' AS E,'' AS F,'' AS ASSET_ID,'' AS H,'' AS I, "+
                		"SBU_CODE FROM AM_ASSET_UNCAPITALIZED WHERE GROUP_ID = ? GROUP BY Vendor_AC,DESCRIPTION,INTEGRIFY,SBU_CODE "+
                		"UNION "+
                		"SELECT ( select c.iso_code +(select accronym from am_ad_ledger_type where series = substring(b.Fed_Wht_Account,1,1))+ "+
                		...
                		"'C' AS DR_CR,SUM(WH_TAX_AMOUNT) AS COST_PRICE, (coalesce(s.INTEGRIFY,'')+ '*'+VENDOR_NAME) AS Description,'' AS E,'' AS F,'' AS ASSET_ID,'' AS H,'' AS I, "+
                		"SBU_CODE FROM AM_ASSET_UNCAPITALIZED s, am_ad_vendor v WHERE GROUP_ID = ? "+
                		"AND S.Vendor_AC = v.ACCOUNT_NUMBER "+
                		"GROUP BY Vendor_AC,VENDOR_NAME,INTEGRIFY,SBU_CODE,CATEGORY_CODE,BRANCH_CODE "+
                		"UNION "+
                		"SELECT ( select c.iso_code +(select accronym from am_ad_ledger_type where series = substring(b.VAT_ACCOUNT,1,1))+ "+
                		...
                		"SBU_CODE FROM AM_ASSET_UNCAPITALIZED s, am_ad_vendor v WHERE GROUP_ID = ? AND S.Vendor_AC = v.ACCOUNT_NUMBER "+
                		"GROUP BY Vendor_AC,VENDOR_NAME,INTEGRIFY,SBU_CODE,CATEGORY_CODE,BRANCH_CODE";
            }*/
//            System.out.println("======>>>>>>>ColQuery=======: "+ColQuery);
          //  ColQuery = ColQuery+" AND ASSET_STATUS = 'ACTIVE' ";
    //        System.out.println("======>>>>>>>ColQuery: "+ColQuery);
            java.util.ArrayList list =rep.getFinacleUncapAssetUploadExportRecords(ColQuery,branchCode,groupid);
            if(list.size()!=0){
