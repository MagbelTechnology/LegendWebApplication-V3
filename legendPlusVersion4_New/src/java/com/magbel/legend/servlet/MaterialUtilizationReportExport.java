...
public class MaterialUtilizationReportExport extends HttpServlet
{
...
//	System.out.println("======>Tmm: "+Tmm);
 ...
 //   System.out.println("======>ToDate: "+ToDate);
   ...
   // String asset_Id = request.getParameter("assetId");
    String report = "rptMenuMaterialUtil";
...
//    String userName = request.getParameter("userName");
    String fileName = "";
    if(report.equalsIgnoreCase("rptMenuMaterialUtil")){fileName = branchCode+"By"+userName+"MaterialUtilizationReportExport.xlsx";}
...
//   System.out.println("<<<<<<branch_Id: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  branchCode: "+branchCode);
     String ColQuery = "";
    	     ColQuery ="SELECT a.ASSET_ID, a.BAR_CODE, a.DESCRIPTION,b.QUANTITY,b.UNIT_COST,b.USED_COST,b.USED_BY,b.TRANS_DATE,b.USER_ID,\n"
    	     		+ "d.BRANCH_CODE,c.category_code,c.category_name,d.BRANCH_NAME,e.Full_Name,c.category_ID,\n"
    	     		+ "d.BRANCH_ID FROM ST_STOCK a, ST_MATERIAL_TRANSACTION b, am_ad_category c, am_ad_branch d, am_gb_User e\n"
    	     		+ "WHERE a.BAR_CODE = b.RFID_TAG\n"
...
//   System.out.println("======>>>>>>>ColQuery: "+ColQuery);
     java.util.ArrayList list =rep.getMaterialUtilRecords(ColQuery,branch_Id,categoryCode,FromDate,ToDate,user);
     System.out.println("======>>>>>>>list size: "+list.size()+"        =====report: "+report);
     if(list.size()!=0){
   	 if(report.equalsIgnoreCase("rptMenuMaterialUtil")){
         ...
         rowhead.createCell( 2).setCellValue("BAR_CODE");
         rowhead.createCell( 3).setCellValue("DESCRIPTION");
         rowhead.createCell( 4).setCellValue("QUANTITY");
         rowhead.createCell( 5).setCellValue("UNIT_COST");
         rowhead.createCell( 6).setCellValue("USED_COST");
         rowhead.createCell( 7).setCellValue("USED_BY");
         rowhead.createCell( 8).setCellValue("TRANS_DATE");
         rowhead.createCell( 9).setCellValue("USER_ID");
         rowhead.createCell( 10).setCellValue("BRANCH_CODE");
		 rowhead.createCell( 11).setCellValue("category_code");
		 rowhead.createCell( 12).setCellValue("category_name");
		 rowhead.createCell( 13).setCellValue("BRANCH_NAME");
		 rowhead.createCell( 14).setCellValue("Full_Name");
		 rowhead.createCell( 15).setCellValue("category_ID");
		 rowhead.createCell( 16).setCellValue("BRANCH_ID");
	 ...
	 for(int k=0;k<list.size();k++)
     {
    	 com.magbel.legend.vao.newAssetTransaction  newTransaction = (com.magbel.legend.vao.newAssetTransaction)list.get(k);    	 
			...
			String quantity = newTransaction.getMultiple();
			double unitCost = newTransaction.getAmount();
			double usedCost = newTransaction.getCostPrice();
			String usedBy = newTransaction.getAssetUser();
			String transDate = newTransaction.getDatepurchased();
			String userID = newTransaction.getUserID();
			...
			String full_Name = newTransaction.getSupervisor();
			String category_ID = newTransaction.getOldCategoryCode();
            ...
            row.createCell((short) 5).setCellValue(unitCost);
            row.createCell((short) 6).setCellValue(usedCost);
            row.createCell((short) 7).setCellValue(usedBy);
            row.createCell((short) 8).setCellValue(transDate);
            row.createCell((short) 9).setCellValue(userID);
            row.createCell((short) 10).setCellValue(branch_Code);
            row.createCell((short) 11).setCellValue(category_Code);
            row.createCell((short) 12).setCellValue(category_Name);
			row.createCell((short) 13).setCellValue(branch_Name);
			row.createCell((short) 14).setCellValue(full_Name);
			row.createCell((short) 15).setCellValue(category_ID);
			row.createCell((short) 16).setCellValue(branch_ID);
