...
//    if(exists==false){   	
   ...
   if(!branch_Id.equals("0")  && !departmentCode.equals("0") && !FromDate.equals("") && !ToDate.equals("")){
     ...
     if(branch_Id.equals("0")  && departmentCode.equals("0") && FromDate.equals("") && ToDate.equals("")){
  	   System.out.println("======>>>>>>>No Selection: "+branch_Id+"   departmentCode: "+departmentCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate);
  	     ColQuery ="select LEVEL,CLASIFICATION,BRANCH_CODE,BRANCH_NAME,DEPT_CODE,DEPT_NAME, SUM(ISNULL([BUILDING], 0 )) AS [BUILDING],SUM(ISNULL([COMPUTER], 0 )) AS [COMPUTER], "
  	     		+ "SUM(ISNULL([MACHINE AND EQUIPMENT], 0 )) AS [MACHINE AND EQUIPMENT], SUM(ISNULL([FURNITURE & FITTINGS], 0 )) AS [FURNITURE & FITTINGS], "
  	     		+ "SUM(ISNULL([STAFF FUNITURE & FITTINGS], 0 )) AS [STAFF FUNITURE & FITTINGS], SUM(ISNULL([LEASEHOLD LAND], 0 )) AS [LEASEHOLD LAND],"
  	     		+ "SUM(ISNULL([LEASEHOLD IMPROVEMENT 1], 0 )) AS [LEASEHOLD IMPROVEMENT 1],SUM(ISNULL([LEASEHOLD IMPROVEMENT 2], 0 )) AS [LEASEHOLD IMPROVEMENT 2],"
  	     		+ "SUM(ISNULL([MOTOR VEHICLE], 0 )) AS [MOTOR VEHICLE],SUM(ISNULL([INTANGIBLE ASSETS - SOFTWARE], 0 )) AS [INTANGIBLE ASSETS - SOFTWARE],"
  	     		+ "SUM(ISNULL([LEASEHOLD IMPROVEMENT 3], 0 )) AS [LEASEHOLD IMPROVEMENT 3],SUM(ISNULL([LEASEHOLD IMPROVEMENT 4], 0 )) AS [LEASEHOLD IMPROVEMENT 4], "
  	     		+ "SUM(ISNULL([LEASEHOLD IMPROVEMENT 5], 0 )) AS [LEASEHOLD IMPROVEMENT 5],SUM(ISNULL([STAFF MACHINE & EQUIPMENTS], 0 )) AS [STAFF MACHINE & EQUIPMENTS],"
  	     		+ "SUM(ISNULL([WORK-IN-PROGRESS], 0 )) AS [WORK-IN-PROGRESS] from ConsolidatedVerification WHERE LEVEL IS NOT NULL "
  	     		+ "GROUP BY DEPT_CODE,LEVEL,CLASIFICATION,BRANCH_CODE,BRANCH_NAME,DEPT_NAME";    
  		}
	 ...
	 if(!branch_Id.equals("0")  && departmentCode.equals("0") && !FromDate.equals("") && !ToDate.equals("")){	   
	   System.out.println("======>>>>>>>Branch and Date Selected: "+branch_Id+"   departmentCode: "+departmentCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate);
	     ColQuery ="select LEVEL,CLASIFICATION,BRANCH_CODE,BRANCH_NAME,DEPT_CODE,DEPT_NAME, SUM(ISNULL([BUILDING], 0 )) AS [BUILDING],SUM(ISNULL([COMPUTER], 0 )) AS [COMPUTER], SUM(ISNULL([GENERATOR - HOUSE], 0 )) AS [GENERATOR - HOUSE], SUM(ISNULL([GENERATORS], 0 )) AS [GENERATORS], SUM(ISNULL([HOUSEHOLD FURNITURE & FITTING], 0 )) AS [HOUSEHOLD FURNITURE & FITTING], "
	     		+ "SUM(ISNULL([LAND], 0 )) AS [LAND],SUM(ISNULL([LEASEHOLD IMPROVEMENT], 0 )) AS [LEASEHOLD IMPROVEMENT],SUM(ISNULL([LEASEHOLD IMPROVEMENT - HOUSE], 0 )) AS [LEASEHOLD IMPROVEMENT - HOUSE],SUM(ISNULL([MOTOR VEHICLES], 0 )) AS [MOTOR VEHICLES],SUM(ISNULL([AIRCRAFT], 0 )) AS [AIRCRAFT],SUM(ISNULL([COMPUTER SOFTWARE], 0 )) AS [COMPUTER SOFTWARE],"
	     		+ "SUM(ISNULL([OFFICE EQUIPMENT - COMPUTER], 0 )) AS [OFFICE EQUIPMENT - COMPUTER], SUM(ISNULL([OFFICE EQUIPMENT - OTHERS], 0 ))[OFFICE EQUIPMENT - OTHERS],SUM(ISNULL([OFFICE FURNITURE & FITTINGS], 0 )) AS [OFFICE FURNITURE & FITTINGS],"
	     		+ "SUM(ISNULL([WIP-BUILDING], 0 )) AS [WIP-BUILDING],SUM(ISNULL([WIP-COMPUTER SOFTWARE], 0 )) AS [WIP-COMPUTER SOFTWARE],SUM(ISNULL([WIP-GENERATORS], 0 )) AS [WIP-GENERATORS],SUM(ISNULL([WIP-GENERATORS-HOUSE], 0 )) AS [WIP-GENERATORS-HOUSE],SUM(ISNULL([WIP-HOUSEHOLD FURNITURE AND FITTINGS], 0 )) AS [WIP-HOUSEHOLD FURNITURE AND FITTINGS],"
	     		+ "SUM(ISNULL([WIP-LEASEHOLD IMPROVEMENT], 0 )) AS [WIP-LEASEHOLD IMPROVEMENT],SUM(ISNULL([WIP-LEASEHOLD IMPROVEMENT-HOUSE], 0 )) AS [WIP-LEASEHOLD IMPROVEMENT-HOUSE],SUM(ISNULL([WIP-MOTOR VEHICLES], 0 )) AS [WIP-MOTOR VEHICLES],SUM(ISNULL([WIP-OFFICE EQUIPMENT - COMPUTERS], 0 )) AS [WIP-OFFICE EQUIPMENT - COMPUTERS],"
...
//   System.out.println("======>>>>>>>ColQuery: "+ColQuery);
     java.util.ArrayList list =rep.getBranchSummaryVerificationRecords(ColQuery,branch_Id,departmentCode,FromDate,ToDate,asset_Id,ThirdPartyLabel);
//     System.out.println("======>>>>>>>list size: "+list.size()+"        =====report: "+report);
     if(list.size()!=0){
   	 if(report.equalsIgnoreCase("rptMenuBranchVerify") && ThirdPartyLabel.equals("ZENITH")){
...
//            System.out.println("we are here");
	 ...
	 System.out.println("we are here 2");
	   OutputStream stream = response.getOutputStream();
   	 ...
   	 if(report.equalsIgnoreCase("rptMenuBranchVerify") && ThirdPartyLabel.equals("INTEGRIFY")){
    	 SXSSFWorkbook workbook = new SXSSFWorkbook();
         Sheet sheet = workbook.createSheet("Demo");
         Row rowhead = sheet.createRow((int) 0);
         ...
         rowhead.createCell( 5).setCellValue("Dept Code");
         rowhead.createCell( 6).setCellValue("Dept Name");
         rowhead.createCell( 7).setCellValue("BUILDING");
         rowhead.createCell( 8).setCellValue("COMPUTER");
         rowhead.createCell( 9).setCellValue("MACHINE AND EQUIPMENT");
         rowhead.createCell( 10).setCellValue("FURNITURE & FITTINGS");
		 rowhead.createCell( 11).setCellValue("STAFF FUNITURE & FITTINGS");
		 rowhead.createCell( 12).setCellValue("LEASEHOLD LAND");
		 rowhead.createCell( 13).setCellValue("LEASEHOLD IMPROVEMENT 1");
		 rowhead.createCell( 14).setCellValue("LEASEHOLD IMPROVEMENT 2");
		 rowhead.createCell( 15).setCellValue("MOTOR VEHICLE");
		 rowhead.createCell( 16).setCellValue("INTANGIBLE ASSETS - SOFTWARE");
		 rowhead.createCell( 17).setCellValue("LEASEHOLD IMPROVEMENT 3");
		 rowhead.createCell( 18).setCellValue("LEASEHOLD IMPROVEMENT 4");
		 rowhead.createCell( 19).setCellValue("LEASEHOLD IMPROVEMENT 5");
         rowhead.createCell( 20).setCellValue("STAFF MACHINE & EQUIPMENTS");
         rowhead.createCell( 21).setCellValue("WORK-IN-PROGRESS");
	 ...
	 for(int k=0;k<list.size();k++)
     {
    	 com.magbel.legend.vao.newAssetTransaction  newassettrans = (com.magbel.legend.vao.newAssetTransaction)list.get(k);    	 
            ...
            row.createCell((short) 5).setCellValue(deptCode);
            row.createCell((short) 6).setCellValue(deptName);
            row.createCell((short) 7).setCellValue(building);
            row.createCell((short) 8).setCellValue(computer);
            row.createCell((short) 9).setCellValue(machine_and_equipment);
            row.createCell((short) 10).setCellValue(furniture_and_Fitting);
            row.createCell((short) 11).setCellValue(staff_Machine_and_Equipments);
            row.createCell((short) 12).setCellValue(leaseholdLand);
            row.createCell((short) 13).setCellValue(leaseholdImprovement1);
            row.createCell((short) 14).setCellValue(leaseholdImprovement2);
            row.createCell((short) 15).setCellValue(motorVehicle);
            row.createCell((short) 16).setCellValue(intangibleAsset);
            row.createCell((short) 17).setCellValue(leaseholdImprovement3);
            row.createCell((short) 18).setCellValue(leaseholdImprovement4);
            row.createCell((short) 19).setCellValue(leaseholdImprovement5);
            row.createCell((short) 20).setCellValue(staff_Furniture_and_Fitting);            
            row.createCell((short) 21).setCellValue(work_In_Progress);
