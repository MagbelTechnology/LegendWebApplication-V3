...
//	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
	    ...
	    try {
	        if (userClass != null && !"NULL".equalsIgnoreCase(userClass)) {
	            String sql = "SELECT BRANCH_CODE, SBU_CODE, DR_ACCT, CR_ACCT, AMOUNT, NARRATION, NARRATION2 FROM FINACLE_EXT";
	            System.out.println("The sql is : " + sql);
	            ...
	            // Decide first column label
	            String label1 = "ZENITH".equalsIgnoreCase(ThirdPartyLabel) ? "BRANCH CODE" : "SBU Code";
	            ...
	            rowhead.createCell(0).setCellValue(label1);
	            rowhead.createCell(1).setCellValue("Debit Acct");
	            rowhead.createCell(2).setCellValue("Credit Acct");
	            rowhead.createCell(3).setCellValue("Amount");
	            rowhead.createCell(4).setCellValue("Narration");
	            rowhead.createCell(5).setCellValue("Narration2");

	            // Populate rows
	            int rowCount = 1;
	            while (result.next()) {
	                String sbuCode = result.getString("SBU_CODE");
	                String branchCode = result.getString("BRANCH_CODE");
	                String debitAccount = result.getString("DR_ACCT");
	                String creditAccount = result.getString("CR_ACCT");
	                ...
	                String labelCode = "BRANCH CODE".equals(label1) ? branchCode : sbuCode;

	                HSSFRow row = sheet.createRow(rowCount++);
	                row.createCell(0).setCellValue(labelCode);
	                row.createCell(1).setCellValue(debitAccount);
	                row.createCell(2).setCellValue(creditAccount);
	                row.createCell(3).setCellValue(amount);
	                row.createCell(4).setCellValue(narration);
	                row.createCell(5).setCellValue(narration2);
	            }
	            ...
	            String fileName = "Finacle Excel Download.xls";
	            response.setContentType("application/vnd.ms-excel");
	            ...
	            System.out.println("Excel file streamed to client.");
	        } else {
	            response.sendError(HttpServletResponse.SC_FORBIDDEN, "Invalid user session.");
	        }
