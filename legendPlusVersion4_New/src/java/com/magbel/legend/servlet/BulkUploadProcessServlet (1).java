...
//				 	 if(thirdPartyLabel.equalsIgnoreCase("K2")){insertBulkUploadPostingEntriesK2(assetId, batchNo,postingDate);}
			 ...
			 if(!thirdPartyLabel.equalsIgnoreCase("K2")){insertBulkUploadPostingEntries();}	 	 
		 	 if(thirdPartyLabel.equalsIgnoreCase("K2")){insertBulkUploadPostingEntriesK2(assetId, batchNo,postingDate);}
...
String[] headers = {"Account No.", "DR CR", "Amount", "Description", "E", "F", "Asset Id", "H", "I", "SBU CODE"};
...
row.createCell(0).setCellValue(asset.getVendor_account());
...
public boolean insertBulkUploadPostingEntries() {
...
public java.util.ArrayList bulkUploadRecordListsReport(String batchId)
{
	java.util.ArrayList _list = new java.util.ArrayList();
	String finacleTransId= null;
		String query = " SELECT  Asset_id,narration,BATCHID,sbu_code,Account,STATUS,amount "
				+ "FROM am_gb_bulkPostingEntries where batchId = ? ORDER BY BATCHID ";
			...
			while (rs.next())
			   {
				...
				trans.setVendor_account(account);
