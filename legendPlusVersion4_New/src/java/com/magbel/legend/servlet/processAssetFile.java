...
@WebServlet(
   name = "ProcessAssetFile"
)
@MultipartConfig(fileSizeThreshold = 1024 * 1024,
maxFileSize = 1024 * 1024 * 5, 
maxRequestSize = 1024 * 1024 * 5 * 5)
public class processAssetFile extends HttpServlet {
	...
	try {
  	...
  	Part part = request.getPart("file");
           ...
           //File savedFile = new File(home + "\\Downloads",part.getSubmittedFileName());
             ...
             System.out.println("=====groupid in processAssetFile: "+groupid);
          		AssetExcelUploadManager excelManager = new AssetExcelUploadManager(uid,groupAssetByAsset,groupid);
  		//it.write(savedFile);
      part.write(savedFile.toString());
  		...
  		if(!list.isEmpty())
  		{
  			//out.print("<script>window.location('DocumentHelp.jsp?np=RejectedAssets&gid="+groupid+"');</script>");
  			out.print("<script>location.replace('DocumentHelp.jsp?np=RejectedAssets&gid="+groupid+"');</script>");
  		}
  			//out.print("<script>window.location('DocumentHelp.jsp?np=uploadAssetDetail&gid="+groupid+"');</script>");
  		out.print("<script>location.replace('DocumentHelp.jsp?np=uploadAssetDetail&gid="+groupid+"');</script>");
