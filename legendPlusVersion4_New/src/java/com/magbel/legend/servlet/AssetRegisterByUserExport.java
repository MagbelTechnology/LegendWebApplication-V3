...
public class AssetRegisterByUserExport extends HttpServlet
{
...
//    String userName = request.getParameter("userName");
    String fileName = branchCode+"By"+userName+"AssetRegisterByUserReport.xlsx";    	
...
//    if(exists==false){   	
   ...
   System.out.println("<<<<<<branch_Id: "+branch_Id+"    categoryCode: "+categoryCode+"  branchCode: "+branchCode);
     ...
     if(!branch_Id.equals("0")  && !categoryCode.equals("0") && FromDate.equals("")  && ToDate.equals("")){
	     ...
	     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
	    		 + "am_ad_category.category_name AS category_name,am_Asset.Accum_dep AS Accum_dep,am_Asset.monthly_dep AS monthly_dep,am_Asset.Cost_Price AS Cost_Price,"
	    		 + "am_Asset.NBV AS NBV,IMPROV_COST, IMPROV_ACCUMDEP, IMPROV_MONTHLYDEP, IMPROV_NBV,Cost_Price + IMPROV_COST AS Total_Cost_Price, TOTAL_NBV,am_Asset.Date_purchased AS Date_purchased,am_ad_category.Dep_rate AS Dep_rate,am_ad_category.Accum_Dep_ledger AS Accum_Dep_ledger,"
	    		 + "am_ad_category.Dep_ledger AS Dep_ledger,am_ad_category.Asset_Ledger AS Asset_Ledger,am_ad_category.gl_account AS gl_account,am_Asset.dep_end_date AS dep_end_date,"
	    		 + "am_Asset.effective_date AS effective_date,am_Asset.Posting_Date AS Posting_Date,am_ad_department.Dept_name AS Dept_name,am_gb_User.Full_Name AS Full_Name,"
	    		 + "am_Asset.Asset_Status AS Asset_Status,sbu_code,Asset_Engine_No,bar_code,Asset_Serial_No,Purchase_Reason,am_asset.BRANCH_CODE AS BRANCH_CODE, "
	    		 + "Spare_1, Spare_2, SPARE_3, SPARE_4,SPARE_5, SPARE_6, Purchase_Reason, Registration_No, Asset_Make, Asset_Model, Asset_Engine_No, Supplier_Name, Wh_Tax,"
	    		 + "Wh_Tax_Amount, Req_Depreciation, Vat, Subject_TO_Vat, Asset_Status, am_Asset.State AS State, Driver, Vendor_AC, am_Asset.Branch_ID AS Branch_ID, am_Asset.Dept_ID AS Dept_ID,"
	    		 + "am_Asset.Category_ID AS Category_ID, Section_id, Asset_Maintenance, am_Asset.Residual_Value AS Residual_Value, Authorized_By, Req_Redistribution, Useful_Life,"
	    		 + "Total_Life, location, Remaining_Life, Vatable_Cost, Who_TO_Rem, Email1, Who_To_Rem_2, Email2, am_Asset.Raise_Entry AS Raise_Entry,Asset_User,"
	    		 + "Dep_Ytd, Section, am_Asset.User_ID AS User_ID, Date_Disposed, am_Asset.PROVINCE AS PROVINCE, Multiple, WAR_START_DATE, WAR_MONTH, WAR_EXPIRY_DATE," 
	    		 + "Last_Dep_Date, am_Asset.SECTION_CODE AS SECTION_CODE, am_Asset.DEPT_CODE AS DEPT_CODE, am_Asset.CATEGORY_CODE AS CATEGORY_CODE, AMOUNT_PTD, AMOUNT_REM, PART_PAY, FULLY_PAID,"
	    		 + "GROUP_ID, SBU_CODE, LPO, supervisor, defer_pay, OLD_ASSET_ID, WHT_PERCENT, Post_reject_reason, Finacle_Posted_Date,"
	    		 + "system_ip, mac_address, asset_code, memo, memovalue, '' AS state_name, am_Asset.VENDOR_NAME, am_Asset.VENDOR_NAME,'' AS Sbu_name "
	    		 + "FROM "
	    		 ...
	    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
	    		 + "WHERE "
	    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_ad_branch.branch_id = ? am_ad_category.category_id = ? "
	    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	
	}      
	 if(branch_Id.equals("0")  && !categoryCode.equals("0") && FromDate.equals("")  && ToDate.equals("")){	   
//	   System.out.println("======>>>>>>>Category Selected: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
    		 ...
    		 + "Last_Dep_Date, am_Asset.SECTION_CODE AS SECTION_CODE, am_Asset.DEPT_CODE AS DEPT_CODE, am_Asset.CATEGORY_CODE AS CATEGORY_CODE, AMOUNT_PTD, AMOUNT_REM, PART_PAY, FULLY_PAID,"
    		 + "GROUP_ID, SBU_CODE, LPO, supervisor, defer_pay, OLD_ASSET_ID, WHT_PERCENT, Post_reject_reason, Finacle_Posted_Date,"
    		 + "system_ip, mac_address, asset_code, memo, memovalue, '' AS state_name, '' AS Section_Name, am_Asset.VENDOR_NAME,'' AS Sbu_name "
    		 + "FROM "
    		 + "dbo.am_ad_branch am_ad_branch INNER JOIN dbo.am_Asset am_Asset ON am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE "
    		 + "INNER JOIN dbo.am_ad_category am_ad_category ON am_Asset.CATEGORY_CODE = am_ad_category.category_code " 
    		 + "INNER JOIN dbo.am_ad_department am_ad_department ON am_Asset.BRANCH_CODE = am_ad_department.Dept_code " 
    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
    		 + "WHERE "
    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_ad_category.category_id = ? "
    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";     
   }
	 if(!branch_Id.equals("0")  && categoryCode.equals("0") && FromDate.equals("")  && ToDate.equals("")){	   
	   System.out.println("======>>>>>>>Branch Selected: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
	     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
	    		 ...
	    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
	    		 + "WHERE "
	    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_ad_branch.branch_id = ? "
	    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";
	}
   if(branch_Id.equals("0")  && categoryCode.equals("0") && FromDate.equals("")  && ToDate.equals("")){
     ...
     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
    		 ...
    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
    		 + "WHERE "
    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE "
    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
	}   
   if(!FromDate.equals("")  && !ToDate.equals("") && branch_Id.equals("0")  && categoryCode.equals("0")){
	System.out.println("======>>>>>>>No Selection but Date selected: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
    		 ...
    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
    		 + "WHERE "
    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_Asset.date_purchased BETWEEN ? AND ? "
    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
	} 
   if(!FromDate.equals("")  && !ToDate.equals("") && !branch_Id.equals("0")  && categoryCode.equals("0")){
	System.out.println("======>>>>>>>Branch and Date Selection: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
    		 ...
    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
    		 + "WHERE "
    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_ad_branch.branch_id = ? AND am_Asset.date_purchased BETWEEN ? AND ? "
    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
	}    
   if(!FromDate.equals("")  && !ToDate.equals("") && !branch_Id.equals("0")  && !categoryCode.equals("0")){
	System.out.println("======>>>>>>>Branch, Category and Date Selection: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
    		 ...
    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
    		 + "WHERE "
    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_ad_branch.branch_id = ? AND am_ad_category.category_id = ? AND am_Asset.date_purchased BETWEEN ? AND ? "
    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
	}    
   if(!FromDate.equals("")  && !ToDate.equals("") && branch_Id.equals("0")  && !categoryCode.equals("0")){
		System.out.println("======>>>>>>> Selection: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
	     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
	    		 + "am_ad_category.category_name AS category_name,am_Asset.Accum_dep AS Accum_dep,am_Asset.monthly_dep AS monthly_dep,am_Asset.Cost_Price AS Cost_Price,"
	    		 + "am_Asset.NBV AS NBV,IMPROV_COST, IMPROV_ACCUMDEP, IMPROV_MONTHLYDEP, IMPROV_NBV,Cost_Price + IMPROV_COST AS Total_Cost_Price, TOTAL_NBV,am_Asset.Date_purchased AS Date_purchased,am_ad_category.Dep_rate AS Dep_rate,am_ad_category.Accum_Dep_ledger AS Accum_Dep_ledger,"
	    		 + "am_ad_category.Dep_ledger AS Dep_ledger,am_ad_category.Asset_Ledger AS Asset_Ledger,am_ad_category.gl_account AS gl_account,am_Asset.dep_end_date AS dep_end_date,"
	    		 ...
	    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
	    		 + "WHERE "
	    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_ad_category.category_id = ? AND am_Asset.date_purchased BETWEEN ? AND ? "
	    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
		}
   if(!FromDate.equals("")  && !ToDate.equals("") && !user.equals("0")){
		System.out.println("======>>>>>>>User Id Selection: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
	     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
	    		 + "am_ad_category.category_name AS category_name,am_Asset.Accum_dep AS Accum_dep,am_Asset.monthly_dep AS monthly_dep,am_Asset.Cost_Price AS Cost_Price,"
	    		 + "am_Asset.NBV AS NBV,IMPROV_COST, IMPROV_ACCUMDEP, IMPROV_MONTHLYDEP, IMPROV_NBV,Cost_Price + IMPROV_COST AS Total_Cost_Price, TOTAL_NBV,am_Asset.Date_purchased AS Date_purchased,am_ad_category.Dep_rate AS Dep_rate,am_ad_category.Accum_Dep_ledger AS Accum_Dep_ledger,"
	    		 + "am_ad_category.Dep_ledger AS Dep_ledger,am_ad_category.Asset_Ledger AS Asset_Ledger,am_ad_category.gl_account AS gl_account,am_Asset.dep_end_date AS dep_end_date,"
	    		 ...
	    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
	    		 + "WHERE "
	    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_Asset.User_ID = ? AND am_Asset.date_purchased BETWEEN ? AND ? "
	    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
		}   
   if(!asset_Id.equals("")){
		System.out.println("======>>>>>>>Asset Id Selection: "+branch_Id+"   categoryCode: "+categoryCode+"    FromDate: "+FromDate+"   ToDate: "+ToDate+"  dept_Code: "+dept_Code+"   asset_Id: "+asset_Id);
	     ColQuery ="SELECT am_gb_company.company_name AS company_name, am_Asset.Asset_id AS Asset_id,am_Asset.Description AS Description,am_ad_branch.BRANCH_NAME AS BRANCH_NAME,"
	    		 ...
	    		 + "INNER JOIN dbo.am_gb_User am_gb_User ON am_Asset.User_ID = am_gb_User.User_id,dbo.am_gb_company am_gb_company "
	    		 + "WHERE "
	    		 + "am_ad_branch.BRANCH_CODE = am_Asset.BRANCH_CODE AND am_Asset.Asset_id = ? "
	    		 + "ORDER BY am_Asset.BRANCH_CODE ASC,am_Asset.CATEGORY_CODE ASC,am_Asset.User_ID ASC,am_Asset.Asset_Status ";	     
		}    
//   System.out.println("======>>>>>>>ColQuery: "+ColQuery);
     ...
     if(list.size()!=0){
		 ...
		 rowhead.createCell((short) 14).setCellValue("ASSET USER");
		 rowhead.createCell((short) 15).setCellValue("Engine No.");
		 rowhead.createCell((short) 16).setCellValue("SBU CODE");
		 rowhead.createCell((short) 17).setCellValue("QRCode/Vehicle");
		 rowhead.createCell((short) 18).setCellValue("Serial No");
		 rowhead.createCell((short) 19).setCellValue("Purchase Reason");
		 rowhead.createCell((short) 20).setCellValue("Vendor Name");
		 rowhead.createCell((short) 21).setCellValue("Asset Status");
...
String value = "16/06/2023";
           ...
           row.createCell((short) 8).setCellValue(depr_endDate);
            ...
            row.createCell((short) 14).setCellValue(assetUser);
            row.createCell((short) 15).setCellValue(engineNo);
			row.createCell((short) 16).setCellValue(sbucode);
			row.createCell((short) 17).setCellValue(barcode);
			row.createCell((short) 18).setCellValue(serialNo);
			row.createCell((short) 19).setCellValue(purchaseReason);
			row.createCell((short) 20).setCellValue(vendorName); 
			row.createCell((short) 21).setCellValue(assetStatus);
