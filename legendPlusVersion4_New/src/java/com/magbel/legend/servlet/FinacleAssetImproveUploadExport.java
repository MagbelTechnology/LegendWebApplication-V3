...
public class FinacleAssetImproveUploadExport extends HttpServlet
{
...
//            	System.out.println("<<<<<<subjectTovat 1====>: "+subjectTovat+"    subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGDISPOSALUPLOAD' AND TYPE = 'VATYWHTS'");
            }
            if(!subjectTovat.equals("Y")  && subjectTowhTax.equals("S")){
//            	System.out.println("<<<<<<subjectTovat 2====>: "+subjectTovat+"    subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGDISPOSALUPLOAD' AND TYPE = 'WHTS'");
            }
            if(subjectTovat.equals("Y")  && !subjectTowhTax.equals("S")){
...
//            	System.out.println("<<<<<<subjectTovat 3====>: "+subjectTovat+"    subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGDISPOSALUPLOAD' AND TYPE = 'VATY'");
            }
            if(!subjectTovat.equals("Y")  && !subjectTowhTax.equals("S")){
//            	System.out.println("<<<<<<subjectTovat 4====>: "+subjectTovat+"    subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGDISPOSALUPLOAD' AND TYPE = ''");
            }
...
//            	System.out.println("<<<<<<subjectTovat 5====>: "+subjectTovat+"    subjectTowhTax: "+subjectTowhTax);
            	ColQuery = records.getCodeName("select PREFIX from ACCOUNT_GLPREFIX_PARAM where THIRDPARTY = 'LEGDISPOSALUPLOAD' AND TYPE = 'WHTF'");
            }
    //        System.out.println("======>>>>>>>ColQuery=======: "+ColQuery);
          //  ColQuery = ColQuery+" AND ASSET_STATUS = 'ACTIVE' ";
    //        System.out.println("======>>>>>>>ColQuery: "+ColQuery);
            java.util.ArrayList list =rep.getFinacleAssetImprovedUploadExportRecords(ColQuery,branchCode,groupid);
            if(list.size()!=0){
