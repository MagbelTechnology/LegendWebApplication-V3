...
//            approvalManager.infoFromRejection(tranIdRepost,userId,systemIp,reject_reason);
            ...
            if (page1 != null && page1.equalsIgnoreCase("ASSET IMPROVEMENT RAISE ENTRY")) {
                  tranId = Integer.parseInt(service.getCodeName("select transaction_id from am_asset_approval a,am_asset_improvement b where b.asset_id='" +
                        id + "' and a.tran_type='Asset Improvement' and  a.transaction_id = b.Revalue_ID "));
            }
            else
            if (page1 != null && page1.equalsIgnoreCase("ASSET TRANSFER RAISE ENTRY")) {
               
                 newAssetId =  service.getCodeName("select NEW_ASSET_ID from AM_ASSETTRANSFER where asset_id ='"+id+"'");
               
                  tranId = Integer.parseInt(service.getCodeName("select max(transaction_id) from am_asset_approval a,am_asset b where b.asset_id='" +
             ...
             //stop update on am_asset continued
              /* tranId = Integer.parseInt(service.getCodeName("select max(transaction_id) from am_asset_approval a,am_asset b where b.asset_id='" +
                ...
                tranId = Integer.parseInt(service.getCodeName("select max(transaction_id) from am_asset_approval a,am_asset b where b.asset_code='" +
