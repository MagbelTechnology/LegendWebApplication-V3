...
public class UploadUncapitalizedAssetServlet extends HttpServlet
{
     ...
     
   {
	   try {
		   ...
		   magma.GroupAssetBean group = new magma.GroupAssetBean();
		...
		//   groupid = groupid+usid;
		   ...
		   //groupid = groupid+usid;
		   String catRate = approvalRec.getCodeName("select dep_rate from AM_GROUP_ASSET_UNCAPITALIZED where Group_id = ?",groupid);
		   ...
		   if(request.getParameter("btnDelete")!=null){
			   ...
			   if(asid!=null){
			   groupM.deleteUncapitalizedUploadGAsset(asid,groupid);
			   }
			   }
		   out.print("<script>");
		   out.print("alert('Records deleted successfully');");
		   out.print("window.location='DocumentHelp.jsp?np=groupExcelUncapUpload'");
		   out.print("</script>");
		   }
		   ...
		   System.out.print("btnApply=== "+btnApply);
		   String id = request.getParameter("id");
		   int numOfTransactionLevel =  ad.getNumOfTransactionLevel("26");
		   ...
		   String Code = "26";   
		   String returnPage = "uploadUncapitalizedAsset";   
		   String tranType = "Asset Creation";
		   ...
		   String total = approvalRec.getCodeName("SELECT SUM(COST_PRICE) FROM AM_GROUP_ASSET_UNCAPITALIZED WHERE GROUP_ID=? AND USER_ID="+userid+" AND PROCESS_FLAG='N'",groupid);
		   ...
		   String ProcessFlag =approvalRec.getCodeName("Select batch_id from am_asset_approval where BATCH_ID=? and user_id = ?",groupid,userid);
		   ...
		   //System.out.print("INSIDE LOOP btnApply 1 "+btnApply);
		 	...
		 	if(Forward != true){Forward = adGroup.RecordUncapitalizedProcess(groupid);}
...
//		 	System.out.print("ASSET CODE GENERATED: "+AssetCode);
		 	boolean Update = adGroup.UploadUncapitalizedRecordSelection(Code,id,AssetCode);
...
//		 		System.out.print("INSIDE LOOP 0"+Forward);
		 		...
		 		//String  last ="N";
		 			...
		 			String count_qry ="select count(*) from AM_GROUP_ASSET_UNCAPITALIZED where process_flag='N' and Group_id = "+ last_id;
...
//		 				System.out.println("Asset Id >>>>> " + groupid);
		 				...
		 				String subjectr ="Uncapitalized Asset Upload Approval";
		 				String msgText11 ="Uncapitalized Asset with GROUP ID: "+ last +" is waiting for your approval.";
		 				String otherprogram = "";
//		 				mail.sendMailSupervisor(supervisor, subjectr, msgText11,otherprogram);
		 						  
		 			out.print("<script>");
		 			out.print("alert('Group Asset Uncapitalized Has Been Sent To  "+ Supervisor_name + "  For Approval');");
//		 			out.print("window.location='DocumentHelp.jsp?np=uploadUncapitalizedAsset' ");
//		 			out.print("window.location='DocumentHelp.jsp?np=uploadAssetDetail&pageDirect=d&gid="+last_id+"&root=group&id="+newAssetID+"';");			
				 ...
				 if(!btnApply.equals(null)){   
					 System.out.print("We are here: ");
					    double Amount = Double.parseDouble(approvalRec.getCodeName("select sum(cost_price) from AM_GROUP_ASSET_UNCAPITALIZED where group_id = ?",groupid));
						//double formatAmount = formata.formatAmount(Double.toString(Amount));
						Amount = 0.00;
					  int quantity = Integer.parseInt(approvalRec.getCodeName("select count(asset_id) from AM_GROUP_ASSET_UNCAPITALIZED where group_id = ? ",groupid));  
					String branchcode = "";
					  String description = "Uncapitalized Asset Upload";
					  String Tran_Type = "Uncapitalized Asset Upload";
...
//			                        System.out.println("The new file path is >=: " + newPath);
					 ...
					 if(singleApproval.equalsIgnoreCase("Y")){  
					   String branchId =approvalRec.getCodeName("select branch from am_gb_User where user_id=?",supervisor);
					     branchcode =approvalRec.getCodeName("select BRANCH_CODE from am_ad_branch where branch_id = ?",branchId);
					...
					String Supervisor_name =approvalRec.getCodeName("select FULL_NAME from am_gb_User where user_id=? ",supervisor);
						String subjectr ="Asset Upload Approval";
						String msgText11 ="Asset with GROUP ID: "+ last +" is waiting for your approval.";
								...
								String otherparam = "UploadAssetApproval&operation=1&id=0&tranId="+approvaltransId+"&transaction_level=1&approval_level_count=0&assetCode=0";				
								 ...
								 ad.setUpdateUnccapUploadTrans(groupid,userid);						  
								out.print("<script>");
								out.print("alert('Upload Uncapitalized Asset Has Been Sent To  "+ Supervisor_name + "  For Approval');");
								out.print("window.location='DocumentHelp.jsp?np=groupExcelUncapUpload';");
								out.print("</script>");
								//out.print("window.location='DocumentHelp.jsp?np=uploadUncapitalizedAsset&pageDirect=d&gid="+groupid+"&root=group&id="+groupid+"';");
//								out.print("window.location='DocumentHelp.jsp?np=uploadImage&previousPage="+returnPage+"&assetCode="+groupid+"';");						
						}
										
							 if(singleApproval.equalsIgnoreCase("N")){
								  String tableName = "AM_GROUP_ASSET_UNCAPITALIZED";  
...
//								System.out.println("SupervisorId#$$$$$$$$$$$ "+supervisorId);
						  	group.setGroupPendingMultipleTrans(ad.setApprovalDataUploadGroup(Long.parseLong(groupid),tableName),"26",AssetCode,supervisorId,mtid);
...
//								out.print("window.location='DocumentHelp.jsp?np=uploadAssetDetail' ");	
							...
							out.print("<script>");
							out.print("alert('Upload Uncapitalized Asset with ID: "+ groupid +" Has Been Sent For Approval');");
							out.print("window.location='DocumentHelp.jsp?np=groupExcelUncapUpload';");
							//out.print("window.location='DocumentHelp.jsp?np=uploadUncapitalizedAsset&pageDirect=d&gid="+groupid+"&root=group&id="+groupid+"';");
//								out.print("window.location='DocumentHelp.jsp?np=uploadImage&previousPage="+returnPage+"&assetCode="+groupid+"';");
								out.print("</script>");			 
