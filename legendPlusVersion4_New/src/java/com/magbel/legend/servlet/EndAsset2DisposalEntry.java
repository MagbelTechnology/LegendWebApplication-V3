...
public class EndAsset2DisposalEntry extends HttpServlet
{  
...
//        System.out.println("destination ================== "+destination);
        ...
        String page1 = "ASSET2 DISPOSAL RAISE ENTRY";
 ...
 //       String isoValueReverse=records.getCodeName("select count(*) from am_Raisentry_Transaction where Trans_id='"+tranIdReverse+"' and page1 = 'ASSET DISPOSAL RAISE ENTRY' and iso != '000' "); 
        String  reversalStatus = records.getCodeName("SELECT REVERSED FROM AM_ASSET2DISPOSAL WHERE Disposal_ID = '"+tranId+"'"); 
        String branchCode=records.getCodeName("select BRANCH_CODE from am_asset2 where asset_id='"+id+"'");
        String assetNarration = records.getCodeName("select Description from am_asset2 where asset_id='"+id+"'");
        System.out.println("prcessCost: "+prcessCost+"  prcessAccum: "+prcessAccum+"   prcessDisposal: "+prcessDisposal+"    prcessProfit: "+prcessProfit);
         if((prcessDisposal.equals("000"))&&(prcessProfit.equals("000"))){
        	 isoValue = "0";
         } 
         ...
         if((prcessDisposalRev.equals("000"))&&(prcessProfitRev.equals("000"))){
        	 isoValueReverse = "0";
         }     
        try
        {
        	...
        	ad.updateAssetStatusChange("update am_asset2Disposal set email_sent ='Y',disposal_status='P',REVERSED = 'Y' where asset_id='"+id+"'"); 
        	ad.updateAssetStatusChange("update am_asset2 set asset_status='Disposed',date_disposed='"+cdate+"' where asset_id='"+id+"'"); 
        	ad.updateAssetStatusChange("update am_raisentry_post set GroupIdStatus = 'Y',entryPostFlag = 'Y' where id='"+id+"' AND page = 'ASSET2 DISPOSAL RAISE ENTRY'");       	
        	String mailaddress=records.getCodeName("select email from am_ad_branch where BRANCH_CODE='"+branchCode+"'");
           	...
           	String msgText1 = "Your Asset2 "+assetNarration+" with asset Id "+id+" has been disposed";
           	mail.sendMail(to,subject,msgText1);		
            }
            if(isoValueReverse.equalsIgnoreCase("0") && reversalStatus.equalsIgnoreCase("Y"))
            {
            ad.updateAssetStatusChange("update am_asset2Disposal set email_sent ='Y',disposal_status='R',REVERSED = 'R' where asset_id='"+id+"'"); 
           	ad.updateAssetStatusChange("update am_asset2 set asset_status='ACTIVE',date_disposed=NULL where asset_id='"+id+"'"); 
           	String mailaddress=records.getCodeName("select email from am_ad_branch where BRANCH_CODE='"+branchCode+"'");
           	String to = mailaddress;
           	String subject = "Asset2 Dissposal Reversal";
           	String msgText1 = "Your Asset2 "+assetNarration+" with asset Id "+id+" which has been previously disposed has been reversed";
           	mail.sendMail(to,subject,msgText1);		
            } 
