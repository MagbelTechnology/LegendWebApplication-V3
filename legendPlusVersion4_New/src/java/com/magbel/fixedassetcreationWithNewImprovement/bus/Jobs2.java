...
//		int statuk = 0;
  try
        {  
//	  	System.out.println("-->Iso Creation About to Start>--> ");
	     java.util.ArrayList list_iso =comp.getSqlRecords();
//	     System.out.println("-->Iso Creation size>--> "+list_iso.size());
	     java.util.ArrayList list =comp.getNewAssetSqlRecords();
//	     System.out.println("-->size in Creation Asset >--> "+list.size());
...
//			System.out.println("supervisorName >>>>>> "+supervisorName);
			...
			if(!UserName.equalsIgnoreCase("")){user_confirm = comp.findObject("select full_name from am_gb_user where user_name = '"+UserName+"' AND USER_STATUS = 'ACTIVE'");}
			if(!UserName.equalsIgnoreCase("")){UserID = comp.findObject("select USER_ID from am_gb_user where user_name = '"+UserName+"' AND USER_STATUS = 'ACTIVE'");}
...
//			System.out.println("Supervisor by Id: "+supervisor);
	...
	//		System.out.println("Supervisor by Id After: "+supervisor);
		//	System.out.println("Approved by Name: "+approvedbyName);
			...
			//	 System.out.println("WhTax:  "+WhTax+"   SubjectTOVat: "+SubjectTOVat);
				 if((!WhTax.equalsIgnoreCase("S")) && (!WhTax.equalsIgnoreCase("F"))){
				      assetstatus = "C";
				 ...
				 /* Flexible Vat 06-02-2020	
				 if(!SubjectTOVat.equalsIgnoreCase("Y")){				 
				      assetstatus = "C";
				      errorMessage = errorMessage+"Invalid Vat Parameter It Must be Y; ";
				      boolean done = comp.newassetinterface(errorMessage, integrifyId,assetstatus,assetId,assetCode);						 
				 }*/					 
		...
		//			 System.out.println("assetexist: "+integrifyIdexist);
				...
				//	 double TotalCostPrice = Double.parseDouble(comp.findObject("select sum(cost_price) from NEW_GROUP_ASSET_INTERFACE where INTEGRIFY_ID = '"+integrifyId+"' AND ASSET_TYPE = 'U'"));
				 ...
				 if(assettype.equalsIgnoreCase("C")&& errorMessage.equalsIgnoreCase("") && (noofitems!=0)){
					 double TotalCostPrice = Double.parseDouble(comp.findObject("select sum(cost_price) from NEW_GROUP_ASSET_INTERFACE where INTEGRIFY_ID = '"+integrifyId+"' AND ASSET_TYPE = 'C'"));
...
/*				 if(SubjectTOVat.equalsIgnoreCase("Y")){
				...
				// System.out.println("WhTax:  "+WhTax);
				 if((!WhTax.equalsIgnoreCase(null)) && (!WhTax.equalsIgnoreCase(""))){				
				...
				//	 System.out.println("whtaxamount:  "+whtaxamount);
				 ...
				 if((!SubjectTOVat.equalsIgnoreCase(null)) && (!SubjectTOVat.equalsIgnoreCase("")) && (SubjectTOVat.equalsIgnoreCase("Y"))){				
					 vatrate =  VatableCost*(vatvalue/100);
					 vatamount = VatableCost*vatrate;
					 CostPrice = VatableCost + (VatableCost*vatrate);
				//	 System.out.println("whtaxamount:  "+whtaxamount);
				 }					 
...
/*				 statux = comp.insertAssetRecord(integrifyId,Description,RegistrationNo,VendorAC,Datepurchased,
						...
						supervisor,posted,assetId,assetCode,assettype,branch_id,dept_id,section_id,cat_id,vatamount,residualvalue,whtaxrate,
...
/*					 int statuk = comp.insertGroupAssetRecord(integrifyId,Description,RegistrationNo,VendorAC,Datepurchased,
							AssetMake,AssetModel,AssetSerialNo,AssetEngineNo,SupplierName,AssetUser,AssetMaintenance,
							CostPrice,VatableCost,AuthorizedBy,WhTax,whtaxamount,PostingDate,EffectiveDate,PurchaseReason,SubjectTOVat,AssetStatus,
							State,Driver,UserID,branchCode,sectionCode,deptCode,categoryCode,barCode,sbuCode,lpo,invoiceNo,
							supervisor,posted,assetId,assetCode,assettype,branch_id,dept_id,section_id,cat_id,vatamount,residualvalue,whtaxrate,noofitems,
							location,memovalue,memo,spare1,spare2,multiple);*/
						 String branchName= comp.findObject(" SELECT branch_name from am_ad_branch where branch_code='"+branchCode+"'");
						 String statuk1 = comp.insertGroupAssetRecord(integrifyId,Description,RegistrationNo,VendorAC,Datepurchased,
							AssetMake,AssetModel,AssetSerialNo,AssetEngineNo,SupplierName,AssetUser,AssetMaintenance,
							CostPrice,VatableCost,AuthorizedBy,WhTax,whtaxamount,PostingDate,EffectiveDate,PurchaseReason,SubjectTOVat,AssetStatus,
							State,Driver,UserID,branchCode,sectionCode,deptCode,categoryCode,subcategoryCode,barCode,sbuCode,lpo,invoiceNo,
						 ...
						 int statuk = Integer.parseInt(statuk1);
//					 System.out.println("insertGroupAssetRecord statuk:  "+statuk);
					 	if(statuk ==0){lastMTID = comp.getCurrentMtid("am_asset_approval");
					 if(assetRaiseEntry != null && assetRaiseEntry.equalsIgnoreCase("Y")){
					    	if(assettype.equalsIgnoreCase("C")){								
				...
				//		  String Name =comp.findObject(" SELECT full_name from am_gb_user where user_id="+Integer.parseInt(supervisor)+"");
						   branchName= comp.findObject(" SELECT branch_name from am_ad_branch where branch_code='"+branchCode+"'");
						  //String description= ad.getDescription();
						  String subjectT= comp.subjectToVat(assetId);
						  String whT= comp.whTax(assetId);
					//	  System.out.println("Supervisor Name in Group: "+approvedbyName+"  User Id: "+Integer.parseInt(supervisor));
						  comp.insertApprovalx(assetId, Description, page1, flag, partPay,approvedbyName,branchName,subjectT,whT,url,lastMTID,Integer.parseInt(assetCode));
							}		
					 }
	...
	//						System.out.println("<<<<<<<< recUserName >>>>>>  "+recUserName);
			...
			/*				 if(recSubjectTOVat.equalsIgnoreCase("Y")){  wwrwrwr
	...
	//						System.out.println("<<<<<<<< supervisorName >>>>>>  "+supervisorName);
							...
							if(numOfTransactionLevel==0){recAssetStatus = "APPROVED";}
							 ...
							 if((!recWhTax.equalsIgnoreCase("S"))&&(!recWhTax.equalsIgnoreCase("F"))){
							      assetstatus = "C";
...
//						System.out.println("New asset_ID : ");
			//			System.out.println("supervisor>>>>>> "+recsupervisor+"  UserID: "+recUserID+"  whtaxamount: "+whtaxamount);
...
/*						boolean savetrue = comp.save(recintegrifyId,recDescription,recRegistrationNo,recVendorAC,recDatepurchased,
								...
								recState,recDriver,recUserID,recbranchCode,recsectionCode,recdeptCode,reccategoryCode,recbarCode,recsbuCode,reclpo,recinvoiceNo,
								recsupervisor,recposted,recassetId,recassetCode,recassettype,recbranch_id,recdept_id,recsection_id,reccat_id,recvatamount,residualvalue,recwhtaxrate,groupid,recno,systemIp,recspare1,recspare2);
						*/
						boolean savetrue = comp.save(recintegrifyId,recDescription,recRegistrationNo,recVendorAC,recDatepurchased,
								recAssetMake,recAssetModel,recAssetSerialNo,recAssetEngineNo,recSupplierName,recAssetUser,recAssetMaintenance,
								recCostPrice,recVatableCost,recAuthorizedBy,recWhTax,recwhtaxamount,recPostingDate,recEffectiveDate,recPurchaseReason,recSubjectTOVat,recAssetStatus,
				...
				//		  System.out.println("Initializing 1");
					...
					//	  System.out.println("old Cost for Initializing 4: "+oldVendorAcct);
						  ...
						  //double accumDep = 0;
						   ...
						   EffectiveDate =  asset.getEffDate();
...
//								System.out.println("About to Insert Record into Table am_asset_improvement");
/*	
								int b = comp.insertAssetMaintananceNoSupervisor(newCost,newNbv,assetId,CostPrice,PurchaseReason,EffectiveDate,
										 ...
										 supplier_nameOld,vendorAcctOld,Description,categoryCode,branchCode,lpo,invoiceNo,newCost,Integer.parseInt(assetCode),
...
/*			 else{ 
			 ...
			 //     System.out.println("Record Successfully Updated 2>>> ");
				...
				errorMessage = "";
				//End	advising management of their NBV with email notification to users
	     }catch (Throwable e) {
