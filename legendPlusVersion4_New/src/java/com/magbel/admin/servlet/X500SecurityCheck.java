...
//            Postilion admin = new Postilion();
            ...
            String Issue_Id = request.getParameter("IssueID");
            ...
            System.out.println("==Issue_Id in X500SecurityCheck=== "+Issue_Id);
            Issue_Id = (Issue_Id == null || Issue_Id.equals(""))?"":Issue_Id;
/*            
System.out.println("==workstationIp=== "+workstationIp);
System.out.println("==address=== "+address);
System.out.println("==workstationName=== "+workstationName);
System.out.println("==logonAttempt=== "+logonAttempt);
*/
                    ...
                    if (sh.login(deployer)) {
                        ...
                        if (list != null && list.size() > 0) {
                            ...
                            if (user != null && user.getUserStatus().equalsIgnoreCase("ACTIVE")) //if(user != null) //the condition for access to system denial for inactive user goes here
                            {
                                   ...
                                   // System.out.println(">>0 the login status of user is "+ user.getLoginStatus());
                                    ...
                                    if (user.getLoginStatus().equals("0")) {
                                       ...
                                       SytemsManager sm = new SytemsManager();
                                        ...
                                        session.setAttribute("stafforg", user.getorganization());
                                        ...
                                        session.setAttribute("compCode", comp.getCompanyCode());
                                         session.setAttribute("isHDManager", user.getIsHelpdesk());
                                        String loguser = "N";
                                        ...
                                        session.setAttribute("FAILED_LOGON_CTN", "0");
                                       // String issue = Issue_Id.substring(0, 3);                                       
                                        if (sh.queryPexpiry(user.getUserId())) {
                                        	...
                                        	if(Issue_Id=="" || Issue_Id==null){response.sendRedirect("DocumentHelp.jsp");}
                                         	if(Issue_Id!="" && Issue_Id!=null){  
                                         		/*System.out.println("===Issue_Id===> "+Issue_Id);
                                         		if(issue !="Inc"){ 
                                         			System.out.println("===issue===> "+issue);
                                         		out.print("<script>window.location='DocumentHelp.jsp?np=hdComplaintResponseForm&reqnId="+Issue_Id+"&reqnId='</script>");}*/
                                     		 	/*if(issue =="Inc"){    System.out.println("===issue===> "+issue); */
                                         		out.print("<script>window.location='DocumentHelp.jsp?np=hdComplaintResponseForm&reqnId="+Issue_Id+"&Status="+Status+"'</script>");} 
                                    	//	}
                                         		//response.sendRedirect("DocumentHelp.jsp?np=hdComplaintResponseForm&reqnId=Req/544");}
                                           session.setAttribute("FAILED_LOGON_CTN", "0");
                                        ...
                                        out.print("<script>alert('User already connected to Oriental Helpdesk')</script>");
                                         out.println("<script>window.location = 'systemConnect.jsp'</script>");
                                    }
                                ...
                                out.print("<script>alert('You are not authorized to use Oriental Helpdesk ')</script>");
                                 out.println("<script>window.location = 'systemConnect.jsp'</script>");
