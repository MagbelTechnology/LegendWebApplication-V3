@POST
	@Path("/getGroupCreationDetails")
	@Produces(MediaType.APPLICATION_JSON) 
	public String groupAssetCreation(List<GroupAssetRequest> request) throws Throwable {
							...
							JSONObject data = new JSONObject();
							 System.out.println(" Starting group asset creation, received " + request.size() + " assets.");

							  
							 try
						        {  
								 String groupStatux = "";
								 GroupAssetRequest summary = request.get(0);  
								 List<GroupAssetRequest> details = request.size() > 1 ? request.subList(1, request.size()) : new ArrayList<>();
							     ...
							     java.util.ArrayList list = records.getGroupAssetSqlRecordsForCapitalised(
							    		 request);
								...
								//     for(int k=0;k<=list.size();k++)
								    ...
								    newAssetTransaction  newassettrans = (newAssetTransaction)list.get(k);    	 
										...
										double cost_Price = newassettrans.getCostPrice();
...
//										 System.out.println("User Name>>>>>> "+UserName); 
										...
										// String groupitemsNo = comp.findObject("SELECT count(*) FROM NEW_GROUP_ASSET_INTERFACE WHERE POSTED = 'N' AND TRAN_TYPE = 'N' AND INTEGRIFY_ID = '"+compintegrifyId+"'");
										 String groupitemsNo = String.valueOf(summary.getNoOfItem());
									...
									//	System.out.println("budgetValue >>>>>> "+budgetValue+"  noofitems: "+noofitems);
										String budgetenfoced =comp.findObject(" SELECT enforce_acq_budget from am_gb_company ");
										 ...
										 double expectedTotalCost = summary.getCost_Price();										
										 int expectedNoOfItems = summary.getNoOfItem();
										 System.out.println("Summary expectedNoOfItems: " + expectedNoOfItems);
										  
										    double actualTotalCost = details.stream()
										                                    .mapToDouble(GroupAssetRequest::getCost_Price)
										                                    .sum();

										    System.out.println("Summary TotalCostPrice: " + expectedTotalCost);
										    System.out.println("Detail Sum CostPrice: " + actualTotalCost);
...
//											 }	
										...
										//	 System.out.println("PostingDate:  "+PostingDate);
											...
											// System.out.println("WhTax:  "+WhTax+"   SubjectTOVat: "+SubjectTOVat);
											 ...
											 System.out.println("newassettrans.getNoofitems(): "+newassettrans.getNoofitems());
											 if(newassettrans.getNoofitems()!=expectedNoOfItems){  
											      assetstatus = "C"; 
											      errorMessage = "Mismatch in NoOfItem: expected %d but found %d in payload index %d (Description: %s); ";
											      boolean done = comp.newassetinterface(errorMessage, integrifyId,assetstatus,assetId,assetCode);						 
											 }	
											...
											//	 double TotalCostPrice = Double.parseDouble(comp.findObject("select sum(cost_price) from NEW_GROUP_ASSET_INTERFACE where INTEGRIFY_ID = '"+integrifyId+"' AND ASSET_TYPE = 'U'"));
											 ...
											 if(assettype.equalsIgnoreCase("C")&& errorMessage.equalsIgnoreCase("") && (noofitems!=0)){
												...
												// System.out.println(" strCostPrice: "+strCostPrice );
												 String costPrice = String.valueOf(cost_Price);
												...
												// double TotalCostPrice = Double.parseDouble(costPrice);
												 double total_Cost_Price = Double.parseDouble(costPrice) * Double.parseDouble(String.valueOf(noofitems)); 
												// System.out.println(" total_Cost_Price: "+total_Cost_Price );
											// System.out.println("Capitalised TotalCostPrice: "+TotalCostPrice+"  treshhold: "+treshhold+"  CostPrice: "+CostPrice);
												 if(Double.compare(expectedTotalCost, actualTotalCost) != 0){
												      assetstatus = "C";
...
//													 System.out.println("groupitemsNo0: "+groupitemsNo0+"   Inside Jobs2 integrifyId: "+integrifyId);
												 ...
												 System.out.println("groupexist: "+groupexist);
												 if(groupexist == null || groupexist.isEmpty()){
													 System.out.println("summary.getNoOfItem(): "+summary.getNoOfItem());
													 System.out.println("summary.getRecordType(): "+summary.getRecordType());
													 if("S".equalsIgnoreCase(summary.getRecordType())){
...
//										    	 comp.newassetinterface(errorMessage, integrifyId,assetstatus,groupAssetId,assetCode);					 
											...
											// }
												 System.out.println("statuk for Group Asset=====: " + statuk + " groupStatux: " + groupStatux);
											 if(groupStatux !=""){
...
//												 String groupitemsNo = comp.findObject("SELECT count(*) FROM NEW_GROUP_ASSET_INTERFACE WHERE POSTED = 'N' AND TRAN_TYPE = 'N' AND INTEGRIFY_ID = '"+compintegrifyId+"'");
											    ...
											    // for(int i=0;i<details.size();i++)
													 for(GroupAssetRequest newrectrans : details)
											     {	
											    	 
											    	 int errorindex = i + 1;
											    	 System.out.println("<<<<<<<<GROUP Loop>>>>>>  "+i);
											    	// GroupAssetRequest  newrectrans = (GroupAssetRequest)details.get(i);    	 
														String recintegrifyId =  newrectrans.getINTEGRIFY_ID();
														String recDescription = newrectrans.getDescription();
														String recRegistrationNo = newrectrans.getRegistration_No();
														String recVendorAC = newrectrans.getVendor_AC();
														String recDatepurchased = newrectrans.getDate_purchased();
														String recAssetMake = newrectrans.getAsset_Make();
														String recAssetModel = newrectrans.getAsset_Model();
														String recAssetSerialNo = newrectrans.getAsset_Serial_No();
														String recAssetEngineNo = newrectrans.getAsset_Engine_No();
														String recSupplierName = newrectrans.getSupplier_Name();
														String recAssetUser = newrectrans.getAsset_User();
														String recAssetMaintenance = newrectrans.getAsset_Maintenance();
														double recVatableCost = newrectrans.getCost_Price();
														String recAuthorizedBy = newrectrans.getAuthorized_By();
														String recWhTax = newrectrans.getWh_Tax();
														String recPostingDate = newrectrans.getPosting_Date();
														String recEffectiveDate = newrectrans.getEffective_Date();
														String recPurchaseReason = newrectrans.getPurchase_Reason();
														String recSubjectTOVat = newrectrans.getSubject_TO_Vat();
														//String AssetStatus = newrectrans.getAssetStatus();
														String recState = newrectrans.getState();
														String recDriver = newrectrans.getDriver();
														String recUserName = newrectrans.getUser_ID();
														String recbranchCode = newrectrans.getBRANCH_CODE();
														String recsectionCode = newrectrans.getSECTION_CODE();
														String recdeptCode = newrectrans.getDEPT_CODE();
														String reccategoryCode = newrectrans.getCATEGORY_CODE();
														String recsubcategoryCode = newrectrans.getSUB_CATEGORY_CODE();
														String recbarCode = newrectrans.getBAR_CODE();
														String recsbuCode = newrectrans.getSBU_CODE();
														String reclpo = newrectrans.getLPO();
														String recinvoiceNo = newrectrans.getINVOICE_NO();
														String recsupervisorName = newrectrans.getSupervisor();
														String recposted = newrectrans.getPOSTED();
														String recassetId = newrectrans.getASSET_ID();
														String recassetCode = newrectrans.getASSET_CODE();
														String recerrormessage = newrectrans.getERROR_MESSAGE();
														String recassettype = newrectrans.getASSET_TYPE();
														double recwhtaxvalue = newrectrans.getWh_Tax_Value();
														String rectranType = newrectrans.getTRAN_TYPE();
														int recwhtaxrate = (int)whtaxvalue;
														int recno = newrectrans.getNoOfItem();	
														...
														String recType = newrectrans.getRecordType();
...
//													System.out.println("New asset_ID : ");
													...
													if ("D".equalsIgnoreCase(recType) && x <= expectedNoOfItems ) {
														System.out.println("<<<<<<<< x >>>>>>  "+x);
														System.out.println("Trying to call save : ");
													 savetrue = comp.save(recintegrifyId,recDescription,recRegistrationNo,recVendorAC,recDatepurchased,
										 ...
										 //     System.out.println("Record Successfully Updated 2>>> ");
											...
											//      System.out.println("Record Successfully Updated 3>>> ");
												 ...
												 if(tranType.equalsIgnoreCase("I")){
													 String integrifyIdExist = comp.getCodeName(" select count(*) from am_asset_improvement where INTEGRIFY = '"+integrifyId+"' ");
													 if(integrifyIdExist.equals("0")){
											...
											//		  System.out.println("Initializing 1");
												...
												//	  System.out.println("Initializing 2");
													  ...
													  com.magbel.model.Asset asset = (com.magbel.model.Asset)comp.getAsset(assetId);
													  oldCost = asset.getCost();
													  System.out.println("oldCost: " + oldCost);
													  oldVatAmt = asset.getVatAmt();
													  System.out.println("oldVatAmt: " + oldVatAmt);
													  oldWhtAmt = asset.getWhtAmt();
													  System.out.println("oldWhtAmt: " + oldWhtAmt);
													  ...
													  System.out.println("old Cost for Initializing 4: "+oldVendorAcct);
													   ...
													   if(fromRepost != null && fromRepost.equalsIgnoreCase("y")){
														   com.magbel.model.Improvement revalue = (com.magbel.model.Improvement)comp.getMaintenanceAsset(assetId);							   
...
//													   System.out.println("supervisor for improvement: "+supervisor);
													   String[] pa = new String[11];
														...
														//	 pa[2]=UserID;
														//	 pa[10]="A";
															 if(postImproveCount == 0){
															String mtid =  comp.setPendingTrans2(pa,"10",Integer.parseInt(assetCode));	
...
//															 	System.out.println("revalue_query:  "+revalue_query);
															 ...
															 String mtid  =  comp.setPendingTrans2(pa,"10",Integer.parseInt(assetCode));
															  System.out.println("EffectiveDate >>>>>>>>>>> " + EffectiveDate+"   mtid: "+mtid);  
