...
import com.magbel.fixedassetcreationLegendPlusZenith.bus.jobsBranchTable;
...
//  	System.out.println(" Here is Runing the Jobs6==");
 ...
 // 	System.out.println(" Here is Runing the Jobs11==");
	...
	// 	System.out.println(" Here is Runing the Jobs20==");
	JobDetail jobsBranchTable = JobBuilder.newJob(jobsBranchTable.class)
	.withIdentity(jobKeyBranchTable).build();
	
	JobKey jobKeySBUTable = new JobKey("jobsSBUTable", "group21");
	// 	System.out.println(" Here is Runing the Jobs21==");
	JobDetail jobsSBUTable = JobBuilder.newJob(jobsSBUTable.class)
	.withIdentity(jobKeySBUTable).build();
	
	JobKey jobKeySectionTable = new JobKey("jobsSectionTable", "group22");
	// 	System.out.println(" Here is Runing the Jobs22==");
	JobDetail jobsSectionTable = JobBuilder.newJob(jobsSectionTable.class)
	.withIdentity(jobKeySectionTable).build();
	...
	.newTrigger()
	.withIdentity("dummyTriggerName20", "group20")
	.withSchedule(
	CronScheduleBuilder.cronSchedule("0/5 * * * * ?"))
	.build();
	...
	.newTrigger()
	.withIdentity("dummyTriggerName21", "group21")
	.withSchedule(
	CronScheduleBuilder.cronSchedule("0/5 * * * * ?"))
	.build();

	Trigger trigger22 = TriggerBuilder
	.newTrigger()
	.withIdentity("dummyTriggerName22", "group22")
	.withSchedule(
	CronScheduleBuilder.cronSchedule("0/5 * * * * ?"))
	.build();
	...
	scheduler.scheduleJob(jobsSBUTable, trigger21);
	scheduler.scheduleJob(jobsSectionTable, trigger22);
	}
	catch(Exception c)
	{
		c.printStackTrace();
