...
public class AjaxServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest request,
        ...
        if (operation.equalsIgnoreCase("LDG2")) {
        	getLedgerNo(request, response);
        } else if (operation.equalsIgnoreCase("TKR")) {
        	getloanRate(request, response);
        } else if (operation.equalsIgnoreCase("DEPT")) {
        	...
        	if (operation.equalsIgnoreCase("AOC")) {
        	getFormFee(request, response);
        }        
        else if (operation.trim().equalsIgnoreCase("PAYNO")) {
        	getPAYNO(request, response);
        }else{}
    ...
    public void getSectionInDept(HttpServletRequest request,
        ...
        ResultSet rs2 = null;
        try {
            con = dbConnection.getConnection("ias");
    ...
    public void getLedgerNo(HttpServletRequest request,
            HttpServletResponse response) throws
ServletException, IOException {
    	String loanCode = request.getParameter("bid");
    	
    	String query ="select LEDGER_NO, LEDGER_NO from IA_PAY_LOAN_TYPE WHERE COMP_CODE + LOAN_CODE ='"+loanCode+"'";
    	Connection con = null;
    	PreparedStatement ps = null;
    	ResultSet rs = null;
    	String name = "",head = "";
    	...
    	try{
    		con = dbConnection.getConnection("ias");
    		System.out.println("==== query getLedgerNo=== "+query);
    		ps = con.prepareStatement(query);
    		rs = ps.executeQuery();
	    	if(rs.next()) {
	    		name = rs.getString(1);
    	...
    	String warning = "WARNING:Error Fetching Ledger No ->" + e.getMessage();
    	e.printStackTrace();
    	//  System.out.println(warning);
    	} finally {
    	dbConnection.closeConnection(con, ps, rs);
    ...
    public void getloanRate(HttpServletRequest request,
            HttpServletResponse response) throws
ServletException, IOException {
    	String loanCode = request.getParameter("bid");
    	
    	String query ="select TAKE_RATE, TAKE_RATE from IA_PAY_LOAN_TYPE WHERE COMP_CODE + LOAN_CODE ='"+loanCode+"'";
    	Connection con = null;
    	PreparedStatement ps = null;
    	ResultSet rs = null;
    	double name = 0.00,head = 0.00;

    	try{
    		con = dbConnection.getConnection("ias");
    		System.out.println("==== query getloanRate=== "+query);
    		ps = con.prepareStatement(query);
    		rs = ps.executeQuery();
	    	if(rs.next()) {
	    		name = rs.getDouble(1);
	    	...
	    	System.out.println("==== name getloanRate=== "+name+"   head: "+head);
	    	response.setContentType(CONTENT_TYPE);
	    	PrintWriter out = response.getWriter();
	    	out.write("<loanRate>");
			out.write("<name>"+name+"</name>");
    	...
    	String warning = "WARNING:Error Fetching Loan Take Rate ->" + e.getMessage();
    	e.printStackTrace();
    	//  System.out.println(warning);
    	} finally {
    	dbConnection.closeConnection(con, ps, rs);
...
public void getPAYNO(HttpServletRequest request,
        HttpServletResponse response) throws
ServletException, IOException {
	String loanCode = request.getParameter("bid");
	
	String query ="select DURATION, DURATION from IA_PAY_LOAN_TYPE WHERE COMP_CODE + LOAN_CODE ='"+loanCode+"'";
	Connection con = null;
	PreparedStatement ps = null;
	ResultSet rs = null;
	double name = 0.00,head = 0.00;
	...
	try{
		con = dbConnection.getConnection("ias");
		System.out.println("==== query getPAYNO=== "+query);
		ps = con.prepareStatement(query);
		rs = ps.executeQuery();
    	if(rs.next()) {
    		name = rs.getDouble(1);
