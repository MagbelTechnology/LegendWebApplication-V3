...
public class InvoiceVoucherItemServlet extends HttpServlet
{

    private InvoiceServiceBus serviceBus;

    public InvoiceVoucherItemServlet()
    {
    }
 ...
 //       System.out.println("<<<<<<<<<Ident>>>>>>>>>>> "+ident);
        try
        {
            ...
            { 
                ...
                if(serviceBus.createInvoiceVoucherItem(itemDescription, amount, quantity, projectCode, unitPrice, invoiceNo, accountNo, branchCode,companyCode,userid,chequeNo,term,customerName,orderNo))
                {
                    out.print("<script>alert('Record successfully saved.')</script>");
                    out.print("<script>window.close();</script>");
                    out.print("<script>window.document.location='DocumentHelp.jsp?np=newVoucherInvoiceDetails&id=="+id+"&invoiceNo="+invoiceNo+"'</script>");
                } else
                {  
            ...
            {  
                serviceBus.updateInvoiceVoucherItem(id, itemDescription, amount, quantity, projectCode, unitPrice,invoiceNo,pageName);
                ...
                out.print("<script>window.document.location='DocumentHelp.jsp?np=newVoucherInvoiceDetails&id=="+mid+"&invoiceNo="+invoiceNo+"'</script>");
            }
        }
