...
public class PayableImprestItemActionServlet extends HttpServlet
{
    private AccountInterfaceServiceBus serviceBus;
    public PayableImprestItemActionServlet()
    {
    }
...
//		System.out.println("Beneficiary Account: "+benaccount);
		ImprestItems imp =serviceBus.findPayableImprestAvailable(refNumber);
			...
			if((impamount == 0.00) || (!imprestAccount.equals(Accounimprest)))
		     {
				...
				out.print("<script>window.close();</script>");
							 ...
							 if(strAmount.indexOf(",") > 0)
							{
								...
								serviceBus.createPayableImprestItem(Accounimprest, refNumber, description, _amount,type,companyCode, branchNo,expglaccount,orderNo);
								out.print("<script>window.close();</script>");
							}
							else
							{
									amount =Double.parseDouble(strAmount);
									serviceBus.createPayableImprestItem(Accounimprest, refNumber, description, amount, type,companyCode, branchNo,expglaccount,orderNo);
									out.print("<script>window.close();</script>");
							}	
				}
   }       
			...
			if(strAmount.indexOf(",") > 0)
							{
								String amt=strAmount.replace(",","");
								_amount =Double.parseDouble(amt.trim()); 
								serviceBus.updatePayableImprestItem(Accounimprest,description, _amount,type,id,expglaccount);
								out.print("<script>window.close();</script>");
							}
							else
							{
									amount =Double.parseDouble(strAmount);
									serviceBus.updatePayableImprestItem(Accounimprest,description, amount,type,id,expglaccount);
									out.print("<script>window.close();</script>");
							}
            }
    }
