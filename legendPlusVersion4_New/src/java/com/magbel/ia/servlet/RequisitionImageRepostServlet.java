 ...
 */
public class RequisitionImageRepostServlet extends HttpServlet {
	 ...
	 */
	public RequisitionImageRepostServlet() {
		super();
	...
	private void processRequisitionImage(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException
	{
		...
		if(!isRequisitionImageExisting(reqnId, pName, compCode))
        {
			 ...
			 if ((pgNum==2)&&(output==true))
	            {
	            	urlLink = "window.location='DocumentHelp.jsp?np=RequisitionFormRepostUpdate&image=y&" +
	            			"ReqnID="+reqnId+"&CompCode="+compCode+"'";
	            	//System.out.println("[processRequisitionImage ] urlLink ::::::: " + urlLink);
	            }
			 else if ((pgNum==2)&&(output==false))
	            {
				 	exitLink = "window.location='DocumentHelp.jsp?np=uploadRequisitionImageRepost&pgNo=1&image=N&" +
     						"ReqnID="+reqnId+"&CompCode="+compCode+"'";
	...
	private void captureData(HttpServletRequest request)
	{
		...
		if (isMultipart) 
		{
			...
			{
				System.out.println("Error occured in RequisitionImageRepostServlet: captureData() method"	+ fue);
			} 
			catch (Exception e)
			{
				System.out.println("there is an error in RequisitionImageRepostServlet: captureData() method"	+ e);
			}
		}
