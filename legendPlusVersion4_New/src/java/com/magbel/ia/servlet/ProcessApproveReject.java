...
public class ProcessApproveReject extends HttpServlet {
    private static final String CONTENT_TYPE = "text/html; charset=windows-1252";
    private InventoryBus serviceBus;
    ApprovalRemark remrk = null;
    ...
    public void service(HttpServletRequest request,HttpServletResponse response) 
       ...
       // int selectTax=Integer.parseInt(selectTaxS);
        ...
        String workstationIp = request.getParameter("workstationIp");
     ...
     //   System.out.println("-selectTax-"+ selectTax);
        ...
        try
        {  
        	//Approved button pressed
        	if(bid.equals("Approve"))
        	{
        		//Check for approval level
        		if(numOfTransactionLevel.equals("1")) 
        			{
        			//Check if item exist in inventory total
                	 if(serviceBus.checkInventoryTotal(ItemTypeCode, WarehouseCode))
                	  {
        			      //No approval
        			      if(serviceBus.createInventory(inventory_id))
        				  //Update inventory total
            			  if(serviceBus.updateInventoryTotal(ItemTypeCode, WarehouseCode))
            				//Insert into inventory history
            				serviceBus.createInventoryHistory(ItemTypeCode, WarehouseCode, user_id);
                	  }
                	//Item does not exist in inventory total
                		else{
                    		//Insert into ia inventory
                			 if(serviceBus.createInventory(inventory_id))	//Insert into ia inventory total
                    			    if(serviceBus.createInventoryTotal(ItemTypeCode, WarehouseCode, user_id,compCode))
                    			    	//Insert into ia inventory history
                    			    	serviceBus.createInventoryHistory(ItemTypeCode, WarehouseCode, user_id);
                    		                out.print("<script>window.document.location='DocumentHelp.jsp?np=InventoryApprovalList'</script>");
                    		}
        			out.print("<script>window.document.location='DocumentHelp.jsp?np=InventoryApprovalList'</script>");
        			}
        		//Perform approval
        		else{
        			if(serviceBus.updateInventoryApprove(inventory_id, supervisor, numOfTransactionLevel))
        			remrk = new ApprovalRemark(inventory_id,user_id,posting_date,remark,bid ,numOfTransactionLevel ,"",workstationIp);
        			serviceBus.createApprovalRemark(remrk);
        			out.print("<script>window.document.location='DocumentHelp.jsp?np=InventoryApprovalList'</script>");
        		    }
        	}
        	//Reject button pressed
        	else if(bid.equals("Reject"))
        	   {
        		//Update back to approval level
        		serviceBus.updateInventoryReject(inventory_id,"5");
        		out.print("<script>window.document.location='DocumentHelp.jsp?np=InventoryApprovalList'</script>");
        	    }
