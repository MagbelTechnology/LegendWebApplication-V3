...
public class APVoucherActionServlet extends HttpServlet {
        ...
        com.magbel.ia.vao.User user = null;
         ...
         if(addChequeBtn != null){
                        if(serviceBus.createVoucherPayment(orderCode,chequeNo,bankCode,amountOwing,amountPaid,userId,pymtType,accountNo,
                                                            ...
                                                            drAmount,transactionDate,crAcctNo,crAcctType,crTranCode,crNarration,crAmount,companyCode,familyid,branchCode,invoiceNo,customerAcct,accountBalance,orderNo)){
                        	boolean done1 = Apprecord.updateCRBalances(crAcctNo,branchCode,crAmount);
                        	if(done1 == true){Apprecord.updateCustomerDRBalances(customerAcct,branchCode,drAmount,accountBalance);}
                        	if(done1==true){
                        	...
                        	for(int i=0; i<noofrows; i++) {	
                        		...
                        		if(done4 == true){done3 = Apprecord.updateCRBalances(creditAccount,SchoolCode,critemAmount);}
                            	if(done3==true){Apprecord.updateDRBalances(drAcctNo,SchoolCode,dritemAmount);}
