...
public class SClassAuditServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		legend.admin.objects.SecurityClass sc = new legend.admin.objects.SecurityClass();
		...
		String isSupervisor = request.getParameter("isSupervisor");
                isSupervisor = isSupervisor == null?"N":isSupervisor.trim();
       ...
       if (hostName.equals(request.getRemoteAddr())) {
		...
		try {
				...
				if (classId.equals("")) {
					if (sh.getSecurityClassByName(className) == null) {
						if (sh.createSecurityClass(sc)) {
							...
							out
									.print("<script>window.location = 'securityClasses.jsp?classId="
			...
			out.print("<script>window.location = 'securityClasses.jsp?classId="
					+ classId + "&PC=10'</script>");
		}
	}
}
