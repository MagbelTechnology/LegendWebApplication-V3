...
public class LocationAuditServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws
		...
		legend.admin.objects.Locations  location = new  legend.admin.objects.Locations();					
            ...
            if(buttSave != null)
				{
                 if(locationId.equals(""))
					{
                      locationCode  = new ApplicationHelper().getGeneratedId("AM_GB_LOCATION");
                      location.setLocationCode(locationCode);
                	 if (locationhandle.getLocationByLocCode(locationCode)!=null) {
						...
						if(locationhandle.createLocation(location))
							{
								String locateId = locationhandle.getLocationByLocCode(locationCode).getLocationId();
								//statusMessage = "Record saved successfully";
								out.print("<script>alert('Record saved successfully.')</script>");
								out.print("<script>window.location = 'DocumentHelp.jsp?np=manageLocation&status=ACTIVE'</script>");
							}
						else
						   	 {
                 ...
                 if(!locationId.equals(""))
					{
                	 location.setLocationId(locationId);	
						...
						updtst = audit.logAuditTrail("AM_GB_LOCATION" ,  branchcode, userID, locationId,hostName,ipAddress,macAddress);
                        if(updtst == true)
							{
								//statusMessage = "Update on record is successfull";
								out.print("<script>alert('Update on record is successful')</script>");
//								out.print("<script>window.location = 'DocumentHelp.jsp?np=locationSetup.jsp?locationId="+locationId+"&PC=62'</script>");
								out.print("<script>window.location = 'DocumentHelp.jsp?np=manageLocation&status=ACTIVE'</script>");
