...
public class CompanyDefaultApprovalServlet extends HttpServlet {
 ...
 //           if (cmdSave != null || cmdNext != null) {
           if ((cmdSave != null)   && (astatus.equalsIgnoreCase("A"))) {
        	   arb.deleteOtherSupervisorswithBatchId(String.valueOf(tranId),userid);
            	...
            	{
            		 if (ch.createDefaultsCompany(company)) {
...
//                                 companyCode + "'</script>");}
                ...
                System.out.println("After Update in CompanyApprovalAuditServlet : " + recType);
                audit.select(2,
                        "SELECT * FROM  AM_GB_COMPANY  WHERE company_code = '" +
                        companyCode + "'");
                /*
