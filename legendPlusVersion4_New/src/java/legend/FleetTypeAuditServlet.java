 ...
 * @version 1.0
 */
public class FleetTypeAuditServlet extends HttpServlet {
    public FleetTypeAuditServlet() {
    }
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws
		...
		if(fleetTypeId == null) { fleetTypeId = "not set"; 	}
        ...
        //String user = (String)session.getAttribute("CurrentUser");
         ...
         String fleetTypeCode =  request.getParameter("fleetTypeCode");
          String description =  encodeForHTML(request.getParameter("description"));
          String glAccount =  encodeForHTML(request.getParameter("glAccount"));
		  		            //            (String)session.getAttribute("CurrentUser")
		legend.admin.objects.fleetType  type = new  legend.admin.objects.fleetType();					
        
		//type.setFleetTypeId(fleetTypeId);	
		type.setFleetTypeCode(fleetTypeCode);
            ...
            if(buttSave != null)
				{
                 if(fleetTypeId.equals(""))
					{
                	 if (locationhandle.getfleetTypeByTypeCode(fleetTypeCode)!=null) {
 						out.print("<script>alert('The Fleet Type with code already exists .')</script>");
 						out.print("<script>history.go(-1);</script>");
 					} else {
						if(locationhandle.createFleetType(type))
							{
								//statusMessage = "Record saved successfully";
								out.print("<script>alert('Record saved successfully.')</script>");
								out.print("<script>window.location = 'DocumentHelp.jsp?np=fleetType&fleetTypeId="+(locationhandle.getfleetTypeByTypeCode(fleetTypeCode)).getFleetTypeId()+"&PC=62;'</script>");
							}
						else
						   	 {
                 ...
                 if(!fleetTypeId.equals(""))
					{
                	 type.setFleetTypeId(fleetTypeId);	
						...
						audit.select( 1, "SELECT * FROM  FT_PROCESSING_TYPE   WHERE FT_MTID = '"+ fleetTypeId +"'");
						boolean isupdt = locationhandle.updateFleetType(type);
						audit.select( 2, "SELECT * FROM  FT_PROCESSING_TYPE  WHERE FT_MTID = '"+ fleetTypeId +"'");
						updtst = audit.logAuditTrail("FT_PROCESSING_TYPE" ,  branchcode, userID, fleetTypeId,hostName,ipAddress,macAddress);
                        if(updtst == true)
							{
								//statusMessage = "Update on record is successfull";
								out.print("<script>alert('Update on record is successful')</script>");
								out.print("<script>window.location = 'DocumentHelp.jsp?np=fleetType&fleetTypeId="+fleetTypeId+"&PC=62'</script>");
								...
								//statusMessage = "No changes made on record";
								 ...
								 out.print("<script>window.location = 'fleetType.jsp?fleetTypeId="+fleetTypeId+"&PC=62'</script>");
								}
					}
				}
        }
