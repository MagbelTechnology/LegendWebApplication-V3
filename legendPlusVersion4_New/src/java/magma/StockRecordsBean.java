 ...
 * @version 1.0
 */
public class StockRecordsBean extends legend.ConnectionClass 
{ 
	...
	public ApprovalManager approvalMan;
	public DistributionOrderServiceBus distributionBus;
    ...
    public StockRecordsBean() throws Exception {
    	approvalRec = new ApprovalRecords(); 
        approvalMan = new ApprovalManager();
...
//        System.out.println("<<<<<<<<<<<INSIDE rinsertAssetRecord>>>>>> category_id: "+category_id);
        Connection con = null;
...
AssetPaymentManager payment = null;
        ...
        noOfMonths = noOfMonths==null ? noOfMonths : "0" ;
...
//        System.out.println("<<<<<<rinsertAssetRecord assetCode: "+assetCode+"     bar_code: "+bar_code+"    parentTag: "+parentTag+"    category_id: "+category_id);
        String createQuery = "INSERT INTO ST_STOCK         " +
                             ...
                             "WAREHOUSE_CODE,ITEMTYPE, ITEM_CODE,QUANTITY,UNIT_CODE,PARENT_TAG)  "+
                             "VALUES" +
                             "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
                             "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
//            System.out.println("<<<<<<rinsertAssetRecord description: "+description);
            ...
            ps.setDate(22, dbConnection.dateConvert(date_of_purchase));
            System.out.println("<<<<<<=====residual_value: "+residual_value);
            if(residual_value==""){residual_value = "10";}
            ...
            ps.setDate(26, dbConnection.dateConvert(date_of_purchase));
  ...
  //          System.out.println("<<<<<<rinsertAssetRecord bar_code: "+bar_code+"     bar_code Length: "+bar_code.length());
            ...
            ps.setInt(85, quantity);
            ps.setString(86, unitCode);
            ps.setString(87, parentTag);
            System.out.println("<<<<<warehouseCode in rinsertAssetRecord: "+warehouseCode+"    itemType: "+itemType+"    asset_id: "+asset_id);
            ps.execute();  
            ...
            System.out.println("WARN:Error creating asset in ST_STOCK Table->" + ex);
        } finally {
    ...
    public boolean updateAssetRecord() throws Exception {
        String query = "update ST_STOCK SET  " +
                       ...
                       "WAREHOUSE_CODE = ?,ITEMTYPE = ?,ITEMCODE = ?,UNIT_CODE = ?,QUANTITY = ?,PARENT_TAG = ?  "+
        ...
        if (parentTag == null || parentTag.equals("")) {
        	parentTag = "0";
        }
...
//            System.out.println("<<<<<<<<<<<<<<<<<<description in updateAssetRecord: "+description+"    bar_code: "+bar_code);
            ...
            ps.setString(67, unitCode);
            ps.setInt(68, quantity);
            ps.setString(69, parentTag);
    ...
    public void getAssetRecords() throws Exception {
        ...
        if (asset_id != "") {
            String query = "SELECT A.ASSET_ID, A.REGISTRATION_NO," +
                           ...
                           "A.WAREHOUSE_CODE, A.ITEM_CODE,A.ITEMTYPE,A.QUANTITY,A.UNIT_CODE,A.PARENT_TAG,A.UNIT_PRICE FROM ST_STOCK A " +
...
//			System.out.println("getAssetRecords query: "+query);
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    selectTax = rs.getInt("wht_percent");
                    ...
                    parentTag = rs.getString("PARENT_TAG");
                ...
                System.out.println("WARN: Error fetching all asset in getAssetRecords->" + ex);
            } finally {
                dbConnection.closeConnection(con, ps);
            }
        }
    ...
    public boolean isDepreciationReCalculatable(String assetid) {
        ...
        String query = "SELECT ACCUM_DEP FROM ST_STOCK  " +
                       "WHERE ASSET_ID = ?";
        try {
            con = dbConnection.getConnection("legendPlus");
            ps = con.prepareStatement(query);
    ...
    public String findResidualValue() {
        ...
        try {
            ...
            System.out.println("WARN: Error fetching all AM_GB_COMPANY ->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        return residual;
...
//     System.out.println("the pre output is " + sdf1.toPattern());
    ...
    public boolean chkidExists(String assetid) {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String SQL = "SELECT * FROM ST_STOCK WHERE asset_id='" + assetid + "'";
        try {
            con = dbConnection.getConnection("legendPlus");
    ...
    public int insertAssetRecord() throws Exception, Throwable {
        ...
        if(budget[3].equalsIgnoreCase("N")){
        	System.out.println("ABOUT TO EXECUTE rinsertAssetRecord");
			rinsertAssetRecord();
			return DONE;
		}
...
//        System.out.println("getting quarters");
    ...
    if (parentTag != null) {
            this.parentTag = parentTag.replaceAll("'", "");
        }
...
String result="";
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query =
                "SELECT Subject_TO_Vat FROM ST_STOCK  " +
                "WHERE asset_id = '" + id + "' ";
...
public String checkCategoryCode()
    ...
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query =
                "SELECT CATEGORY_CODE FROM ST_STOCK" ;
...
public String checkBranchCode()
    ...
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query =
                "SELECT BRANCH_CODE FROM ST_STOCK" ;
...
//    	System.out.println("TEST a[9]setApprovalData)===> "+a[9]+"   a[2]: "+a[2]+"    code: "+code);
...
//String q ="select asset_id, asset_status,user_ID,supervisor,Cost_Price,Posting_Date,description,effective_date,BRANCH_CODE from ST_STOCK where asset_id ='" +id+"'";
    ...
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query ="select asset_id,user_ID,supervisor,Cost_Price,Posting_Date," +
                 " description,effective_date,BRANCH_CODE,Asset_Status from ST_STOCK where asset_id ='" +id+"'";
...
public String[] setApprovalData(String id,String code){
   ...
   // System.out.println("the $$$$$$$$$$$ "+currentDate);
    ...
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query ="select asset_id,user_ID,supervisor,Cost_Price,Posting_Date," +
                 " description,effective_date,BRANCH_CODE,Asset_Status from ST_STOCK where asset_id ='" +id+"'";
         String tranTypeQuery = "select Transaction_type from approval_level_setup where code =?";
         String tranType = approvalRec.getCodeName(tranTypeQuery,code);
...
public void updateAssetStatusApproval(int transId, String userId){
...
String query_r ="update am_asset_approval set asset_status='ACTIVE',DATE_APPROVED = '"+approveddate+"' where transaction_id = '"+transId+"' AND super_id = '"+userId+"'";
...
String query_r ="update ST_STOCK set asset_status=?,raise_entry=? where asset_id = '"+assetId+"'";
//System.out.println("AssetRecordBean: updateAssetStatus()>>>>>" + query_r);
Connection con = null;
        PreparedStatement ps = null;
        //ResultSet rs = null;
   ...
   String[] result= new String[12];
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query ="select asset_id,user_ID,supervisor,Cost_Price,Posting_Date,description,effective_date,BRANCH_CODE,Asset_Status from ST_STOCK where asset_id ='" +id+"'"; ;
...
Connection con = null;
        PreparedStatement ps = null;
        //ResultSet rs = null;
 //       System.out.println("in update stock status change of stock records beans================ "+query_r);
try {
    con = dbConnection.getConnection("legendPlus");
...
ps = con.prepareStatement(query_r);
           ...
           dbConnection.closeConnection(con, ps);
            ...
            System.out.println("StockRecordBean: updateAssetStatusChange()>>>>>" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
...
public String checkBranchCode()
	{
		String branch="";
		String query="SELECT BRANCH_CODE FROM ST_STOCK";
...
*/
	...
	{
		String branch="";
		String query="SELECT BRANCH_CODE FROM ST_STOCK where Asset_id='"+Asset_id+"'";
	...
	{
		String category="";
		String query="SELECT CATEGORY_CODE FROM ST_STOCK  where Asset_id='"+Asset_id+"'";
...
*/
    	public boolean checkApprovalStatus(String id)
	{
            ...
            String asset_status="";
		String query="SELECT asset_status FROM ST_STOCK where asset_id ='"+id+"'";
   ...
   String result="";
    Connection con = null;
        ...
        try {
            ...
            System.out.println("StockRecordsBean WARN: Error fetching getAssetTransInfo(int tran_id) ->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
   ...
   int result=100;
    Connection con = null;
       ...
       try {
            ...
            System.out.println("StockRecordsBean: getNumOfTransactionLevel(String levelCode) WARN: Error fetching CategoryCode ->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
...
public boolean updateNewAssetStatus(String assetId) throws Exception {
        ...
        String query = "update ST_STOCK SET  asset_status = 'ACTIVE' ,Finacle_Posted_Date= ? where asset_id ='" +assetId+"'";
         boolean done = true;
        Connection con = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            System.out.println("StockRecordsBean: updateNewAssetStatus: WARN:Error updating asset->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        return done;
    ...
    Connection con = null;
        ...
        ResultSet rs = null;
         ...
         String query ="select asset_id,user_ID,supervisor,Cost_Price,Posting_Date,description," +
                 "effective_date,BRANCH_CODE,Asset_Status from ST_STOCK where asset_id ='" +id+"'"; ;
...
public boolean insertInToAssetRecordTemp(String assetId,String userId) throws Exception {
        ...
        try {
            ...
            System.out.println("StockRecordsBean: insertInToAssetRecordTemp():WARN:Error inserting to am_assetUpdate table->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        return done;
...
public boolean insertInToAssetRecordTemp(String assetId,String userId,String sup_id,String status) throws Exception {
...
public int changeAssetID(String oldAssetId,String newAssetId)
    ...
    String query_r ="update ST_STOCK set asset_id = ? where ASSET_ID=? ";
    	...
    	ps = con.prepareStatement(query_r);
    	            ...
    	            System.out.println("StockRecordsBean: changeAssetID(): asset id update not successful" + ex);
    	        } finally {
    	            dbConnection.closeConnection(con, ps);
    	        }
...
//	System.out.println("====code 1====> "+code);
    String mtid_id ="";
        ...
        ResultSet rs;
		String []spuervisorlist = a[2].split("#");
		int No = spuervisorlist.length;
//		System.out.println("====No 1====> "+No);
		// String reqnID = cg.generateCode("BULKSTOCKTRANSFER", "", "", "");
		 String reqnID =  a[0];
//		 if(No>1){batchId = new ApplicationHelper().getGeneratedId("am_asset_approval");}
 String pq = "insert into am_asset_approval(asset_id,user_id,super_id,amount,posting_date,description," +
         "effective_date,branchCode,asset_status,tran_type, process_status,tran_sent_time,transaction_id,batch_id," +
         "transaction_level) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 String tranLevelQuery = "select level from approval_level_setup where code ='"+code+"'";
        ...
        try
        {
            ...
            /////////////To get transaction level
             ps1 = con.prepareStatement(tranLevelQuery);
              rs = ps1.executeQuery();
            ...
            for(int j=0;j<No;j++){
            String mtid =  new ApplicationHelper().getGeneratedId("am_asset_approval");
//            System.out.println("====asset id====> "+reqnID);
            ps.setString(1, reqnID);
            ps.setString(2, (a[1]==null)?"":a[1]);
            ps.setString(3, (a[2]==null)?"":spuervisorlist[j]);
            ps.setDouble(4, (a[3]==null)?0:Double.parseDouble(a[3]));
            //ps.setDate(5, (a[4])==null?null:dbConnection.dateConvert(a[4]));
            ps.setTimestamp(5,dbConnection.getDateTime(new java.util.Date()));
            ps.setString(6, (a[5]==null)?"":a[5]);
            ...
            ps.setString(12, timer.format(new java.util.Date()));
            ps.setString(13,mtid);
            ps.setString(14, reqnID);
            ps.setInt(15, transaction_level);
    //        System.out.println("====transaction level====> "+transaction_level);
            ps.execute();
...
public boolean updateAsset(String assetId,String userId) throws Exception {
    String query = "update ST_STOCK set " +
                   ...
                   "dep_rate=?,wht_percent=?,SPARE_3=? ,SPARE_4=? ,SPARE_5=? ,SPARE_6=?,sub_category_id=?,SUB_CATEGORY_CODE=?,"+
                   "UNIT_CODE = ?,QUANTITY = ?,PARENT_TAG = ?  where asset_id=? and user_id=? " ;
...
//	System.out.println("vatable_cost "+vatable_cost);
        ...
        ps.setString(63, unitCode);
        ps.setInt(64, quantity);
        ps.setString(65, parentTag);
        ps.setString(66, assetId);
        ps.setString(67,userId);
       
        ps.execute();

    } catch (Exception ex) {
        done = false;
        System.out.println("StockRecordsBean: updateasset():WARN:Error updating to ST_STOCK table->" + ex);
    } finally {
        dbConnection.closeConnection(con, ps);
    }
    return done;
 ...
 /**
	...
	/*String query = "UPDATE ST_STOCK SET DESCRIPTION=? "
	...
	try {
		...
		for (int i = 0; i < list.size(); i++) {
            ...
            String subcatQuery = "SELECT sub_category_code FROM   am_ad_sub_category WHERE sub_category_id = ?";
            String subcatcode = approvalRec.getCodeName(subcatQuery,subcatid);
        ...
        //int groupId = Integer.parseInt(id);
         String query ="select group_id,user_ID,supervisor,Cost_Price,Posting_Date," +
         		"		description,effective_date,BRANCH_CODE," +
         				"Asset_Status from am_group_stock_main where group_id =" +id ;
...
//        		 System.out.println("query in setApprovalDataGroup : " + query);
        try {
            ...
            while (rs.next()) {
              //  result[0] = String.valueOf(id);
                result[0]= rs.getString(1);//Group_Id
                result[1]= rs.getString(2);
                result[2] = rs.getString(3);
               result[3] = rs.getString(4);
     ...
     //          System.out.println("Before Conversion" + rs.getDate(5));
      ...
      //      System.out.println("Final Conversion");
            ...
            System.out.println("StockRecordsBean : setApprovalData()WARN: Error setting approval data for group asset creation ->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        	result[9] = "Group Stock Creation";
        	result[10] = "P";
//result[11] = timeInstance();
        return result;
...
public boolean updateNewApprovalAssetStatus(String groupID, int supervise) throws Exception {
        ...
        try {
            ...
            System.out.println("StockRecordsBean: updateNewApprovalAssetStatus: WARN:Error updating asset->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        return done;
...
public boolean updateNewAssetStatux(String assetId) throws Exception {
        ...
        String query = "update ST_STOCK SET  asset_status = 'APPROVED' where asset_id ='" +assetId+"'";
         boolean done = true;
        Connection con = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            System.out.println("StockRecordsBean: updateNewAssetStatux(): WARN:Error updating asset->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        return done;
    ...
    public String vendorName (String assetId)
		{
			...
			String query="select  vendor_name from am_ad_vendor where vendor_id=" +
                    "(select supplier_name from ST_STOCK where asset_id='"+assetId+"')";
...
*/
        ...
        residual_value = residual_value.replaceAll(",", "");
        amountPTD = amountPTD.replaceAll(",","");
        String createQuery = "INSERT INTO ST_STOCK_ARCHIVE        " +
                             "(" +
                             "ASSET_ID, REGISTRATION_NO, BRANCH_ID, DEPT_ID," +
                             "SECTION_ID, CATEGORY_ID, [DESCRIPTION], VENDOR_AC," +
                             "DATE_PURCHASED, DEP_RATE, ASSET_MAKE, ASSET_MODEL," +
                             ...
                             "defer_pay,wht_percent,system_ip,mac_address,asset_code,SPARE_3, SPARE_4,SPARE_5, SPARE_6,sub_category_id, SUB_CATEGORY_CODE,PARENT_TAG) " +
                             "VALUES" +
                             "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
                              "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        /*
        ...
        try {
          ...
          //  ps.setDate(22, dbConnection.dateConvert(depreciation_start_date));
            ps.setDate(22, dbConnection.dateConvert(date_of_purchase));
            ps.setDouble(23, Double.parseDouble(residual_value));
            ps.setString(24, authorized_by);
            ps.setTimestamp(25, dbConnection.getDateTime(new java.util.Date()));
...
public boolean insertRaiseEntryTransactionArchive(String id,String Description,String debitAccount,String creditAccount,double amount,String iso,String asset_id,String page1,String transactionId,String systemIp,String mac_Address ) {
    boolean done=true;
...
System.out.println("AssetRecordBean: updateAssetStatusArchive()>>>>>" + query_r);
Connection con = null;
        PreparedStatement ps = null;
        //ResultSet rs = null;
...
*/
   ...
   public boolean insertRaiseEntryTransaction(String id,String Description,String debitAccount,String creditAccount,double amount,String iso,String asset_id,String page1,String transactionId,String ip_address ) {
    boolean done=true;
...
String query_r ="update am_asset_approval set approval_level_count=?,posting_date=? where transaction_id =? and super_id=?";
 ...
 public void getAssetRecordsForApprovalArchive(String assetId, String update_changes) throws Exception {
        ...
        if (!(assetId.equalsIgnoreCase(""))) {
            String query = "SELECT A.ASSET_ID, A.REGISTRATION_NO," +
                           ...
                           "A.VENDOR_AC,A.SPARE_1,A.SPARE_2,A.SPARE_3,A.SPARE_4,A.SPARE_5,A.SPARE_6,A.REQ_REDISTRIBUTION,A.DATE_DISPOSED," +
                           "A.[USER_ID], A.WHO_TO_REM_2,A.MULTIPLE,A.WAR_START_DATE,WAR_MONTH,WAR_EXPIRY_DATE," +
                           "AMOUNT_PTD,AMOUNT_REM,PART_PAY,FULLY_PAID, GROUP_ID,A.BAR_CODE,A.SBU_CODE,A.LPO,A.SUPERVISOR,A.defer_pay,A.wht_percent,PARENT_TAG " +
                           "FROM ST_STOCKUPDATE A " +
 ...
 //                   System.out.print("nothing");
         ...
         "ASSET_MAINTENANCE = ?, AUTHORIZED_BY = ? , SUPPLIER_NAME = ?, PURCHASE_REASON = ?,"+
          ...
          ps = con.prepareStatement(updateQuery);
                    ...
                    ps.setString(23, lpo);
                    ps.setInt(24,selectTax);
                    ps.setString(25, spare_3);
                    ps.setString(26, spare_4);
                    ps.setString(27, spare_5);
                    ps.setString(28, spare_6);
                    ps.setString(29, asset_id);
                ...
                System.out.println("StockRecordsBean: getAssetRecordsForApprovalArchive()WARN: Error fetching all Stock/ updating ST_StockUpdate table ->" + ex);
            } finally {
                dbConnection.closeConnection(con, ps,rs);
            }
        }
        ...
        if (!(assetId.equalsIgnoreCase(""))) {
            ...
            try {  
                ...
                if(update_changes.equalsIgnoreCase("YES")){
                    ...
                    String updateQuery = "UPDATE ST_STOCK SET REGISTRATION_NO = ?," +
         "ASSET_MAINTENANCE = ?, AUTHORIZED_BY = ? , SUPPLIER_NAME = ?, PURCHASE_REASON = ?,"+
          "DESCRIPTION = ?, VENDOR_AC = ? , ASSET_SERIAL_NO = ?, ASSET_MODEL = ?,"+
          "USER_ID = ?, SUBJECT_TO_VAT = ? , WHO_TO_REM = ?, EMAIL1 = ?,"+
          "EMAIL2 = ?, WH_TAX = ? , SPARE_1 = ?, SPARE_2 = ?,"+
          "WHO_TO_REM_2 = ?, DRIVER = ? , ASSET_ENGINE_NO = ?, WAR_MONTH = ?,"+
          "BAR_CODE = ?, LPO = ?, wht_percent=?, SPARE_3 = ?, SPARE_4 = ?, SPARE_5 = ?, SPARE_6 = ?,PARENT_TAG = ?  WHERE ASSET_ID = ?";
          ...
          ps = con.prepareStatement(updateQuery);
                    ...
                    ps.setString(27, spare_5);
                    ps.setString(28, spare_6);
                    ps.setString(29, parentTag);
                    ps.setString(30, asset_id);
                ...
                System.out.println("StockRecordsBean: getAssetRecordsForApproval()WARN: Error fetching all Stock/ updating ST_StockUpdate table ->" + ex);
            } finally {
                dbConnection.closeConnection(con, ps,rs);
            }
        }
 ...
 String tranLevelQuery = "select level from approval_level_setup where code ='"+code+"'";
   ...
   //String pq = "insert into am_asset_approval(asset_id,user_id,super_id,amount,posting_date,description,effective_date,branchCode,tran_type, process_status,tran_sent_time) values(?,?,?,?,?,?,?,?,?,?,?)";
    ...
    PreparedStatement ps = null;
      ...
      //  System.out.println("In StockRecordsBean+++++++++++++++++++++++++++++++updateAssetStatus++");
        if (vatable_cost == null || vatable_cost.equals("")) {
        vatable_cost = "0.0";
    }
              if (vat_amount == null || vat_amount.equals("")) {
...
public boolean insertRaiseEntryTransactionArchiveTranId(String id,String Description,String debitAccount,String creditAccount,double amount,String iso,String asset_id,String page1,String transactionId,String systemIp,String mac_Address,int tranId ) {
  ...
  public int insertAssetRecordUnclassified() throws Exception, Throwable {
    String[] budget = getBudgetInfo();
    double[] bugdetvalues = getBudgetValues();
    int DONE = 0; //everything is oK
    int BUDGETENFORCED = 1; //EF budget = Yes, CF = NO, ERROR_FLAG
    ...
    String Q = getQuarter();
    if(budget[3].equalsIgnoreCase("N")){
    	rinsertAssetRecordUnclassified();
		return DONE;
	}
	else if(budget[3].equalsIgnoreCase("Y")){
    if (!Q.equalsIgnoreCase("NQ")) {
...
private boolean rinsertAssetRecordUnclassified() throws Exception, Throwable {
    asset_id = new legend.AutoIDSetup().getIdentity(branch_id,
            department_id, section_id, category_id);
    Connection con = null;
    PreparedStatement ps = null;
...
*/
    ...
    String createQuery = "INSERT INTO ST_STOCK_UNCAPITALIZED         " +
                         "(" +
                         "ASSET_ID, REGISTRATION_NO, BRANCH_ID, DEPT_ID," +
                         "SECTION_ID, CATEGORY_ID, [DESCRIPTION], VENDOR_AC," +
                         "DATE_PURCHASED, DEP_RATE, ASSET_MAKE, ASSET_MODEL," +
                         ...
                         "BRANCH_CODE,DEPT_CODE,SECTION_CODE,CATEGORY_CODE,	AMOUNT_PTD," +
                         "AMOUNT_REM,PART_PAY,FULLY_PAID,BAR_CODE,SBU_CODE,LPO,supervisor,defer_pay,wht_percent," +
                         "system_ip,mac_address,asset_code, SPARE_3, SPARE_4, SPARE_5, SPARE_6, SUB_CATEGORY_ID,SUB_CATEGORY_CODE) " +
...
public void getAssetRecordsUnclassified() throws Exception {
    ...
    if (asset_id != "") {
        ...
        try {
            ...
            System.out.println("WARN: Error fetching all asset in getAssetRecordsUnclassified ->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
    }
...
public String[] setApprovalDataBranch(String id){
	   ...
	   // System.out.println("the $$$$$$$$$$$ "+currentDate);
	    ...
	    Connection con = null;
	        ...
	        ResultSet rs = null;
	         ...
	         String query ="select asset_id,user_ID,supervisor,Cost_Price,Posting_Date," +
	                 " description,effective_date,BRANCH_CODE,Asset_Status from ST_STOCK_UNCAPITALIZED where asset_id ='" +id+"'";
	...
	result[9] = "Sock Creation Uncapitalized";
	result[10] = "P";
	//result[11] = timeInstance();
	        return result;
	...
	String result="";
	    Connection con = null;
	        ...
	        ResultSet rs = null;
	         ...
	         String query =
	                "SELECT Subject_TO_Vat FROM ST_STOCK_UNCAPITALIZED  " +
	                "WHERE asset_id = '" + id + "' ";
	...
	String result="";
	    Connection con = null;
	        ...
	        ResultSet rs = null;
	         ...
	         String query =
	                "SELECT wh_tax FROM ST_STOCK_UNCAPITALIZED  " +
	                "WHERE asset_id = '" + id + "' ";
...
return 	assetAcqusitionSuspense;
    ...
    public void setPendingTrans(String[] a, String code,int assetCode){
        ...
        PreparedStatement ps;
        ResultSet rs;
        System.out.print("<<<<<<<    spuervisorlist: "+a[2]);
		String []spuervisorlist = a[2].split("#");
		
		int No = spuervisorlist.length;
		System.out.print("<<<<<<<    counter No: "+No);
...
//              System.out.println(code);
   ...
   //String pq = "insert into am_asset_approval(asset_id,user_id,super_id,amount,posting_date,description,effective_date,branchCode,tran_type, process_status,tran_sent_time) values(?,?,?,?,?,?,?,?,?,?,?)";
    ...
    String mtid_id ="";
		...
		String []spuervisorlist = a[2].split("#");
		int No = spuervisorlist.length;
//		System.out.println("====No 2====> "+No);
	//	String reqnID = cg.generateCode("BULKSTOCKTRANSFER", "", "", "");
		String reqnID =  new ApplicationHelper().getGeneratedId("am_asset_approval");
 String pq = "insert into am_asset_approval(asset_id,user_id,super_id,amount,posting_date,description," +
         "effective_date,branchCode,asset_status,tran_type, process_status,tran_sent_time,transaction_id,batch_id," +
         "transaction_level,asset_code) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 String tranLevelQuery = "select level from approval_level_setup where code ='"+code+"'";
        ...
        {
            System.out.println(">>>AssetRecordBeans:setPendingTrans2 in setPendingTrans2>>>>>>" + er);
...
//		System.out.print("<<<<<<<    counter No: "+No);
 ...
 public boolean insertRaiseEntryTransaction(String id,String Description,String debitAccount,String creditAccount,double amount,String iso,String asset_id,String page1,String transactionId,int assetCode ) {
    boolean done=true;
...
String query_r ="update am_asset_uncapitalized set asset_status=?,raise_entry=? where asset_id = '"+assetId+"'";
...
public boolean updateNewAssetStatusBranch(String assetId) throws Exception {
        ...
        String query = "update am_asset_uncapitalized SET  asset_status = 'ACTIVE' ,Finacle_Posted_Date= ? where asset_id ='" +assetId+"'";
         boolean done = true;
        Connection con = null;
        PreparedStatement ps = null;
...
public String checkCategoryCodeByAssetCode(int assetCode)
	{
		String category="";
		String query="SELECT CATEGORY_CODE FROM ST_STOCK  where asset_Code="+assetCode;
...
{
	String category="";
	String query="SELECT CATEGORY_CODE FROM ST_STOCK_UNCAPITALIZED  where asset_Code="+assetCode;
...
public void getAssetRecords3() throws Exception {
    ...
    if (asset_id != "") {
        ...
        String query = "SELECT A.ASSET_ID, A.REGISTRATION_NO," +
                       ...
                       "A.[USER_ID], A.WHO_TO_REM_2,A.MULTIPLE,A.WAR_START_DATE,WAR_MONTH,WAR_EXPIRY_DATE," +
                       "AMOUNT_PTD,AMOUNT_REM,PART_PAY,FULLY_PAID, GROUP_ID,A.BAR_CODE,A.SBU_CODE," +
                       "A.LPO,A.SUPERVISOR,A.defer_pay,A.wht_percent,A.asset_code,PARENT_TAG " +
                       "FROM ST_STOCK A " +
        ...
        try {
            ...
            System.out.println("WARN: Error fetching all asset in getAssetRecords3->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
    }
...
public String getDisposalSupervisor3(String asset_code, String trans_type){
	  String query = "select super_id from am_asset_approval where asset_code ='"+asset_code+"'and tran_type ='"+trans_type+"'";
	   String result="";
	    Connection con = null;
	        PreparedStatement ps = null;
	        ResultSet rs = null;
...
public boolean rinsertAssetMemoRecord() throws Exception, Throwable {
        ...
        try 
		{
			...
			String createQueryMemo = "INSERT INTO ST_STOCK_MEMO         " +
            "(" +
...
public void getMemoAssetRecords() throws Exception {
    ...
    if (asset_id != "") {
        String query = "SELECT A.MEMO_ASSET_ID, A.REGISTRATION_NO," +
                       ...
                       "AMOUNT_PTD,AMOUNT_REM,PART_PAY,FULLY_PAID, GROUP_ID,A.BAR_CODE,A.SBU_CODE," +
                       "A.LPO,A.SUPERVISOR,A.defer_pay,A.wht_percent,A.asset_code " +
                       "FROM ST_STOCK_MEMO A " +
                       "WHERE A.MEMO_ASSET_ID = '" + asset_id + "'";
        ...
        try {
            ...
            System.out.println("WARN: Error fetching all asset in getMemoAssetRecords ->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
    }
...
//new------------------
public boolean insertRaiseEntryTransaction(String id,String Description,String debitAccount,String creditAccount,double amount,String iso,String asset_id,String page1,String transactionId,String ip_address,String tranId, int assetCode,String Finacle_trans_id ) {
  boolean done=true;
...
public boolean insertRaiseEntryTransaction(String id,String Description,String debitAccount,String creditAccount,double amount,String iso,String asset_id,String page1,String transactionId,int assetCode,String Finacle_trans_id ) {
  boolean done=true;
...
//	System.out.println("setUploadPendingTrans AssetId===> "+AssetId);	
    ...
    PreparedStatement ps;
    ResultSet rs; 
	  	String qrylevel =" select level from approval_level_setup  where code=?";
  	    transaction_level = Integer.parseInt(approvalRec.getCodeName(qrylevel,code));
...
String pq = "insert into am_asset_approval_archive(asset_id,user_id,super_id,amount,posting_date,description,"+
        "effective_date,branchCode,asset_status,tran_type, process_status,tran_sent_time,transaction_id,batch_id,"+
        "transaction_level,Asset_code) select asset_id,user_ID,'"+supervisor+"',Cost_Price,'"+dbConnection.getDateTime(new java.util.Date())+"',description,effective_date,"+
        "BRANCH_CODE,Asset_Status,'Asset Upload','P','"+timer.format(new java.util.Date())+"','"+new ApplicationHelper().getGeneratedId("am_asset_approval")+"',Group_id,'"+transaction_level+"','"+AssetCode+"' from ST_STOCK Asset_Id = '"+AssetId+"'";
...
public boolean setUploadCompleteTrans(String AssetId){
    ...
    //	   System.out.println("Before Update Asset Code ====  "+AssetCode);
    	    ...
    	    String UpadteQuerry = "UPDATE ST_STOCK set Asset_Status = 'ACTIVE' where ASSET_ID = '"+AssetId+"'";
    	    try {
    	        con = dbConnection.getConnection("legendPlus");
    	        ...
    	        System.out.println("WARN: Error fetching all asset in setUploadCompleteTrans->" + ex);
    	    } finally {
    	        dbConnection.closeConnection(con, ps);
    	    }
    	    return done;
...
public boolean setUpdateUploadTrans(String groupid,String userid){
    	    ...
    	    String UpadteQuerry = "update am_group_asset set process_flag = 'Y' where  GROUP_ID='"+groupid+"' AND USER_ID='"+userid+"'";
    	    try {
    	        con = dbConnection.getConnection("legendPlus");
    	        ps = con.prepareStatement(UpadteQuerry);
    	        done = (ps.executeUpdate() != -1);
...
String pq = "insert into am_asset_approval(asset_id,user_id,super_id,amount,posting_date,description,"+
        ...
        "transaction_level,Asset_code) values('"+AssetId+"',"+userid+",'"+supervisor+"','"+amount+"','"+dbConnection.getDateTime(new java.util.Date())+"','"+description+"','"+dbConnection.getDateTime(new java.util.Date())+"',"+
        "'"+branchcode+"','PENDING','"+TranType+"','P','"+timer.format(new java.util.Date())+"','"+new ApplicationHelper().getGeneratedId("am_asset_approval")+"','"+AssetId+"','"+transaction_level+"','"+AssetCode+"')";
//from ST_STOCK_UNCAPITALIZED where Asset_Id = '"+AssetId+"'
//System.out.println("setUploadPendingTrans Query pq===> "+pq);
    try
    {
        con = dbConnection.getConnection("legendPlus");
...
//        System.out.println("Query getUncapitalizedAssetRecords>>>> "+query);
        ...
        try {
            ...
            System.out.println("WARN: Error fetching all asset in getUncapitalizedAssetRecords->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
    }
...
String query_r ="update AM_GROUP_ASSET_UNCAPITALIZED set process_flag='Y' where GROUP_ID = '"+batchId+"'";
 ...
 * *Bulk Stock Transfer
 */
public String bulkStockTransfer(java.util.ArrayList list,String reqnId,String compCode,String userid) {
 String query = "update ST_STOCK SET  " +
				 "BRANCH_ID=?,BRANCH_CODE=?,DEPT_ID=?," +
				 "DEPT_CODE=?,SECTION_ID=?," +
				 "SECTION_CODE=?,SBU_CODE=?," +
				 "ASSET_USER=?,ASSET_ID=?,OLD_ASSET_ID=?,ASSET_STATUS=? " +
				 "WHERE ASSET_ID=? ";
...
try {
	...
	for (int i = 0; i < list.size(); i++) {
       ...
       if (asset_id1 == null || asset_id1.equals("")) {
        ...
        ps.setString(12, asset_id1);        
        
        ps.addBatch();
        
        String inventCode =approvalRec.getCodeName("SELECT ITEM_CODE FROM ST_STOCK WHERE Asset_id = ? ",asset_id1);
        String oldBranch =approvalRec.getCodeName("SELECT REQUESTBRANCH FROM am_ad_TransferRequisition WHERE Asset_id = ? ",asset_id1);
        String requestBranch =approvalRec.getCodeName("SELECT REQUESTBRANCH FROM am_ad_TransferRequisition WHERE Asset_id = ? ",reqnId);
        String branchCode =approvalRec.getCodeName("SELECT BRANCH_CODE FROM AM_AD_BRANCH WHERE BRANCH_ID = ? ",requestBranch);	
        String warehouseCode =approvalRec.getCodeName("SELECT WAREHOUSE_CODE FROM ST_WAREHOUSE WHERE BRANCH_CODE = ? ",branchCode);	
        String quantity =approvalRec.getCodeName("SELECT QUANTITY FROM am_ad_TransferRequisition WHERE Asset_id = ? ",reqnId);	
        String inventTotal =approvalRec.getCodeName("SELECT COUNT(*) FROM ST_INVENTORY_TOTALS WHERE COMP_CODE = ? AND WAREHOUSE_CODE = ? AND ITEM_CODE = ?",compCode,warehouseCode,inventCode);

 //       System.out.println("bulkStockTransfer oldBranch: "+oldBranch+"  requestBranch: "+requestBranch+"    branchCode: "+branchCode+"   warehouseCode: "+warehouseCode+"  quantity: "+quantity+"   inventTotal: "+inventTotal+"   inventCode: "+inventCode);
        if(inventTotal.equalsIgnoreCase("0")){
//        	System.out.print("<<<<<<========Inside Addition: ");
        	approvalMan.createInventoryTotal(compCode, inventCode, warehouseCode,  Integer.parseInt(quantity), Integer.parseInt(userid),Integer.parseInt(branchid));
 //       	approvalMan.updateInventoryTotalforTransfer(compCode, inventCode, warehouseCode,  Integer.parseInt(quantity),Integer.parseInt(oldBranch));
        		}
        		else{
 //       			System.out.print("<<<<<<========Inside Update: ");
 //       			approvalMan.updateInventoryTotal(compCode, inventCode, warehouseCode,  Integer.parseInt(quantity),Integer.parseInt(branchid));
        			approvalMan.updateInventoryTotalforTransfer(compCode, inventCode, warehouseCode,  Integer.parseInt(quantity),Integer.parseInt(oldBranch));
        			
        		}	        
        
//        System.out.println("bulkStockTransfer newmtid: "+newmtid);
    	String revalue_query = "update st_stocktransfer set approval_Status='ACTIVE' where Asset_Id = ? and approval_Status IS NULL ";
 //   	System.out.println("bulkStockTransfer revalue_query: "+revalue_query);
    	updateAssetStatusChange(revalue_query,newAsset_id);	 
//    	System.out.println("I AM DONE WITH TRANSFER ");
	...
	System.out.println("Error Updating Stock for Transfer ->" + ex);
} finally {
	closeConnection(con, ps);
}
mtidassetcode =mtid+String.valueOf(assetcode);
//System.out.println("bulkStockTransfer mtidassetcode: "+mtidassetcode);
 ...
 * *Bulk asset Transfer
 */
public boolean bulkStockTransfer(java.util.ArrayList list,String newAsset_id) {
 String query = "update ST_STOCK SET  " +
                   "BRANCH_ID=?,BRANCH_CODE=?,DEPT_ID=?," +
                   "DEPT_CODE=?,SECTION_ID=?," +
                   "SECTION_CODE=?,SBU_CODE=?," +
...
public String accountReturn(String assetId) {
    ...
    String query = "select c.category_name,(d.country_prefix + d.dr_prefix + b.branch_code + c.Dep_ledger) AS DRACCT,(d.country_prefix + "+
    		...
    		"am_ad_branch b,am_ad_category c,ST_STOCK a,am_gb_company d where a.category_code = c.category_code "+
    		"and a.Asset_id = 'FCMB/ASP2/FFF/0843' "+
 ...
 * *Bulk asset Transfer
 */
public String bulkStockTransferUpdate(java.util.ArrayList list) {
 String query = "update ST_STOCKtransfer SET  " +
				 "approval_Status=? " +
				 "WHERE transfer_id=? ";
System.out.println("bulkStockTransferUpdate query: "+query);
String mtidassetcode = "";
Connection con = null;
PreparedStatement ps = null;
ResultSet rs = null;
...
try {
  ...
  //      System.out.println("bulkStockTransferUpdate newmtid: "+newmtid+"   MTID: "+mtid);
...
public ArrayList findStockByQuery(String queryFilter) {
    ...
    String selectQuery_old =
            ...
            "RESIDUAL_VALUE,POSTING_DATE,RAISE_ENTRY,DEP_YTD,SECTION,BAR_CODE, " +
            "NBV,REMAINING_LIFE,TOTAL_LIFE,EFFECTIVE_DATE,REVALUE_COST, WAREHOUSE_CODE,ITEM_CODE,ItemType,UNIT_CODE,QUANTITY   " +
            "FROM ST_STOCK  WHERE ASSET_ID IS NOT NULL " + queryFilter;
 
  //  String selectQuery = "select *from ST_STOCK where Asset_id not in (select Asset_id from am_gb_bulkTransfer) and Asset_id !='' " +
 //   					queryFilter;
    String selectQuery = "select *from ST_STOCK where Asset_id !='' " +
			queryFilter;    		
    Connection con = null;
    PreparedStatement ps = null;
    System.out.println("selectQuery in findStockByQuery: " +selectQuery);
    ResultSet rs = null;
...
public ArrayList findStockAcceptedByQuery(String queryFilter) {
    //System.out.println("the value of filter is ]]]]]]]]]]]]]]]]]]" +queryFilter);
    double minCost =0;
    double maxCost =0;

    String selectQuery =
            "SELECT a.MTID,a.DORDER_CODE,a.QUANTITY_DELIVER,a.QUANTITY_REMAIN,a.REQNID,a.QUANTITY_REMAIN," +
            "a.QUANTITY_REQUEST,a.MEASURING_CODE, a.COMP_CODE,a.STOCK_CODE,b.BAR_CODE,a.QUANTITY_DELIVER," +
            "b.ITEMTYPE,b.WAREHOUSE_CODE,b.ITEM_CODE,b.DESCRIPTION,a.ACCEPT_REJECT,a.UNITPRICE,a.ADVANCE_PYMT " +
            "FROM ST_DISTRIBUTION_ITEM a, ST_STOCK b WHERE a.STOCK_CODE = b.Asset_id " + queryFilter;
// System.out.println("<<<<<<<selectQuery in findStockAcceptedByQuery: "+selectQuery);
    String selectQueryOld = "select *from ST_DISTRIBUTION_ITEM where STOCK_CODE in (select STOCK_CODE from ST_STOCK) and STOCK_CODE !='' AND ACCEPT_REJECT = 'ACCEPTED' " +
    		queryFilter; 		
    Connection con = null;
    PreparedStatement ps = null;
	...
	//		String posted = rs.getString("POSTED");
			String status = rs.getString("ACCEPT_REJECT");
            ...
            String barcode = rs.getString("BAR_CODE"); 
            String itemType = rs.getString("ITEMTYPE"); 
        DistributionOrderItem Dorder = new DistributionOrderItem(id,orderNo,quantity,unitPrice,0.0,
                                    advancePymt,itemCode,quantDeliver,quantRemain,warehouseCode);
        Dorder.setReqnID(reqnID);
        Dorder.setDesc(desc);
        Dorder.setStatus(status);
        Dorder.setMeasureCode(measureCode);
        Dorder.setCompCode(compCode);
        Dorder.setStockCode(stockCode);
        Dorder.setBarcode(barcode);
...
public ArrayList findStockByQueryList(String queryFilter) {
    //System.out.println("the value of filter is ]]]]]]]]]]]]]]]]]]" +queryFilter);
    double minCost =0;
    double maxCost =0;
  ...
  //  String selectQuery = "select *from ST_STOCK where Asset_id not in (select Asset_id from am_gb_bulkTransfer) and Asset_id !='' " +
 //   					queryFilter;
    String selectQuery = "select *from ST_STOCK where BAR_CODE in (select rfid_tag from ST_STOCK_DISTRBUTE) " +
			queryFilter;    		
    Connection con = null;
    PreparedStatement ps = null;
//    System.out.println("selectQuery in findStockByQuery: " +selectQuery);
...
/**
 * *Bulk stock Acceptance
 */
public String bulkStockAcceptance(java.util.ArrayList list,String reqnId,String compCode,String userid) {
 String query = "update ST_STOCK SET ASSET_STATUS=? " +
				 "WHERE ASSET_ID=? ";
 String query1 = "update ST_DISTRIBUTION_ITEM SET ACCEPT_REJECT=? " +
		 "WHERE STOCK_CODE=? ";
...
try {
	...
	for (int i = 0; i < list.size(); i++) {
        ...
        ps.setString(1, "ACTIVE");
        ps.setString(2, asset_id1);        
        
        ps.addBatch();
    	
        ps1.setString(1, "ACCEPTED");
        ps1.setString(2, asset_id1);        
	...
	System.out.println("Error Updating stock for Acceptance ->" + ex);
} finally {
	closeConnection(con, ps);
}
mtidassetcode =mtid+String.valueOf(assetcode);
...
public boolean updatebulkTransferRequisitionStatus(String groupID) throws Exception {
	String []batchId = groupID.split("-");
        /*String query = "update am_asset_approval set  asset_status = ?, process_status=?, " +
        		"super_id =?, approval1 =? where asset_id =?";*/
        String query = "update am_ad_TransferRequisition set  PROCESS_STATUS = ?, " +
		" where Batch_id =?";
         boolean done = true;
        Connection con = null;
        PreparedStatement ps = null;
        ...
        try {
        	...
        	System.out.println("updatebulkTransferRequisitionStatus batch Id 0: "+batchId[0]+"   Batch Id 1: "+batchId[0]);

            con = dbConnection.getConnection("legendPlus");
            ps = con.prepareStatement(query);
            ps.setString(1, "P");
            ps.setInt(2, Integer.parseInt(batchId[0]));
            ps.execute();

        } catch (Exception ex) {
            done = false;
            System.out.println("StockRecordsBean: updatebulkTransferRequisitionStatus: WARN:Error updating stock->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
        return done;
...
public String[] setApprovalDataAsset(String id){
...
public void setPendingTrans(String[] a, String code,int assetCode,String supervisor){
...
//          System.out.println(code);
        ...
        ps.setString(1, (a[0]==null)?"":a[0]);
        ps.setString(2, (a[1]==null)?"":a[1]);
        ps.setString(3, (a[2]==null)?"":supervisor);
        ps.setDouble(4, (a[3]==null)?0:Double.parseDouble(a[3]));
        //ps.setDate(5, (a[4])==null?null:dbConnection.dateConvert(a[4]));
        ps.setTimestamp(5,dbConnection.getDateTime(new java.util.Date()));
        ps.setString(6, (a[5]==null)?"":a[5]);
...
public void setPendingTransFT(String[] a, String code,int assetCode,String supervisor){
...
public void incrementApprovalCount3(int tran_id,int count,int nextSupervisor){
String query_r ="update am_asset_approval set approval_level_count=?,posting_date=?,process_status='A' where transaction_id =? and super_id=?";
...
public int insertStockRecord(String oldAssetId) throws Exception, Throwable {
    String[] budget = getBudgetInfo();
    double[] bugdetvalues = getBudgetValues();
    int DONE = 0; //everything is oK
    int BUDGETENFORCED = 1; //EF budget = Yes, CF = NO, ERROR_FLAG
    ...
    String Q = getQuarter();
    if(budget[3].equalsIgnoreCase("N")){
//    	System.out.println("ABOUT TO EXECUTE rinsertStockRecord");
		rinsertStockRecord(oldAssetId);
		return DONE;
	}
	else if(budget[3].equalsIgnoreCase("Y")){
    if (!Q.equalsIgnoreCase("NQ")) {
 ...
 * @return boolean
 */
private boolean rinsertStockRecord(String oldAssetId) throws Exception, Throwable {
    asset_id = new legend.AutoIDSetup().getIdentity(branch_id,
            department_id, section_id, category_id);
//    System.out.println("<<<<<<<<<<<INSIDE rinsertStockRecord>>>>>> category_id: "+category_id);
    Connection con = null;
    PreparedStatement ps = null;
    boolean done = true;
AssetPaymentManager payment = null;
    ...
    amountPTD = amountPTD.replaceAll(",","");
    assetCode = Integer.parseInt(new ApplicationHelper().getGeneratedId("ST_STOCK"));
//    System.out.println("<<<<<<rinsertStockRecord assetCode: "+assetCode+"     bar_code: "+bar_code+"    parentTag: "+parentTag+"    category_id: "+category_id);
    String createQuery = "INSERT INTO ST_STOCK_USED         " +
                         "(" +
                         "ASSET_ID, REGISTRATION_NO, BRANCH_ID, DEPT_ID," +
                         "SECTION_ID, CATEGORY_ID, [DESCRIPTION], VENDOR_AC," +
                         "DATE_PURCHASED, DEP_RATE, ASSET_MAKE, ASSET_MODEL," +
                         ...
                         "AMOUNT_REM,PART_PAY,FULLY_PAID,BAR_CODE,SBU_CODE,LPO,supervisor,defer_pay,wht_percent," +
                         "system_ip,mac_address,asset_code,memo,memoValue, SPARE_3, SPARE_4, SPARE_5, SPARE_6,sub_category_id,SUB_CATEGORY_CODE, " +
                         "WAREHOUSE_CODE,ITEMTYPE, ITEM_CODE,QUANTITY,UNIT_CODE,PARENT_TAG,OLD_ASSET_ID)  "+
                         "VALUES" +
                         "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
                         "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
//                           Double.parseDouble(vatable_cost);
        ...
        ps.setInt(5, Integer.parseInt(section_id));
        ps.setInt(6, Integer.parseInt(category_id));
        System.out.println("<<<<<<rinsertStockRecord description: "+description);
        ps.setString(7, description);
        ps.setString(8, vendor_account);
        ps.setDate(9, dbConnection.dateConvert(date_of_purchase));
        ps.setString(10, getDepreciationRate(category_id));
...
//          System.out.println("<<<<<<rinsertAssetRecord bar_code: "+bar_code+"     bar_code Length: "+bar_code.length());
        ...
        ps.setString(86, unitCode);
        ps.setString(87, parentTag);
        ps.setString(88, oldAssetId);
 //       System.out.println("<<<<<warehouseCode in rinsertStockRecord: "+warehouseCode+"    itemType: "+itemType+"    asset_id: "+asset_id);
        ps.execute();  

    } catch (Exception ex) {
        done = false;
        System.out.println("WARN:Error creating asset in ST_STOCK_USED Table->" + ex);
    } finally {
        dbConnection.closeConnection(con, ps);
    }
...
public boolean updateNewStockStatux(String assetId) throws Exception {

    String query = "update ST_STOCK_USED SET  asset_status = 'APPROVED' where asset_id ='" +assetId+"'";
     boolean done = true;
    Connection con = null;
    PreparedStatement ps = null;
    ...
    try {
        ...
        System.out.println("StockRecordsBean: updateNewStockStatux(): WARN:Error updating asset->" + ex);
    } finally {
        dbConnection.closeConnection(con, ps);
    }
    return done;
 ...
 * getStockRecords
 */
public void getStockRecords() throws Exception {
    ...
    if (asset_id != "") {
        String query = "SELECT A.ASSET_ID, A.REGISTRATION_NO," +
                       ...
                       "A.LPO,A.SUPERVISOR,A.defer_pay,A.wht_percent,A.asset_code,A.memo,A.memovalue, " +
                       "A.IMPROV_NBV,A.IMPROV_ACCUMDEP,A.IMPROV_COST,A.IMPROV_MONTHLYDEP,A.TOTAL_NBV, " +
                       "A.WAREHOUSE_CODE, A.ITEM_CODE,A.ITEMTYPE,A.QUANTITY,A.UNIT_CODE,A.PARENT_TAG FROM ST_STOCK_USED A " +
                       "WHERE A.ASSET_ID = '" + asset_id + "'";
//		System.out.println("getAssetRecords query: "+query);
        Connection con = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            System.out.println("WARN: Error fetching all asset in getStockRecords->" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
    }
 ...
 * *Bulk Stock Issuance
 */
public String bulkStockIssuance(java.util.ArrayList list,String reqnId,String compCode,String userid,String warehouseCode,String requisitionId) {
/* String query = "update ST_STOCK SET  " +
				 "BRANCH_ID=?,BRANCH_CODE=?,DEPT_ID=?," +
				 "DEPT_CODE=?,SECTION_ID=?," +
				 "SECTION_CODE=?,SBU_CODE=?," +
...
//	System.out.print("<<<<<<========list.size(): "+list.size());
	for (int i = 0; i < list.size(); i++) {
        ...
        String branchid = bd.getNewbranch_id();
        String requestBranch = bd.getNewbranch_id(); 
        String branchcode = bd.getBranch_Code();
        String deptid = bd.getNewdepartment_id();
        String deptcode = bd.getDepartment_code();
        String sectionid = bd.getNewsection_id();
        ...
        String asset_id1 = bd.getAsset_id();
        String newAsset_id = bd.getNewasset_id();
        String quantityIssue = bd.getQuantityIssue();
        String requestedquantity = bd.getRequestedQuantity();
//        System.out.println("<<<<<<bd.getRequestedQuantity(): "+bd.getRequestedQuantity());
        int quantityavailable = bd.getQtyAvailable();  
//        System.out.println("<<<<<<bd.getQtyAvailable(): "+bd.getQtyAvailable());
        assetcode = bd.getAssetCode();
        String assetId = bd.getAsset_id();
        String transactId = bd.getEngine_number();
        String newmtid = transactId+String.valueOf(assetcode);

        String customerCode = bd.getCustomerCode();
        String description = bd.getDescription();
...
//       System.out.println("<<<<<<<=====newAsset_id: "+newAsset_id+"    asset_id1: "+asset_id1);
       String userBranch =approvalRec.getCodeName("SELECT BRANCH FROM am_gb_User WHERE USER_ID = ? ",userid);
        String inventCode =approvalRec.getCodeName("SELECT ITEM_CODE FROM ST_STOCK WHERE Asset_id = ? ",asset_id1);
        String oldBranch =approvalRec.getCodeName("SELECT REQUESTBRANCH FROM am_ad_TransferRequisition WHERE Asset_id = ? ",asset_id1);
        String oldAssetId =approvalRec.getCodeName("SELECT OLD_ASSET_ID FROM ST_STOCK WHERE Asset_id = ? ",asset_id1);
        String branchCode =approvalRec.getCodeName("SELECT BRANCH_CODE FROM AM_AD_BRANCH WHERE BRANCH_ID = ? ",requestBranch);	
        String oldbranchCode =approvalRec.getCodeName("SELECT BRANCH_CODE FROM AM_AD_BRANCH WHERE BRANCH_ID = ? ",userBranch);	
//        String warehouseCode =approvalRec.getCodeName("SELECT WAREHOUSE_CODE FROM ST_WAREHOUSE WHERE BRANCH_CODE = '"+branchWarehouse+"' ");	
//        String OldwarehouseCode =approvalRec.getCodeName("SELECT WAREHOUSE_CODE FROM ST_WAREHOUSE WHERE BRANCH_CODE = '"+branchWarehouse+"' ");	
        String unitCost =approvalRec.getCodeName("SELECT UNIT_PRICE FROM ST_STOCK WHERE Asset_id = ? ",asset_id1);
        int prevqtyBalance =Integer.parseInt(approvalRec.getCodeName("SELECT QUANTITY FROM ST_STOCK WHERE Asset_id = ? ",asset_id1));
        String inventTotal =approvalRec.getCodeName("SELECT COUNT(*) FROM ST_INVENTORY_TOTALS WHERE COMP_CODE = ? AND WAREHOUSE_CODE = ? AND ITEM_CODE = ?",compCode,warehouseCode,inventCode);
        String oldinventTotal =approvalRec.getCodeName("SELECT COUNT(*) FROM ST_INVENTORY_TOTALS WHERE COMP_CODE = ? AND WAREHOUSE_CODE = ? AND ITEM_CODE = ?",compCode,warehouseCode,inventCode);
        
        String shipdate = sdf.format(dbConnection.getDateTime(new java.util.Date()));
        
//        System.out.println("bulkStockTransfer oldBranch Before Defaulting to One: "+oldBranch+"  requestBranch: "+requestBranch+"    branchCode: "+branchCode+"   warehouseCode: "+warehouseCode+"  quantityIssue: "+quantityIssue+"   inventTotal: "+inventTotal+"   inventCode: "+inventCode+"  oldAssetId: "+oldAssetId+"   userBranch: "+userBranch+"   OldwarehouseCode: "+OldwarehouseCode);
   //     quantityIssue = "1";
   //     requestedquantity = "1";
//    	System.out.println("<<<<<<<<<<<<<inventTotal===: "+inventTotal+"   prevqtyBalance: "+prevqtyBalance);
        if(inventTotal.equalsIgnoreCase("0")){
//        	System.out.print("<<<<<<========Inside New Addition: ");
//        	System.out.println("<<<<<<<<<<<<<compCode===: "+compCode+"  inventCode: "+inventCode+"   warehouseCode: "+warehouseCode+"  quantityIssue: "+quantityIssue+"  userid: "+userid+"  branchId: "+branchid+"  requestedquantity: "+requestedquantity);
        	approvalMan.createInventoryTotal(compCode, inventCode, warehouseCode,  Integer.parseInt(quantityIssue), Integer.parseInt(userid),Integer.parseInt(branchid));
        	approvalMan.insertHistoryTableforIssuance(compCode,inventCode,warehouseCode,Integer.parseInt(quantityIssue),Integer.parseInt(userid),projectCode,prevqtyBalance);
        		}
        		else{
//        			System.out.print("<<<<<<========Inside  New Update: ");
//        			approvalMan.updateInventoryTotal(compCode, inventCode, warehouseCode,  Integer.parseInt(quantityIssue),Integer.parseInt(branchid));
        			approvalMan.updateInventoryTotalforIssuance(compCode, inventCode, warehouseCode,  Integer.parseInt(quantityIssue),Integer.parseInt(branchid));
        			approvalMan.insertHistoryTableforIssuance(compCode,inventCode,warehouseCode,Integer.parseInt(quantityIssue),Integer.parseInt(userid),projectCode,prevqtyBalance);
        		}	       
        /*
        if(oldinventTotal.equalsIgnoreCase("0")){
        	System.out.print("<<<<<<========Inside Old Addition: ");
        	System.out.println("<<<<<<<<<<<<<compCode in OldInventoryTotal: "+compCode+"  inventCode: "+inventCode+"   warehouseCode: "+warehouseCode+"  quantityIssue: "+quantityIssue+"  userid: "+userid+"  branchId: "+branchid+"   requestedquantity: "+requestedquantity);
        	approvalMan.createInventoryTotalSubtraction(compCode, inventCode, OldwarehouseCode,  Integer.parseInt(quantityIssue), Integer.parseInt(userid),Integer.parseInt(userBranch));
        		}
        		else{
        			System.out.print("<<<<<<========Inside Old Update: ");
//        			approvalMan.updateInventoryTotal(compCode, inventCode, warehouseCode,  Integer.parseInt(quantityIssue),Integer.parseInt(branchid));
        			approvalMan.updateInventoryTotalforSubtraction(compCode, inventCode, OldwarehouseCode,  Integer.parseInt(quantityIssue),Integer.parseInt(userBranch));
        			
        		}	                          
        */
//******************************************************
//        System.out.println("<<<<<<<<<<<<<dstrbCode: "+dstrbCode+"  customerCode: "+customerCode+"   meansOfId: "+meansOfId+"  shipdate: "+shipdate+"  userid: "+userid+"  projectName: "+projectName+"   unitCost: "+unitCost+"   requestedquantity: "+requestedquantity+"   quantityIssue: "+quantityIssue+"  unitCode: "+unitCode+"  quantityIssue: "+quantityIssue+"  itemCode: "+itemCode+"  reqnId: "+reqnId+"  compCode: "+compCode+"   projectCode: "+projectCode+"   requisitionId: "+requisitionId);
    	if(distributionBus.createDistributionOrder(requisitionId,dstrbCode,customerCode,"",meansOfId,shipdate,
    			   "","",projectName,"", Integer.parseInt(userid),0,projectCode,compCode,projectCode,itemCode,Integer.parseInt(quantityIssue),
    			   Double.parseDouble(unitCost),(Integer.parseInt(requestedquantity)-Integer.parseInt(quantityIssue)),unitCode))
    		   {
//    		System.out.println("I AM ABOUT TO DISTRIBUTE ");  
...
//            {   //  System.out.println("<<<<<<<=======stockId[j]: "+stockId[j]);
     //    	   String rfidtag = serviceBus.getCodeName("SELECT BAR_CODE FROM ST_STOCK WHERE ASSET_ID= '"+stockId[j]+"' ");
         	   String quantvalue = approvalRec.getCodeName("SELECT QUANTITY FROM ST_STOCK WHERE ASSET_ID= ? ",asset_id1);
//         	  System.out.println("<<<<<<<=======quantvalue>>>>>>>:  "+quantvalue);
         	   int quantityval = Integer.parseInt(quantvalue);
//         	  System.out.println("<<<<<<<=======quantityval>>>>>>>:  "+quantityval);
         	   if(distributionBus.createDistributionOrderItemDetailForBulk(dstrbCode,Integer.parseInt(requestedquantity),Integer.parseInt(quantityIssue),Double.parseDouble(unitCost),0.00,
     		   itemCode,warehouseCode,compCode,projectCode,Integer.parseInt(userid),description,asset_id1,(Integer.parseInt(requestedquantity)-Integer.parseInt(quantityIssue)),unitCode,"",projectCode)){
    		  	//	distributionBus.updateStockQry(stockId[j]);
         		   if(quantityval == quantity){distributionBus.updateStockQry(asset_id1);}
            }	
//*********************************************************            
        }
//	d = ps.executeBatch();
	//System.out.println("Executed Successfully ");
    	approvalMan.updateStockQuantityforIssuance(assetId,Integer.parseInt(quantityIssue));

	}
} catch (Exception ex) {
	System.out.println("Error Updating Stock for Transfer in bulkStockIssuance->" + ex);
} finally {
	closeConnection(con, ps);
}
//mtidassetcode =mtid+String.valueOf(assetcode);
...
public String setPendingTrans3(String[] a, String code,String requestedby){
//	System.out.println("====code 1====> "+code);
    String mtid_id ="";
        int transaction_level=0;
        Connection con;
...
//		 if(No>1){batchId = new ApplicationHelper().getGeneratedId("am_asset_approval");}
 String pq = "insert into am_asset_approval(asset_id,user_id,super_id,amount,posting_date,description," +
         "effective_date,branchCode,asset_status,tran_type, process_status,tran_sent_time,transaction_id,batch_id," +
         "transaction_level,requested_by) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 String tranLevelQuery = "select level from approval_level_setup where code ='"+code+"'";
        con = null;
        ps = null;
        ps1 = null;
        ...
        try
        {
            ...
            for(int j=0;j<No;j++){
            String mtid =  new ApplicationHelper().getGeneratedId("am_asset_approval");
//            System.out.println("====asset id====> "+reqnID+"    mtid: "+mtid);
            ps.setString(1, mtid);
            ps.setString(2, (a[1]==null)?"":a[1]);
            ps.setString(3, (a[2]==null)?"":spuervisorlist[j]);
            ps.setDouble(4, (a[3]==null)?0:Double.parseDouble(a[3]));
            ...
            ps.setString(14, mtid);
            ps.setInt(15, transaction_level);
            ps.setInt(16, Integer.parseInt(requestedby));
    //        System.out.println("====transaction level====> "+transaction_level);
            ps.execute();
...
public String setPendingTransStore(String[] a, String code,String requestedby){
//	System.out.println("====code 1====> "+code);
    String mtid_id ="";
        int transaction_level=0;
        Connection con;
...
public String setPendingTrans4(String[] a, String code,String requestedby){
//	System.out.println("====code 1====> "+code);
    String mtid_id ="";
        int transaction_level=0;
        Connection con;
...
return mtid_id;
	...
	public String checkAssetLedgerStoreDrAccount(String category,String branch)
	{
		String assetledgeraccount="";
		System.out.println("category "+category);
		System.out.println("branch "+branch);
		String query=" select c.iso_code,"
					+" (select accronym from am_ad_ledger_type where series = substring((select distinct INVENTORY_ACCT from ST_INVENTORY_ITEMS),1,1)),"
					+" ?,"
					+" (select distinct COG_SOLD from ST_INVENTORY_ITEMS),"
					+" c.iso_code +(select accronym from am_ad_ledger_type where series = substring((select distinct INVENTORY_ACCT from ST_INVENTORY_ITEMS),1,1))+"
					+" ? + (select distinct COG_SOLD from ST_INVENTORY_ITEMS) asd"
					+" from am_ad_branch d, "
					+" AM_GB_CURRENCY_CODE c, am_gb_company b"
					+" where d.branch_code = ?  and  c.iso_code = 'NGN'";
//		System.out.println("query in checkAssetLedgerStoreDrAccount >>>> " + query);
	...
	public String checkAssetLedgerStoreCrAccount(String category,String branch)
	{
		String assetledgeraccount="";
//		System.out.println("category "+category);
//		System.out.println("branch "+branch);
		String query=" select c.iso_code,"
					+" (select accronym from am_ad_ledger_type where series = substring((select distinct INVENTORY_ACCT from ST_INVENTORY_ITEMS),1,1)),"
					+" ?,"
					+" (select distinct INVENTORY_ACCT from ST_INVENTORY_ITEMS),"
					+" c.iso_code +(select accronym from am_ad_ledger_type where series = substring((select distinct INVENTORY_ACCT from ST_INVENTORY_ITEMS),1,1))+"
					+" ? + (select distinct INVENTORY_ACCT from ST_INVENTORY_ITEMS) asd"
					+" from am_ad_branch d, "
					+" AM_GB_CURRENCY_CODE c, am_gb_company b"
					+" where d.branch_code = ?  and  c.iso_code = 'NGN'";
//		System.out.println("query in checkAssetLedgerStoreCrAccount >>>> " + query);
     ...
     * getAssetRecords
     */
    public void getGroupStockRecords() throws Exception {

        if (asset_id != "") {
    		String query = "SELECT group_id, quantity, Registration_No, "
    				...
    				+ "spare_2,spare_3,spare_4,spare_5,spare_6, user_ID, province, WAR_START_DATE, WAR_MONTH, "
    				+ "WAR_EXPIRY_DATE, branch_code, dept_code, section_code, "
    				+ "category_code,AMOUNT_PTD,AMOUNT_REM,PART_PAY,FULLY_PAID,PROJECT_CODE "
    				+ " FROM am_group_stock_main WHERE GROUP_ID = '" + asset_id + "'";            
//			System.out.println("getAssetRecords query: "+query);
            Connection con = null;
            PreparedStatement ps = null;
            ResultSet rs = null;
...
//                    assetCode = rs.getInt("asset_code");
                ...
                System.out.println("WARN: Error fetching all asset in getGroupStockRecords->" + ex);
            } finally {
                dbConnection.closeConnection(con, ps);
            }
        }
    ...
    public ArrayList findStockForIssue(String query_)
    {
       // System.out.println("in method and the query is ............." +query_);
  
  
        String SELECT_QUERY = "SELECT DISTINCT a.BATCH_ID,a.REQUEST_HEADING,a.REQUESTBRANCH,a.REQNCOLLECTOR,a.REQUESTSECT_ID,a.SBU_CODE,CONVERT(VARCHAR(10),a.TRANSFER_DATE,101) AS TRANSFER_DATE " +
        		"FROM am_ad_TransferRequisition a, am_asset_approval b WHERE a.BATCH_ID = b.BATCH_ID "+
        		"AND a.PROCESS_STATUS = 'A' AND b.PROCESS_STATUS = 'A'" +query_;

//        System.out.println("SELECT_QUERY >>>>>in findStockForIssue>>>>>> " + SELECT_QUERY);  
...
//				String description = rs.getString("Description");
				String requestHeader = rs.getString("REQUEST_HEADING");
				String requestBranchId = rs.getString("REQUESTBRANCH");
				String requestCollector = rs.getString("REQNCOLLECTOR");
				String requestSectionId = rs.getString("REQUESTSECT_ID");
				String sbuCode = rs.getString("SBU_CODE");
	//			String postingDate = (rs.getString("TRANSFER_DATE")==null)?"":formatDate(rs.getDate("TRANSFER_DATE"));
				...
				String newpostingDate = yyyy+"-"+mm+"-"+dd;
				tran = new Asset();
				tran.setAssetId(batchId);
//				tran.setDescription(description)
				if(requestBranchId==null){requestBranchId = "0";}
				tran.setBranchId(Integer.parseInt(requestBranchId));
				tran.setAssetUser(requestCollector);
				if(requestSectionId==null){requestSectionId = "0";}
				tran.setSectionId(Integer.parseInt(requestSectionId));
				tran.setModel(sbuCode);
				tran.setPostingDate(postingDate);
				tran.setRequestHeader(requestHeader);
				list.add(tran);
			}

		} catch (Exception e) {
			String warning = "WARNING:Error Fetching Transaction from findStockForIssue"
					+ " ->" + e.getMessage();
			System.out.println(warning);
		} finally {
			closeConnection(con, ps, rs);
