...
public class ExcelAssetUpdateBean extends ConnectionClass
{
        ...
        throws Exception
    {
        ...
        String query = "INSERT INTO AM_GROUP_ASSET(REGISTRATION_NO,BRANCH_ID,DEPT_ID,CATEGORY_ID,SECTION" +
...
"DE,SECTION_CODE,CATEGORY_CODE,GROUP_ID,AMOUNT_PTD,AMOUNT_REM,PART_PAY,FULLY_PAID,SBU_CODE" +
" )\tVALUES('"
;
        b.append(query);
    ...
    throws Exception
{
    ...
    String query = "INSERT INTO AM_GROUP_ASSET(ASSET_ID,REGISTRATION_NO,BRANCH_ID,DEPT_ID,CATEGORY_ID,SECTION" +
"_ID,DESCRIPTION,VENDOR_AC,DATE_PURCHASED,DEP_RATE,ASSET_MAKE,ASSET_MODEL,ASSET_S" +
"ERIAL_NO,ASSET_ENGINE_NO,SUPPLIER_NAME,ASSET_USER,ASSET_MAINTENANCE,COST_PRICE,D" +
"EP_END_DATE,RESIDUAL_VALUE,AUTHORIZED_BY,WH_TAX,WH_TAX_AMOUNT,POSTING_DATE,EFFEC" +
"TIVE_DATE,PURCHASE_REASON,LOCATION,VATABLE_COST,VAT,REQ_DEPRECIATION,SUBJECT_TO_" +
"VAT,WHO_TO_REM,EMAIL1,WHO_TO_REM_2,EMAIL2,STATE,DRIVER,SPARE_1,SPARE_2,[US" +
"ER_ID], PROVINCE, WAR_START_DATE, WAR_MONTH, WAR_EXPIRY_DATE,BRANCH_CODE,DEPT_CO" +
"DE,SECTION_CODE,CATEGORY_CODE,GROUP_ID,AMOUNT_PTD,AMOUNT_REM,PART_PAY,FULLY_PAID,PROCESS_FLAG,POST_FLAG" +
" )VALUES('"
;
    b.append(query);
    b.append(Assetid);
    ...
    b.append(fullyPAID);
    b.append("','");   
    b.append(processflag);
    b.append("','");
    b.append(postflag);
    b.append("')");
    System.out.println("=====createGroupUpload query=====> "+query);
...
public void createGroupUpload(String asset_id, String gid)
throws Exception
{
	Connection con = null;
...
"PROCESS_FLAG,POST_FLAG,REQ_REDISTRIBUTION,RAISE_ENTRY,ACCUM_DEP,DEP_YTD,ASSET_STATUS,SBU_CODE" +
" )VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    throws Exception, Throwable
{
    ...
    if(budget[3].equalsIgnoreCase("N"))
    {
        createGroupUpload(asset_id,gid);
        value = DONE;
    } else
    if(budget[3].equalsIgnoreCase("Y"))
    {
...
"GORY]='"
    ...
    public void updateBudget(String quarter, String bugdetinfo[])
    {
        ...
        int finomonth = 0;
        String fiedate = "";
        conn = dbConnection.getConnection("fixedasset");
        stmt = null;
        try
        {
    ...
    throws Exception, Throwable
{
    ...
    if(budget[3].equalsIgnoreCase("N"))
    {
    	createUncapitalizedGroupUpload(asset_id,gid);
        value = DONE;
    } else
    if(budget[3].equalsIgnoreCase("Y"))
    {
    ...
    public void createUncapitalizedGroupUpload(String asset_id, String gid)
    throws Exception
    {
    	Connection con = null;
    	PreparedStatement ps = null;
...
//    	POSTING_DATE
    	ps.setString(25, DateManipulations.CalendarToDb(depreciation_start_date));
    	System.out.println("=====posting_date=====> "+depreciation_start_date);
//    	EFFECTIVE_DATE
    	ps.setString(26, reason);
//    	PURCHASE_REASON 
    	ps.setInt(27, Integer.parseInt(location));
...
//    	WAR_MONTH
    	System.out.println("=====noOfMonths=====> "+noOfMonths);
    	System.out.println("=====Before expiryDate=====> "+sdf.format(new java.util.Date()));
    	ps.setString(44, DateManipulations.CalendarToDb(expiryDate));
    	System.out.println("=====expiryDate=====> "+expiryDate);
//    	WAR_EXPIRY_DATE
    	ps.setString(45, branch_code);
...
//    	dep_ytd
    ...
    public boolean updateBulkAsset(ExcelAssetUpdateBean asset) {
      ...
      //  ExcelAssetUpdateBean asset = null;
        ...
        PreparedStatement ps = null;
        htmlUtil = new HtmlUtility();
//        System.out.println("Inside updateBulkAsset");
        int[] d =null;
/*        String query= "UPDATE AM_ASSET SET REGISTRATION_NO=?," +
        ...
        String query= "UPDATE AM_ASSET SET Registration_No=?,DESCRIPTION=?," +
        " Asset_Serial_No=?,Asset_Engine_No=?, BAR_CODE=?," +
        " ASSET_USER = ?, ASSET_MAKE = ?,ASSET_MODEL = ?,PURCHASE_REASON = ?,SPARE_1 = ?," +
        " SPARE_2 = ? WHERE ASSET_ID=?" ;
//        System.out.println("query: "+query);
        try {
             con = dbConnection.getConnection("fixedasset");
      ...
      //       for (int i = 0; i < list.size(); ++i) {
       // asset = (Asset)list.get(i);
           //  asset = new ExcelAssetUpdateBean();
                String assetId = asset.getAsset_id();
                System.out.println("assetId: "+assetId);
                String registrationNo = asset.getRegistration_no();
                if(registrationNo.equalsIgnoreCase(" ")){registrationNo =  htmlUtil.findObject("SELECT Registration_No FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}
...
//                String assetModel = asset.getAssetMake();
                String AssetSerialNo = asset.getSerial_no();
                System.out.println("AssetSerialNo 1: "+AssetSerialNo);
                if(AssetSerialNo.equalsIgnoreCase(" ")){AssetSerialNo =  htmlUtil.findObject("SELECT Asset_Serial_No FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}
                System.out.println("AssetSerialNo 2: "+AssetSerialNo);
                String AssetEngineNo = asset.getEngine_number();
                if(AssetEngineNo.equalsIgnoreCase(" ")){AssetEngineNo =  htmlUtil.findObject("SELECT Asset_Engine_No FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}                
                String barCode = asset.getBar_code();
                System.out.println("barCode 1: "+barCode);
                if(barCode.equalsIgnoreCase(" ")){barCode =  htmlUtil.findObject("SELECT BAR_CODE FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}
                System.out.println("barCode 2: "+barCode);
//                String lpo_no = asset.getLpo_no();
//                int SupplierName = asset.getSupplierName();
                String assetUser = asset.getUser();
                if(assetUser.equalsIgnoreCase(" ")){assetUser =  htmlUtil.findObject("SELECT Asset_User FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}
                String model = asset.getModel();
                if(model.equalsIgnoreCase(" ")){model =  htmlUtil.findObject("SELECT Asset_Model FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}
                String purchaseReason = asset.getReason();
                String make = asset.getMake();
                if(!make.equalsIgnoreCase(" ")){make = htmlUtil.findObject("SELECT AssetMake_ID FROM am_gb_assetMake  WHERE AssetMake = '"+make+"'");}
                if((make.equalsIgnoreCase(" "))||(make.equalsIgnoreCase("UNKNOWN"))){make =  htmlUtil.findObject("SELECT Asset_Make FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}
                String spare1 = asset.getSpare_1();
                if(spare1.equalsIgnoreCase(" ")){spare1 =  htmlUtil.findObject("SELECT SPARE_1 FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}                
                String spare2 = asset.getSpare_2();
                if(spare2.equalsIgnoreCase(" ")){spare2 =  htmlUtil.findObject("SELECT SPARE_2 FROM AM_ASSET  WHERE asset_id = '"+assetId.trim()+"'");}                
//                 System.out.println("the asset id in Excel update manager is >>>>>>>>>>> " + assetId);
                asset=null;
 /*               System.out.println("registrationNo: "+registrationNo); 
                System.out.println("description: "+description);
                System.out.println("AssetSerialNo: "+AssetSerialNo);
                System.out.println("AssetEngineNo: "+AssetEngineNo);
	...
	//             ps.setString(5,lpo_no);             
	             ...
	             ps.setString(6,assetUser);             
	             ps.setString(7,make);             
	             ps.setString(8,model);             
	             ps.setString(9,purchaseReason);             
	             ps.setString(10,spare1);             
	             ps.setString(11,spare2);             
           ...
           //  }
            ...
            System.out.println("AssetExcelUpdateManager: ExcelAssetUpdate()>>>>>" + ex);
        } finally {
            dbConnection.closeConnection(con, ps);
        }
            return (d.length > 0);
