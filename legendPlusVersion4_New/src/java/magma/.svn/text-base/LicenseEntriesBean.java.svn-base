package magma;

/**
 * <p>Title: Magma.net System</p>
 *
 * <p>Description: Fixed Assets Manager</p>
 *
 * <p>Copyright: Copyright (c) 2006. All rights reserved.</p>
 *
 * <p>Company: Magbel Technologies Limited.</p>
 *
 * @author Charles Ayoola Ayodele-Peters.
 * @version 1.0
 */
public class LicenseEntriesBean {
    public LicenseEntriesBean() throws Exception {
    }

    /**
     * insertLicenseEntry
     *
     * @return boolean
     */
    public boolean insertLicenseEntry() {
        return false;
    }

    /**
     * updateLicenseEntry
     *
     * @return boolean
     */
    public boolean updateLicenseEntry() {
        return false;
    }

    /**
     * getLicenseEntries
     *
     * @return String[]
     */
    public String[] getLicenseEntries() {
        return null;
    }
}
