...
public class ExcelAssetDisposalBean extends ConnectionClass
{
        ...
        throws Exception
    {
        ...
        supplied_by = "0";
        registration_no = "";
        disposalDate = new GregorianCalendar();
...
" )\tVALUES('";
        ...
        b.append(costPrice);
        b.append(",");
        b.append(disposalReason);
        b.append(",");
        b.append(DateManipulations.CalendarToDb(disposalDate));
        b.append(",'");
        b.append(user_id);
        b.append("','");
        b.append("N");
        ...
        b.append(Double.parseDouble(oldnbv));
        b.append(",");
        b.append(Double.parseDouble(disposalAmount));
        b.append(",");
...
b = new StringBuffer(400);
Codes code = new Codes();
double vatAmount = Double.parseDouble(disposalAmount)*(Double.parseDouble(vat_amount)/100);
double whtaxamount = Double.parseDouble(disposalAmount)*(Double.parseDouble(wh_tax_amount)/100);
if(no_of_items == null || no_of_items.equals(""))
{
    no_of_items = "0";
}
...
String approvalStatus = "PENDING";
String disposed = "Y";
double  costPrice = Double.parseDouble(disposalAmount);
if(subject_to_vat.equals("Y")){costPrice = Double.parseDouble(disposalAmount)+vatAmount;}
...
String query = "INSERT INTO AM_GROUP_DISPOSAL(disposal_ID,asset_id,disposalCost,disposal_reason,Disposal_Date," +
"User_ID,raise_entry,buyer_ac,disposalAmount,vat_amount,wht_amount,Cost_Price,accum_dep,nbv," +
...
try {   
	...
	ps.setString(4, disposalReason);
	ps.setString(5, DateManipulations.CalendarToDb(disposalDate));
	ps.setInt(6, Integer.parseInt(user_id));
	ps.setString(7, "N");
	ps.setString(8, vendor_account);
	ps.setDouble(9, Double.parseDouble(disposalAmount)); 
	ps.setDouble(10, vatAmount); 
	ps.setDouble(11, whtaxamount);
	ps.setDouble(12, Double.parseDouble(oldcost_price));
	ps.setDouble(13, Double.parseDouble(oldaccum_dep)); 
	ps.setDouble(14, Double.parseDouble(oldnbv)); 
	ps.setString(15, DateManipulations.CalendarToDb(effectiveDate));
	ps.setString(16, approvalStatus);
	ps.setDouble(17, Double.parseDouble(wh_tax_amount)); 
	ps.setString(18, wh_tax);
	ps.setString(19, subject_to_vat);
	ps.setString(20, branch_code);
	ps.setString(21, category_code);
	ps.setString(22, description);
	ps.setString(23, lpo_no);
	ps.setString(24, invoice_No);
	ps.setString(25, asset_code);
	ps.setString(26, integrifyId);
	ps.setString(27, disposed);
	ps.setString(28, projectCode);
	ps.setString(29, location);
	ps.setString(30, sbu_code);
...
System.out.println(this.getClass().getName()
    ...
    public void setDisposalDate(String disposalDate)
    {
        ...
        {
            this.disposalDate = DateManipulations.DateToCalendar(disposalDate);
        }   
    }
    ...
    public void setEffectiveDate(String effectiveDate)
    {
        ...
        {
            this.effectiveDate = DateManipulations.DateToCalendar(effectiveDate);
        }
    }
    ...
    public void setDisposalAmount(String disposalAmount)
    {
        ...
        {
            this.disposalAmount = disposalAmount.replaceAll(",", "");
        }
    }
    ...
    {
        return DateManipulations.CalendarToDate(disposalDate);
    }

    public String getEffectiveDate()
    {
        return DateManipulations.CalendarToDate(effectiveDate);
    }
    ...
    public int insertGroupAssetDisposalRecordUpload(String groupId)
    throws Exception, Throwable
{
...
//        String asset_code = Integer.toString(newcode);
    String budget[] = getBudgetInfo();
    double bugdetvalues[] = getBudgetValues();
    System.out.println("=====insertGroupAssetDisposalRecordUpload asset_id=====> "+asset_id+"   gid: "+gid);
    int DONE = 0;
    int BUDGETENFORCED = 1;
    int BUDGETENFORCEDCF = 2;
    int ASSETPURCHASEBD = 3;
...
//        dbConnection.closeConnection(con, ps, rs);
    ...
    public String getQuarter()
    {
        ...
        int month = dateFormat.getDayDifference(sdf.format(disposalDate.getTime()), budg[0]) / 30;
        boolean btw = dateFormat.isDateBetween(budg[0], budg[2], sdf.format(disposalDate.getTime()));
        if(btw)
        {
            if((double)month <= q1)
            {
  ...
  //      dbConnection.closeConnection(con, ps, rs);
    ...
    public boolean chkBudgetAllocation(String quarter, double values[], boolean cf)
    {
        ...
        if(cf)
        {
            ...
            {
                result = values[0] - (values[1] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
            } else
            if(quarter.equalsIgnoreCase("2ND"))
            {
                result = (values[0] + values[2]) - (values[1] + values[3] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
            } else
            if(quarter.equalsIgnoreCase("3RD"))
            {
                result = (values[0] + values[2] + values[4]) - (values[1] + values[3] + values[5] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
            } else
            if(quarter.equalsIgnoreCase("4TH"))
            {
                result = (values[0] + values[2] + values[4] + values[6]) - (values[1] + values[3] + values[5] + values[7] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
            }
        } else
        if(quarter.equalsIgnoreCase("FIRST"))
        {
        ...
        if(quarter.equalsIgnoreCase("2ND"))
        {
            result = values[2] - (values[3] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
        } else
        if(quarter.equalsIgnoreCase("3RD"))
        {
            result = values[4] - (values[5] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
        } else
        if(quarter.equalsIgnoreCase("4TH"))
        {
            result = values[6] - (values[7] + Double.parseDouble(disposalAmount.replaceAll(",", "")));
        }
        if(result < 0.0D)
        {
            allocation = false;
