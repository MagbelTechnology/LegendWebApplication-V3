...
public class DisposalExcelUploadManager extends MagmaDBConnection
{
    ...
    public DisposalExcelUploadManager(String uid,String groupAssetByAsset,String groupId)
    { 
        userid = uid;
        groupID = groupId;
...
//        System.out.println("excelType: "+excelType);
    ...
    private void insertCellContents(Cell cell[])
    {
        ...
        ExcelAssetDisposalBean asset = null;
        boolean bad = false;
        String empty = "";
        String assetId = "";
        String disposalAmount = "0";
        String wh_taxRate = "0";
        String wh_tax_cb = "N";
        String wh_tax = "S";
        String subjectToVat = "";
...
//        String deleterecord = approve.getCodeName("DELETE FROM am_uploadCheckErrorLog WHERE TRANSACTION_TYPE = 'Asset Creation'");
        ...
        if(excelType.equalsIgnoreCase("Y")){
            ...
            disposalAmount = cell[3].getContents();        
            disposalDate = cell[4].getContents();  
            effectiveDate = cell[5].getContents();
            wh_taxRate = cell[6].getContents();                
            wh_tax = cell[7].getContents();          
            subject_to_vat = cell[8].getContents();  
            vatRate = cell[9].getContents();  
            lpo_no = cell[10].getContents();         
            invoice_No = cell[11].getContents();
            Vendor_Account = cell[12].getContents();
            disposalReason = cell[13].getContents();
        }
        if(excelType.equalsIgnoreCase("N")){
            sNo = cell[0].getContents();
            assetId = cell[1].getContents();  
            System.out.println("<<<<<<<<assetId: "+assetId);
            disposalAmount = cell[2].getContents();  
            System.out.println("<<<<<<<<disposalAmount: "+disposalAmount);
            disposalDate = cell[3].getContents();  
            effectiveDate = cell[4].getContents();
            lpo_no = cell[5].getContents();         
            invoice_No = cell[6].getContents();
            Vendor_Account = cell[7].getContents();
            disposalReason = cell[8].getContents();
        }
           System.out.println("<<<<<<sNo: "+sNo+"  assetId: "+assetId+"  sbu_code: "+sbu_code+"  vatable_cost: "+disposalAmount+"  wh_tax_amount: "+wh_taxRate+"  location: "+location+"   projectCode: "+projectCode);
            sheetno = sheetno + 1;
        }
        catch(Exception ex)
        {
...
//        System.out.println("<<<<<<<status: "+status+"    sNo: "+sNo);
     //   String deleterecord = approve.getCodeName("DELETE FROM am_uploadCheckErrorLog WHERE TRANSACTION_TYPE = 'Asset Creation'");
        ...
        status = true;        	
        String error = "";
        String param = "select cast(ASSET_CODE as varchar(50))+'#'+BRANCH_CODE+'#'+CATEGORY_CODE from AM_ASSET where ASSET_ID = ? AND ASSET_STATUS = 'ACTIVE'";
//        System.out.println("<<<<<<<param: "+param); 
        String assetparam = approve.getCodeName(param,assetId);
...
//        System.out.println("<<<<<<<assetCode Before=====: "+assetCode+"   status: "+status);
        if(status && !assetCode.equals(""))
        {
//        	System.out.println("<<<<<<<assetCode After=====: "+assetCode+"   status: "+status);
            try
            {
                ...
                double cost = Double.parseDouble(vat_amount) + Double.parseDouble(disposalAmount);
                asset = new ExcelAssetDisposalBean();
                ...
                asset.setDisposalAmount(disposalAmount);
                asset.setWh_tax_amount(wh_taxRate);
                ...
                asset.setDisposalReason(disposalReason);
    ...
    private void addCellContent(ExcelAssetDisposalBean asset,String groupId)
    {
        int staux = 0;
        try
        {
            staux = asset.insertGroupAssetDisposalRecordUpload(groupId);
        }
        catch(Exception e)
        {
            e.printStackTrace();
