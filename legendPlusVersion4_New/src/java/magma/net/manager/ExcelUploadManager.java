...
public class ExcelUploadManager extends MagmaDBConnection {
    ...
    String groupIdent = "";
    public ExcelUploadManager(String groupId) {
...
System.out.println("<<<<<<<<<<groupIdent in  insertCellContents >>>>>>>: "+groupIdent);
 ...
 //       System.out.println("<<<<budgetexist in addCellContent: "+budgetexist);
        if(budgetexist.equals("0")){
	        ...
	        categoryName,sbuCode,categoryCode, firstQauterAmount,secondQuaterAmount,
	        thirdQauterAmount,fourthQuaterAmount,totalAmount,balanceAmount, type, groupIdent);
        }
        if(budgetexist!="0"){
        		allocate.extralBudgetary(branchCode, branchName,categoryName,sbuCode,
        		categoryCode,firstQauterAmount,secondQuaterAmount,thirdQauterAmount,fourthQuaterAmount);
        }
    }
