    ...
    public void processDepreciation(String userid) {
       ...
       //-- System.out.println("size  " + asets.size());
        for (int x = 0; x < asets.size(); x++) {
            ...
            isImprovement = false;
           //isNew = false;
            String asset_Id = aset.getAssetId();

            String insertrec =  appRecord.getCodeName("INSERT INTO DEPRECIATION_PROCESSING_VALIDATION (ASSET_ID)VALUES('"+aset.getAssetId()+"')");
...
//                    System.out.println(" The value Status >>>>>>>>> before depreciation " + astatus);
                    if (astatus.equalsIgnoreCase("ACTIVE")) {
                        ...
                        System.out.println("<<<<<<=====improv_remainLife: "+improv_remainLife);
                        if(improv_remainLife != null && !improv_remainLife.equalsIgnoreCase("") && !improv_remainLife.equalsIgnoreCase("0")){
                        ...
                        System.out.println("<<<<<<=====isimproved: "+isimproved);
...
//        ps4 = null;
        ps5 = null;
        System.out.println("<<<<<<<<<<assetTran.getAssetId(): "+assetTran.getAssetId());
...
//        query4 = "UPDATE AM_ASSET SET TOTAL_NBV = NBV+coalesce(IMPROV_NBV,0) WHERE ASSET_ID = ?";
       ...
       // ps = con.prepareStatement(query2);
        ...
        System.out.println("<<<<<<<Asset Id>>>>>>: "+assetTran.getAssetId()+"<<<<<<<depreciationmonthly: "+depreciationmonthly+"   <<<<<improve_usefullife: "+improve_usefullife+"  <<<<<<remainLife: "+remainLife+"   <<<<<<isImprovement: "+isImprovement+"   <<<<<<<<<<<improvremainLife: "+improvremainLife+"  <<<<<<Frequency: "+assetTran.getFrequency()+"  <<<<<<<<<<newmonthlyDep: "+newmonthlyDep);
        if(remainLife > 0){
...
/*            if(isImprovement)
 ...
 //       System.out.println("=====Exists for Monthly Insertion=====> "+exists);
    ...
    public boolean validationtablesap()
    {
        String sql;
        boolean exists;
        Connection con;
        Statement stmt;
        ResultSet rs;
        System.out.println("About to Delete records from file DEPRECIATION_PROCESSING_VALIDATION");
        String Sapquery="delete from DEPRECIATION_PROCESSING_VALIDATION  ";
 //       System.out.println(Sapquery);
        exists = false;
        con = null;
        stmt = null;
