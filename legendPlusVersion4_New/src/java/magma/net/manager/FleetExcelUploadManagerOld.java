...
public class FleetExcelUploadManagerOld extends MagmaDBConnection
{
    ...
    public FleetExcelUploadManagerOld(String uid,String transType)
    {
        userid = uid;
        tranType = transType;
...
//        String deleterecord = approve.getCodeName("DELETE FROM am_uploadCheckErrorLog WHERE TRANSACTION_TYPE = 'Asset Creation'");
        ...
        { 
            ...
            effective_Date = cell[4].getContents();
            vendorCode = cell[5].getContents();
...
//        System.out.println("<<<<<<<status: "+status+"    sNo: "+sNo);
     //   String deleterecord = approve.getCodeName("DELETE FROM am_uploadCheckErrorLog WHERE TRANSACTION_TYPE = 'Asset Creation'");
        ...
        String param = "select cast(ASSET_CODE as varchar(50))+'#'+ cast(BRANCH_ID as varchar(50))+'#'+BRANCH_CODE+'#'+CATEGORY_CODE+'#'+DESCRIPTION+'#'+ASSET_ID+'#'+SBU_CODE from AM_ASSET where registration_No = '"+oldregNo+"' AND ASSET_STATUS = 'ACTIVE'";
        ...
        {
        	System.out.println("<<<<<<<assetCode: "+assetCode+"   assetId: "+assetId+"   status: "+status);
            try
            {
