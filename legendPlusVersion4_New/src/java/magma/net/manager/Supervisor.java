...
//		System.out.println("=======query_ " + query_); 
		...
		String SELECT_QUERY = "SELECT DISTINCT a.BATCH_ID,a.TRANSACTION_ID,a.DR_ACCT,CR_ACCT,a.DR_ACCT_TYPE,a.CR_ACCT_TYPE,"
					...
					+ "OR a.PROCESS_STATUS = 'Q'  OR a.PROCESS_STATUS = 'C' OR a.PROCESS_STATUS = 'PR' OR a.PROCESS_STATUS = 'R' OR a.PROCESS_STATUS = 'WA' OR a.PROCESS_STATUS = 'WAA') "+query_+"  and a.posting_date between getdate()-365 and getdate() "
					+ " UNION "
					...
					+ "WHERE a.BATCH_ID IS NOT NULL AND (a.PROCESS_STATUS = 'P' OR a.PROCESS_STATUS = 'F' OR a.PROCESS_STATUS = 'A'  OR a.PROCESS_STATUS = 'U' "
					+ "OR a.PROCESS_STATUS = 'Q'  OR a.PROCESS_STATUS = 'C' OR a.PROCESS_STATUS = 'PR' OR a.PROCESS_STATUS = 'R' OR a.PROCESS_STATUS = 'RP' OR a.PROCESS_STATUS = 'WA' OR a.PROCESS_STATUS = 'WAA') "+query_+"  and a.posting_date between getdate()-365 and getdate() "
					+ " UNION "
					+"SELECT DISTINCT a.BATCH_ID,a.TRANSACTION_ID,a.DR_ACCT,CR_ACCT,a.DR_ACCT_TYPE,a.CR_ACCT_TYPE,"
					+ "a.DR_TRAN_CODE,a.CR_TRAN_CODE,a.DR_NARRATION,a.CR_NARRATION,a.AMOUNT,a.USER_ID,"
					+ "a.SUPER_ID,a.LEGACY_ID,a.POSTING_DATE,"
					...
					+ "ON (a.ASSET_ID=c.ASSET_ID)  "
					+ "WHERE a.BATCH_ID IS NOT NULL AND (a.PROCESS_STATUS = 'R' OR a.PROCESS_STATUS = 'RP' OR a.PROCESS_STATUS = 'A' OR a.PROCESS_STATUS = 'P' OR a.PROCESS_STATUS = 'WA' OR a.PROCESS_STATUS = 'WAA') AND a.PROCESSING IS NULL "+query_+" and a.posting_date between getdate()-365 and getdate() and a.POSTING_DATE between getdate()-365 and getdate() "
					...
					+ "ON a.SUPER_ID = b.USER_ID  LEFT JOIN AM_ASSET c " 
					+ "ON (a.ASSET_ID=c.ASSET_ID)  "
					+ "WHERE a.BATCH_ID IS NOT NULL AND (a.PROCESS_STATUS = 'R' OR a.PROCESS_STATUS = 'RP' OR a.PROCESS_STATUS = 'R' OR a.PROCESS_STATUS = 'WA' OR a.PROCESS_STATUS = 'WAA') AND a.PROCESSING = 'Processing Rejection' "+query_+" and a.posting_date between getdate()-365 and getdate() and a.DATE_APPROVED between getdate()-365 and getdate() "
					+ " UNION "
...
//        System.out.println("status in findTransactionForApproval >>>>>>>>>>> " + status+"    tranType: "+tranType); 
    ...
    public ArrayList findTransactionForTransferAcceptance(String query_,String order,String branch,String branchFld1,String branchFld2)
    {
       // System.out.println("in method and the query is ............." +query_);
        ...
        String SELECT_QUERY = "SELECT DISTINCT CONVERT(VARCHAR(10),a.POSTING_DATE,101),a.description,a.DR_ACCT,a.CR_ACCT," +
        		...
        		"a.ASSET_ID,a.transaction_id,a.tran_sent_time,a.tran_type,a.USER_ID, " +
        		"a.transaction_level,a.approval_level_count,a.asset_code,a.RecordType FROM " +
        		"am_asset_approval a, AM_GB_USER b,am_assetTransfer t where  a.USER_ID = b.USER_ID and a.asset_id = t.asset_id AND "+branchFld1+" = "+branch+"  "+query_+" "+
        		"UNION "+
        		"SELECT DISTINCT CONVERT(VARCHAR(10),a.POSTING_DATE,101),a.description,a.DR_ACCT,a.CR_ACCT," +
        		"a.DR_NARRATION,a.CR_NARRATION,a.AMOUNT,a.POSTING_DATE,'' AS FULL_NAME," +
        		"a.ASSET_ID,a.transaction_id,a.tran_sent_time,a.tran_type,a.USER_ID, " +
        		"a.transaction_level,a.approval_level_count,a.asset_code,a.RecordType FROM " +
        		"am_asset_approval a, AM_GB_USER b,am_gb_bulkTransfer t where a.USER_ID = b.USER_ID and a.asset_id = CONVERT(varchar, t.Batch_id) AND "+branchFld2+" = "+branch+" "+query_+" "+order+"";
//        System.out.println("SELECT_QUERY >>>>>>>>>>> " + SELECT_QUERY);  
		ArrayList list = new ArrayList();
		Connection con = null;
		PreparedStatement ps = null;
...
public ArrayList findTransactionForAcceptanceApproval(String query_,String order)
{
   // System.out.println("in method and the query is ............." +query_);
    ...
    String SELECT_QUERY = "SELECT DISTINCT CONVERT(VARCHAR(10),a.POSTING_DATE,101),a.description,a.DR_ACCT,a.CR_ACCT," +
    		...
    		"a.ASSET_ID,a.transaction_id,a.tran_sent_time,a.tran_type,a.USER_ID, " +
    		"a.transaction_level,a.approval_level_count,a.asset_code,a.RecordType FROM " +
    		"am_asset_approval a JOIN AM_GB_USER b ON a.USER_ID = b.USER_ID AND ((asset_status = 'Bulk Asset Transfer') OR (asset_status = 'Transferred'))  "+query_+" "+
    		"UNION "+
    		"SELECT DISTINCT CONVERT(VARCHAR(10),a.POSTING_DATE,101),a.description,a.DR_ACCT,a.CR_ACCT," +
    		"a.DR_NARRATION,a.CR_NARRATION,a.AMOUNT,a.POSTING_DATE,'' AS FULL_NAME," +
    		"a.ASSET_ID,a.transaction_id,a.tran_sent_time,a.tran_type,a.USER_ID, " +
    		"a.transaction_level,a.approval_level_count,a.asset_code,a.RecordType FROM " +
    		"am_asset_approval a JOIN AM_GB_USER b ON a.USER_ID IS NOT NULL AND ((asset_status = 'Asset Transfer') OR (asset_status = 'Transferred')) "+query_+" "+order+"";
...
public ArrayList findTransactionForUploadDeletionList(String query_)
{
   // System.out.println("in method and the query is ............." +query_);
		

    String SELECT_QUERY = "SELECT disposal_id AS Id,'Asset Disposal Upload with Id '+CONVERT(varchar,disposal_id) AS description, SUM(COST_PRICE) AS COST_PRICE, "
    		+ "'DISPOSAL ASSET' AS TRANSTYPE,Disposal_Date AS POSTING_DATE FROM AM_GROUP_DISPOSAL WHERE APPROVED = 'N' "+query_+" GROUP BY disposal_id,Disposal_Date "
    		+ "UNION "
    		+ "SELECT Group_id AS Id,'Asset Upload with Id '+CONVERT(varchar,Group_id) AS description, SUM(COST_PRICE) AS COST_PRICE, "
    		+ "'NEW ASSET' AS TRANSTYPE,Posting_Date AS POSTING_DATE FROM AM_GROUP_ASSET  WHERE APPROVED = 'N' "+query_+" GROUP BY Group_id,Posting_Date "
    		+ "UNION "
    		+ "SELECT Revalue_ID AS Id,'Asset Improvement Upload with Id '+CONVERT(varchar,Revalue_ID) AS description, SUM(COST_PRICE) AS COST_PRICE, "
    		+ "'IMPROVEMENT ASSET' AS TRANSTYPE,revalue_Date AS POSTING_DATE FROM AM_GROUP_IMPROVEMENT WHERE APPROVED = 'N' "+query_+" GROUP BY Revalue_ID,revalue_Date "
			+ "UNION "
			+ "SELECT Group_id AS Id,'Uncapitalized Asset Upload with Id '+CONVERT(varchar,Group_id) AS description, SUM(COST_PRICE) AS COST_PRICE, "
			+ "'NEW UNCAPITALIZED ASSET' AS TRANSTYPE,Posting_Date AS POSTING_DATE FROM AM_GROUP_ASSET_UNCAPITALIZED  WHERE APPROVED = 'N' "+query_+" GROUP BY Group_id,Posting_Date ";
	...
	ResultSet rs = null;
	try {
//		System.out.println("the size of the SELECT_QUERY is>>>>" +SELECT_QUERY);
		con = getConnection("legendPlus");
		ps = con.prepareStatement(SELECT_QUERY);
		rs = ps.executeQuery();

		while (rs.next()) {
			double amount = rs.getDouble("COST_PRICE");
			String assetId = (rs.getString("ID")==null)?"":rs.getString("ID");
			String desc =(rs.getString("DESCRIPTION")==null)?"":rs.getString("DESCRIPTION");
            String tran_type = (rs.getString("TRANSTYPE")==null)?"":rs.getString("TRANSTYPE");
            String postingDate = (rs.getString("POSTING_DATE")==null)?"":formatDate(rs.getDate("POSTING_DATE"));

			Transaction tran = new Transaction("",
					"", "", "", amount,
					postingDate,"", assetId, desc,"","",tran_type,0,0);
			list.add(tran);

		}

	} catch (Exception e) {
		String warning = "WARNING:Error Fetching Transaction for findTransactionForUploadDeletionList"
				+ " ->" + e.getMessage();
		System.out.println(warning);
	} finally {
		closeConnection(con, ps, rs);
