...
public class Asset2ExcelUploadManagerOld extends MagmaDBConnection
{
    ...
    public Asset2ExcelUploadManagerOld(String uid)
    {
        userid = uid;
...
//        String deleterecord = approve.getCodeName("DELETE FROM am_uploadCheckErrorLog WHERE TRANSACTION_TYPE = 'Asset Creation'");
        try
        {
            ...
            wh_tax_amount = cell[9].getContents();          
            ...
            model = cell[28].getContents();
            make = cell[29].getContents();
            vendor = cell[30].getContents();
            assetMaintainBy = cell[31].getContents();
            purchaseReason = cell[32].getContents();
            location = cell[33].getContents();
            projectCode = cell[34].getContents();
            regionCode = cell[35].getContents();
            zoneCode = cell[36].getContents();
...
//        System.out.println("<<<<<<<status: "+status);
     //   String deleterecord = approve.getCodeName("DELETE FROM am_uploadCheckErrorLog WHERE TRANSACTION_TYPE = 'Asset Creation'");
        ...
        System.out.println("<<<<<<<zonecodeValid: "+zonecodeValid);
...
//        System.out.println("<<<<errorMessage: "+errorMessage+"  status: "+status);
        if(!status){
        boolean rec = insertErrorTransaction(errorMessage,userid,transType);
        }
         }
...
//                System.out.println("<<<<<depreciation_end_date=====: "+depreciation_end_date);
 ...
 //               System.out.println("<<<<<<<<purchaseReason: "+purchaseReason);
    ...
    private void addCellContent(ExcelAsset2Bean asset)
    {
        int staux = 0;
        try
        {
...
public boolean insertErrorTransaction(String errorMessge,String userId, String transType) {
	  boolean done=true;
