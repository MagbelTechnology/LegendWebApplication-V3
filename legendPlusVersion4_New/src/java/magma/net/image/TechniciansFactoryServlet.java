    ...
    private ArrayList findAllTechniciansByType(String type) {
        ...
        if (type.equalsIgnoreCase("I")) {
            SELECT_QUERY = "SELECT EMPLOYEE_ID,EMPLOYEE_NAME  " +
                           "FROM am_ad_employee WHERE EMPLOYEE_STATUS='ACTIVE' ORDER BY EMPLOYEE_NAME ";
        } else {
            SELECT_QUERY = "SELECT VENDOR_ID,VENDOR_NAME " +
                           "FROM AM_AD_VENDOR WHERE VENDOR_STATUS='ACTIVE' ORDER BY VENDOR_NAME";
        }
        Connection con = null;
...
//        System.out.println("<<<<<<SELECT_QUERY: "+SELECT_QUERY+"   type: "+type);
        try {
            con = mCon.getConnection("legendPlus");
