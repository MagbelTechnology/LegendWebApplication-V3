...
public class ExcelAssetBean extends ConnectionClass
{
...
//    	assetMaintainBy = "";
        ...
        throws Exception
    {
        ...
        String query = "INSERT INTO AM_GROUP_ASSET(REGISTRATION_NO,BRANCH_ID,DEPT_ID,CATEGORY_ID,SUB_CATEGORY_ID,SECTION" +
...
"PROJECT_CODE,REGION_CODE,ZONE_CODE,VENDOR_NAME" +
" )\tVALUES('";
...
if(done==true){
	id ="SUCCESS";
	htmlUtil.insToAm_Invoice_No(gid,lpo_no,invoice_No,"Asset Creation Upload");
}else{
	id ="FAILED";
}
...
public void setAsset_id(String asset_id)
{
    ...
    public int insertGroupAssetRecordUpload(String groupId)
    throws Exception, Throwable
{
...
//        try
//        {
//            for(rs = getStatement().executeQuery(query); rs.next();)
//            {
...
//    }
    ...
    public String getCatCode()
    {
        ...
        try
        {
            ...
            for(rs = conn.createStatement().executeQuery(query); rs.next();)
            {
                catid = rs.getString(1);
            }
        ...
        catch(Exception ex)
        {
        	...
        	dbConnection.closeConnection(conn,statement,rs);
...
//        System.out.println("Exiting update of Aquicisition Budget due to Reclassification");
 ...
 //       ResultSet rs = null;
    ...
    public int insertGroupUncapitalizedAssetRecordUpload(String groupId)
    throws Exception, Throwable
{
        asset_id = new legend.AutoIDSetup().getIdentity(branch_id,
                department_id, section_id, category_id);
...
//    System.out.println("=====insertGroupUncapitalizedAssetRecordUpload asset_id=====> "+asset_id+" gid: "+gid);
    ...
    "ASSET_STATUS,SBU_CODE,ASSET_CODE,SUB_CATEGORY_ID,SUB_CATEGORY_CODE,SPARE_3,SPARE_4,SPARE_5,SPARE_6,PROJECT_CODE,REGION_CODE,ZONE_CODE,VENDOR_NAME) " +
    "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
    "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    if(done==true){
    	id ="SUCCESS";
    	htmlUtil.insToAm_Invoice_No(gid,lpo_no,invoice_No,"Uncap Asset Creation Upload");
    }else{
    	id ="FAILED";
    }
    ...
    throws Exception, Throwable
{
        asset_id = new legend.AutoIDSetup().getIdentity(branch_id,
                department_id, section_id, category_id);
  //      String codeno = approve.getCodeName("select mt_id from IA_MTID_TABLE where mt_tablename = 'AM_ASSET'");
        String codeno =  new ApplicationHelper().getGeneratedId("AM_ASSET2"); 
        int newcode = Integer.parseInt(codeno);
        String gid = approve.getCodeName("select mt_id from IA_MTID_TABLE where mt_tablename = 'am_group_asset2_main'");
//    	String gid =  new ApplicationHelper().getGeneratedId("AM_GROUP_ASSET_MAIN"); 
        String asset_code = Integer.toString(newcode);
/*    String budget[] = getBudgetInfo();
    double bugdetvalues[] = getBudgetValues();  */
...
public int UpdateAssetVerificationRecordUpload(String groupId)
throws Exception, Throwable
{
	int value = 0;
	int DONE = 0;
//	String gid = approve.getCodeName("select mt_id from IA_MTID_TABLE where mt_tablename = 'am_group_asset_main'");
//	System.out.println("===>gid in UpdateAssetVerificationRecordUpload: "+gid);
//	updateAmAssetWithVerification();
	String gid = groupId+user_id; 
	createGroupUpListVrifyload(gid);
	value = DONE; 
return value;
}


public void createGroupUpListVrifyload(String gid)
throws Exception
{
	Connection con = null;
	PreparedStatement ps = null;
...
//System.out.println("=====createGroupUpListVrifyload asset_id=====> "+asset_id+"    groupId: "+gid);
//System.out.println("=====createGroupUpListVrifyload description=====> "+description+"    user_id: "+user_id);
String query = "INSERT INTO AM_GROUP_ASSET_VERIFICATION(ASSET_ID,asset_serial_no,BRANCH_ID" +
",DESCRIPTION,SPARE_1,BRANCH_CODE,BAR_CODE,CATEGORY_CODE,PROCESS_FLAG,POST_FLAG,ASSET_STATUS,GROUP_ID,POSTING_DATE,USER_ID"+
" )VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
try {   
	...
	ps.setString(5, spare_1);
	ps.setString(6, branch_code);
	ps.setString(7, bar_code); 
	ps.setString(8, category_code);
	ps.setString(9, "N");
	ps.setString(10, "P");  
...
e.printStackTrace();
System.out.println(this.getClass().getName()
	+ " INFO: Error creating group aset Verification >> "
		+ e.getMessage());
} finally {
	dbConnection.closeConnection(con, ps);
}
