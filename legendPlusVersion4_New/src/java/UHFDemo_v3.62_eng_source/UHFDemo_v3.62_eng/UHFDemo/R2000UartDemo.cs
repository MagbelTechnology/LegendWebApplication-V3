...
namespace UHFDemo
{
	public partial class R2000UartDemo : Form
	{
		...
		//private MySqlConnection con = new MySqlConnection( "server=localhost;user=root;password=justjava;database=rfid_reader" );
		SqlConnection sqlServerConnection = new SqlConnection("Data Source=localhost;Initial Catalog=legendplus;User ID=sa;Password=magbel");
		
		private String readerIP = "";
		private string readerId  = "";
		ArrayList loadedTags = new ArrayList();
		...
		private void updateAllTag(String state){
			try {
				DateTime Time = DateTime.Now;
				if(sqlServerConnection.State == ConnectionState.Closed)
					sqlServerConnection.Open();
				
				String query = "UPDATE st_inventory_verify_rfid SET SCANN_STATUS='"+state+"'";
				SqlCommand cmd = new SqlCommand(query, sqlServerConnection);
				Debug.WriteLine(" cmd.ExecuteNonQuery()=="+cmd.ExecuteNonQuery());
				sqlServerConnection.Close();
			} catch (Exception exc) {
				MessageBox.Show("updateAllTag ==== Exception Thrown  "+exc.Message);
				
			}
		}

		private void updateTagRecord(String epc, String state){
			try {
				DateTime Time = DateTime.Now;
				if(sqlServerConnection.State == ConnectionState.Closed)
					sqlServerConnection.Open();
				
				String query = "UPDATE st_inventory_verify_rfid SET scann_status='"+state+"' WHERE rfid_tag='"+epc+"'";
				SqlCommand cmd = new SqlCommand(query, sqlServerConnection);
				Debug.WriteLine(" cmd.ExecuteNonQuery()=="+cmd.ExecuteNonQuery());
				sqlServerConnection.Close();
			} catch (Exception exc) {
				MessageBox.Show("Connection Not Open "+exc.Message);
				
			}
		}
		
		private void updateProcessed(String state){
			try {
				DateTime Time = DateTime.Now;
				if(sqlServerConnection.State == ConnectionState.Closed)
					sqlServerConnection.Open();
				
				String query = "UPDATE st_inventory_verify_rfid SET processed='"+state+"' WHERE scann_status='O'";
				SqlCommand cmd = new SqlCommand(query, sqlServerConnection);
				Debug.WriteLine(" cmd.ExecuteNonQuery()=="+cmd.ExecuteNonQuery());
				sqlServerConnection.Close();
			} catch (Exception exc) {
		...
		private void insertTagRecord(String epc){
			try {
				DateTime Time = DateTime.Now;
				if(sqlServerConnection.State == ConnectionState.Closed)
					sqlServerConnection.Open();
				...
				String time = Time.ToString("yyyyMMdd");
				String dateTime = Time.ToString("MM-dd-yyyy HH:mm:ss.fff");
				String query = "INSERT INTO st_inventory_verify_rfid (rfid_tag,scann_type,location,scann_status," +
					"create_date,create_datetime) " +
					"VALUES ('"+epc+"', 'N','"+readerId+"', 'I','"+time+"','"+dateTime+"')";
		...
		private Boolean tagExist(String epc){
			try {
				if(sqlServerConnection.State == ConnectionState.Closed){
					sqlServerConnection.Open();
				}

				String query = "SELECT COUNT(*) FROM st_inventory_verify_rfid WHERE rfid_tag='"+epc+"'";
				SqlCommand cmd = new SqlCommand(query, sqlServerConnection);
				int Count=int.Parse(cmd.ExecuteScalar() + "");
				
				Debug.WriteLine(" The Count=="+Count);
				sqlServerConnection.Close();
				return (Count >0);
			} catch (Exception exc) {
				MessageBox.Show("Connection Not Open "+exc.Message);
		...
		private void loadTags(){
			try {
				...
				String query = "SELECT rfid_tag FROM st_inventory_verify_rfid ";
				SqlCommand cmd = new SqlCommand(query, sqlServerConnection);
				SqlDataReader sqlReader = cmd.ExecuteReader();
				while (sqlReader.Read())
                {
					loadedTags.Add(sqlReader.GetString(0));
                    MessageBox.Show(sqlReader.GetString(0));
                }
                sqlReader.Close();
				sqlServerConnection.Close();
		...
		private void R2000UartDemo_Load(object sender, EventArgs e)
		{
			...
			Console.WriteLine(" Im about loading here.......");
			reader = new Reader.ReaderMethod();
			string setting = System.IO.File.ReadAllText(@"setting.txt");
			Console.WriteLine(" The Content ==" + setting);
			string[] records = setting.Split("\n".ToCharArray());
			...
			foreach (string record in records){
				if(record.Contains("datasource")){
					databaseServer = record.Split("=".ToCharArray())[1];
					Console.WriteLine(" The Datasource ==" + databaseServer);
					
				}
				
				if(record.Contains("databaseName")){
					databaseName = record.Split("=".ToCharArray())[1];
					Console.WriteLine(" The databaseName ==" + databaseName);

					
				}	
				if(record.Contains("userName")){
					userName = record.Split("=".ToCharArray())[1];
					Console.WriteLine(" The userName ==" + userName);
					
				}	
				if(record.Contains("password")){
					password = record.Split("=".ToCharArray())[1];
					Console.WriteLine(" The password ==" + password);
					
				}				

					
				if(record.Contains("reader")){
					readerIP = record.Split("=".ToCharArray())[1];
					Console.WriteLine(" The readerIP ==" + readerIP);					
				}				
			}
			
			String connectionString = "Data Source="+databaseServer+";Network Library=DBMSSOCN;Initial Catalog="+databaseName+";User ID="+userName+";Password="+password;
			//Console.WriteLine("connectionString=="+connectionString);
			//String conStr="Data Source=192.168.6.27;Network Library=DBMSSOCN;Initial Catalog=InventoryDB;User ID=inventoryusr;Password=Inventory#123";
			sqlServerConnection = new SqlConnection(connectionString);	

			Console.WriteLine("sqlServerConnection.ConnectionString=="+sqlServerConnection.ConnectionString);
			//回调函数
			...
			Console.WriteLine("readerIP load =="+readerIP);
			ipIpServer.IpAddressStr = readerIP;
			txtDatabase.Text=databaseServer;
...
//			Console.WriteLine(" strLog ==="+ strLog);
		...
		private void AnalyData(Reader.MessageTran msgTran)
		{
			...
			String s1 = CCommondMethod.ByteArrayToString(msgTran.AryData, 0, msgTran.AryData.Length);
			String s2 = CCommondMethod.ByteArrayToString(msgTran.AryTranData, 0, msgTran.AryTranData.Length);
			...
			Console.WriteLine(" strEPC==="+strEPC);
			if(loadedTags.Contains(strEPC)){
				loadedTags.Remove(strEPC);
			}else{
				insertTagRecord(strEPC);
			}
			
			readerId = Convert.ToChar(msgTran.ReadId).ToString();
			//Console.WriteLine("AnalyData 1 "+msgTran.ReadId +"\r\n 2="+s1 + "\r\n 3="+s2+"\r\n 4="+msgTran.Cmd+"\r\n  5="+msgTran.PacketType);
			
//			if(tagExist(s1)){
//				Console.WriteLine("Update This "+s1);
//				updateTagRecord(s1,"I");
//			}else{
//				Console.WriteLine("Insert  This "+s1);
//				insertTagRecord(s1);
//			}
		...
		private void RefreshInventoryReal(byte btCmd)
		{
			...
			//Console.WriteLine(" RefreshInventoryReal = "+btCmd);
			if (this.InvokeRequired)
			{
					...
					case 0x8B:
						{
							...
							String epc = "";
							//Console.WriteLine(" nEpcCount =="+nEpcCount);
							//Console.WriteLine(" nEpcLength =="+nEpcLength);
...
//								epc = row[2].ToString();
//								//Console.WriteLine("nEpcCount="+nEpcCount+" nEpcLength="+nEpcLength);
//								if(tagExist(row[2].ToString())){
//									Console.WriteLine("Update This "+row[2].ToString());
//									updateTagRecord(row[2].ToString(),"I");
//								}else{
//									Console.WriteLine("Insert  This "+row[2].ToString());
//									insertTagRecord(row[2].ToString());
//								}
								
							}else{
//								nEpcLength = nEpcLength + nEpcLength;
//								Console.WriteLine("Still Running On "+epc + "  nEpcCount=="+nEpcCount);
//								updateProcessed("N");
								//btRealTimeInventory_Click(btRealTimeInventory,null);
							...
							if (m_nTotal % m_nRealRate == 1)
							{
								...
								foreach (DataRow row in m_curInventoryBuffer.dtTagTable.Rows)
								{
									...
									//Console.WriteLine("row[5].ToString()"+row[5].ToString());
					...
					case 0x00:
						{
							...
							txtFastTagList.Text = "Tag List: " + nTagCount.ToString();
					...
					case 0xb4:
						{
							...
							{
								case 0x00:
									txtStatus.Text = "The byte is unlock";
									break;
								case 0xFE:
									txtStatus.Text = "The byte is already locked";
									break;
								default:
									break;
							}
		...
		private void enableTcpIp(){
			...
			//{
				...
				gbCmdReaderAddress.Enabled=true;
		...
		private void rdbTcpIp_CheckedChanged(object sender, EventArgs e)
		{
			...
			Console.WriteLine(" rdbTcpIp got fired.. with readerIP=="+readerIP);

			rdbTcpIp.Checked=true;
			if (rdbTcpIp.Checked)
			{
				gbTcpIp.Enabled = true;
				gbCmdReaderAddress.Enabled=true;
				...
				gbRS232.Enabled = false;
				//btnConnectTcp_Click(btnConnectTcp,new EventArgs());
			}
		}
		...
		private void btnConnectRs232_Click(object sender, EventArgs e)
		{
			...
			//Console.WriteLine(" The nBaudrate==="+nBaudrate);
		...
		private void btnConnectTcp_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				ipIpServer.IpAddressStr = readerIP;
				txtTcpPort.Text = "4001";
				//Console.WriteLine("1 txtTcpPort.Text =="+txtTcpPort.Text);
				//updateAllTag("O");
//				try{
//					sqlServerConnection.Open();
//					MessageBox.Show("Connection Open");
//				}catch(Exception exc){
//					MessageBox.Show("Connection Cannot be Opened");
//				}
				
				//处理Tcp连接读写器
				string strException = string.Empty;
				IPAddress ipAddress = IPAddress.Parse(ipIpServer.IpAddressStr);
				Console.WriteLine("2a txtTcpPort.Text =="+txtTcpPort.Text);
				int nPort = Convert.ToInt32(txtTcpPort.Text);
				Console.WriteLine("2b txtTcpPort.Text =="+ipAddress+"  nPort=="+nPort);
				


				Console.WriteLine("2c reader =="+reader);
				int nRet = reader.ConnectServer(ipAddress,nPort,out strException);
				Console.WriteLine("0 2c txtTcpPort.Text =="+txtTcpPort.Text
				                 +" nRet=="+nRet);
				...
				{
					string strLog = "Reader connected  " +
						ipIpServer.IpAddressStr + "@" +
						nPort.ToString() + " And Reading Already";
						Console.WriteLine("1 2c txtTcpPort.Text =="+txtTcpPort.Text
				                 +" nRet=="+nRet);
					WriteLog(lrtxtLog, strLog, 0);
					Console.WriteLine("2 2c txtTcpPort.Text =="+txtTcpPort.Text
				                 +" nRet=="+nRet);
					btRealTimeInventory_Click(btRealTimeInventory,null);
					Console.WriteLine("3 2c txtTcpPort.Text =="+txtTcpPort.Text
				                 +" nRet=="+nRet);					
				}

				Console.WriteLine("3 txtTcpPort.Text =="+txtTcpPort.Text);
				//处理界面元素是否有效
				SetFormEnable(true);
		...
		{
			string strCmd = "Set reader RS-485 address ";
		...
		{
			string strCmd = "Get firmware version ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Get internal temperature ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Get current working antenna ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Set buzzer behavior ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Get antenna connection detector sensitivity threshold ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Get Impinj Monza tag FastTID status ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Set Impinj Monza tag FastTID status ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Measure RF port return loss ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Set antenna connection detector sensitivity threshold ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "";
			//Console.WriteLine(" ProcessInventoryReal =="+ msgTran);
			if (msgTran.Cmd == 0x89)
			{
			...
			else
			{
				...
				//Console.WriteLine("  strEPC==="+strEPC + "  strPC==="+strPC);
		...
		{
			//Console.WriteLine("2 msgTran =="+msgTran);
			string strCmd = "Buffer mode inventory ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Read and clear buffer ";
			string strErrorCode = string.Empty;
		...
		{
			string strCmd = "Query buffered tag quantity ";
			string strErrorCode = string.Empty;
		...
		private void btnSetAccessEpcMatch_Click(object sender, EventArgs e)
		{
			...
			if (reslut == null)
			{
				MessageBox.Show("Please select an EPC ");
				return;
			}
		...
		{
			string strCmd = "Select/unselect tag ";
			string strErrorCode = string.Empty;
		...
		private void btnReadTag_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					MessageBox.Show("Please select a memory bank ");
					return;
				}
				...
				{
					MessageBox.Show("Please input start address");
					return;
				}
		...
		private void btnWriteTag_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (reslut == null)
				{
					MessageBox.Show("Invalid characters have been input");
					return;
				}
				else if (reslut.GetLength(0) < 4)
				{
					MessageBox.Show("At least input 4 bytes ");
					return;
				}
		...
		private void btnLockTag_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please select a lock type ");
				return;
			}
		...
		private void btnInventoryISO18000_Click(object sender, EventArgs e)
		{
			if (m_bContinue)
			{
				...
				btnInventoryISO18000.Text = "Start 18000-6B tag inventory ";
			}
			else
			{
				//判断EPC盘存是否正在进行
				if (m_curInventoryBuffer.bLoopInventory)
				{
					if (MessageBox.Show("EPC C1G2 tag inventory is ongoing, stop it ? ", "Note", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
					{
						return;
					}
					else
		...
		private void btnReadTagISO18000_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please select a UID");
				return;
			}
			...
			{
				MessageBox.Show("At least input 8 bytes ");
				return;
			}
		...
		private void WriteTagISO18000()
		{
			...
			{
				MessageBox.Show("Please input the data to be written ");
				return;
			}
		...
		private void btnLockTagISO18000_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please input lock address ");
				return;
			}

			//确认写保护提示
			if (MessageBox.Show("Lock operation is permanent, proceed?", "Note", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
			{
				return;
			}
			...
			else
			{
				...
				switch (msgTran.AryData[1])
				{
					case 0x00:
						...
						break;
					case 0xFE:
						strLog = strCmd + ": " + "the byte is already locked";
						break;
					case 0xFF:
						strLog = strCmd + ": " + "can not lock the byte ";
						break;
					default:
						break;
				}
		...
		private void btnQueryTagISO18000_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please input query address ");
				return;
			}
...
//			foreach(String tag in loadedTags){
//				updateTagRecord(tag,"O");
//			}
//			loadTags();
		...
		private void btRealTimeInventory_Click(object sender, EventArgs e)
		{
			try
			{
				Console.WriteLine(" Im now inside btRealTimeInventory_Click textRealRound.Text.Length="+
				                 textRealRound.Text.Length);
				m_curInventoryBuffer.ClearInventoryPar();
		...
		private void btFastInventory_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				{
					MessageBox.Show("Please select at least one antenna and repeat at least one time ");
					m_bInventory = false;
					m_curInventoryBuffer.bLoopInventory = false;
					btFastInventory.BackColor = Color.WhiteSmoke;
					btFastInventory.ForeColor = Color.DarkBlue;
					btFastInventory.Text = "Start inventory ";
					return;
				}
		...
		private void btSetIdentifier_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					MessageBox.Show("Please input 12 bytes ");
					return;
				}
		...
		{
			String dataSource = txtDatabase.Text;
			String connectionString = "Data Source="+dataSource+";Initial Catalog=legendplus;User ID=sa;Password=magbel";
			Console.WriteLine("connectionString=="+connectionString);
			sqlServerConnection = new SqlConnection(connectionString);
		}
